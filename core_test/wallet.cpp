...
unsigned constexpr vxldollar::wallet_store::version_current;

TEST (wallet, no_special_keys_accounts)
{
	bool init;
	vxldollar::mdb_env env (init, vxldollar::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	vxldollar::kdf kdf{ vxldollar::dev::network_params.kdf_work };
	vxldollar::wallet_store wallet (init, kdf, transaction, vxldollar::dev::genesis->account (), 1, "0");
	ASSERT_FALSE (init);
	...
	for (uint64_t account = 0; account < vxldollar::wallet_store::special_count; account++)
	{
		vxldollar::account account_l (account);
...
TEST (wallet, one_item_iteration)
{
	...
	{
		ASSERT_EQ (key1.pub, vxldollar::uint256_union (i->first));
		vxldollar::raw_key password;
		wallet.wallet_key (password, transaction);
		vxldollar::raw_key key;
		key.decrypt (vxldollar::wallet_value (i->second).key, password, (vxldollar::uint256_union (i->first)).owords[0].number ());
...
TEST (wallet, two_item_iteration)
{
	...
	std::unordered_set<vxldollar::public_key> pubs;
	std::unordered_set<vxldollar::raw_key> prvs;
	vxldollar::kdf kdf{ vxldollar::dev::network_params.kdf_work };
	{
		auto transaction (env.tx_begin_write ());
		vxldollar::wallet_store wallet (init, kdf, transaction, vxldollar::dev::genesis->account (), 1, "0");
		...
		for (auto i (wallet.begin (transaction)), j (wallet.end ()); i != j; ++i)
		{
			...
			wallet.wallet_key (password, transaction);
			vxldollar::raw_key key;
			key.decrypt (vxldollar::wallet_value (i->second).key, password, (i->first).owords[0].number ());
			prvs.insert (key);
		}
	}
...
TEST (wallet, insufficient_spend_one)
{
	...
	auto block (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key1.pub, 500));
	ASSERT_NE (nullptr, block);
	ASSERT_EQ (nullptr, system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key1.pub, vxldollar::dev::constants.genesis_amount));
}
...
TEST (wallet, spend_all_one)
{
	...
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	vxldollar::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, std::numeric_limits<vxldollar::uint128_t>::max ()));
	vxldollar::account_info info2;
	{
		auto transaction (node1.store.tx_begin_read ());
		node1.store.account.get (transaction, vxldollar::dev::genesis_key.pub, info2);
...
TEST (wallet, send_async)
{
	...
	std::thread thread ([&system] () {
		ASSERT_TIMELY (10s, system.nodes[0]->balance (vxldollar::dev::genesis_key.pub).is_zero ());
	});
	std::atomic<bool> success (false);
	system.wallet (0)->send_async (vxldollar::dev::genesis_key.pub, key2.pub, std::numeric_limits<vxldollar::uint128_t>::max (), [&success] (std::shared_ptr<vxldollar::block> const & block_a) { ASSERT_NE (nullptr, block_a); success = true; });
	thread.join ();
...
TEST (wallet, change)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->change_action (vxldollar::dev::genesis_key.pub, key2.pub));
	auto block2 (system.nodes[0]->rep_block (vxldollar::dev::genesis_key.pub));
...
TEST (wallet, partial_spend)
{
	...
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	vxldollar::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, 500));
	ASSERT_EQ (std::numeric_limits<vxldollar::uint128_t>::max () - 500, system.nodes[0]->balance (vxldollar::dev::genesis_key.pub));
}
...
TEST (wallet, spend_no_previous)
{
	vxldollar::system system (1);
	{
		...
		ASSERT_FALSE (system.nodes[0]->store.account.get (transaction, vxldollar::dev::genesis_key.pub, info1));
		for (auto i (0); i < 50; ++i)
		{
...
TEST (wallet, find_none)
{
	...
	vxldollar::wallet_store wallet (init, kdf, transaction, vxldollar::dev::genesis->account (), 1, "0");
	ASSERT_FALSE (init);
	vxldollar::account account (1000);
...
TEST (account, encode_zero)
{
	 ...
	 */
	// NUMEROS CARACTERES DA HASH 60 + 5 (4 letras + underscore) vxld_
	ASSERT_EQ (65, str0.size ());
	ASSERT_EQ (65, str0.size ());
	vxldollar::account number1;
...
{
	vxldollar::raw_key value0 (0);
	vxldollar::fan fan (value0, 1024);
	for (auto & i : fan.values)
	{
		ASSERT_NE (value0, *i);
	}
	vxldollar::raw_key value1;
	fan.value (value1);
...
{
	auto error (false);
	vxldollar::mdb_env env (error, vxldollar::unique_path ());
	ASSERT_FALSE (error);
	auto transaction (env.tx_begin_write ());
	vxldollar::kdf kdf{ vxldollar::dev::network_params.kdf_work };
	vxldollar::wallet_store wallet (error, kdf, transaction, vxldollar::dev::genesis->account (), 1, "0");
	ASSERT_FALSE (error);
	ASSERT_FALSE (wallet.is_representative (transaction));
	ASSERT_EQ (vxldollar::dev::genesis->account (), wallet.representative (transaction));
	ASSERT_FALSE (wallet.is_representative (transaction));
	vxldollar::keypair key;
...
TEST (wallet, serialize_json_empty)
{
	...
	auto transaction (env.tx_begin_write ());
	vxldollar::kdf kdf{ vxldollar::dev::network_params.kdf_work };
	vxldollar::wallet_store wallet1 (error, kdf, transaction, vxldollar::dev::genesis->account (), 1, "0");
	ASSERT_FALSE (error);
	std::string serialized;
	wallet1.serialize_json (transaction, serialized);
	vxldollar::wallet_store wallet2 (error, kdf, transaction, vxldollar::dev::genesis->account (), 1, "1", serialized);
	ASSERT_FALSE (error);
	vxldollar::raw_key password1;
	vxldollar::raw_key password2;
...
TEST (wallet_store, move)
{
	...
	vxldollar::wallet_store wallet2 (error, kdf, transaction, vxldollar::dev::genesis->account (), 1, "1");
	ASSERT_FALSE (error);
	vxldollar::keypair key2;
	...
	std::vector<vxldollar::public_key> keys;
...
TEST (wallet, work)
{
	...
	while (!done)
	{
		...
		uint64_t work (0);
		if (!wallet->store.work_get (transaction, vxldollar::dev::genesis_key.pub, work))
		{
			done = vxldollar::dev::network_params.work.difficulty (vxldollar::dev::genesis->work_version (), vxldollar::dev::genesis->hash (), work) >= system.nodes[0]->default_difficulty (vxldollar::dev::genesis->work_version ());
		}
...
TEST (wallet, work_generate)
{
	...
	auto & node1 (*system.nodes[0]);
	auto wallet (system.wallet (0));
	vxldollar::uint128_t amount1 (node1.balance (vxldollar::dev::genesis_key.pub));
	uint64_t work1;
	wallet->insert_adhoc (vxldollar::dev::genesis_key.prv);
	...
	vxldollar::keypair key;
	auto block (wallet->send_action (vxldollar::dev::genesis_key.pub, key.pub, 100));
	auto transaction (node1.store.tx_begin_read ());
	ASSERT_TIMELY (10s, node1.ledger.account_balance (transaction, vxldollar::dev::genesis_key.pub) != amount1);
	...
	while (again)
	{
		...
		auto transaction (system.wallet (0)->wallets.tx_begin_read ());
		again = wallet->store.work_get (transaction, account1, work1) || vxldollar::dev::network_params.work.difficulty (block->work_version (), node1.ledger.latest_root (block_transaction, account1), work1) < node1.default_difficulty (block->work_version ());
	}
}
...
TEST (wallet, work_cache_delayed)
{
	...
	vxldollar::keypair key;
	auto block1 (wallet->send_action (vxldollar::dev::genesis_key.pub, key.pub, 100));
	ASSERT_EQ (block1->hash (), node1.latest (vxldollar::dev::genesis_key.pub));
	auto block2 (wallet->send_action (vxldollar::dev::genesis_key.pub, key.pub, 100));
	ASSERT_EQ (block2->hash (), node1.latest (vxldollar::dev::genesis_key.pub));
	ASSERT_EQ (block2->hash (), node1.wallets.delayed_work->operator[] (vxldollar::dev::genesis_key.pub));
	auto threshold (node1.default_difficulty (vxldollar::work_version::work_1));
	auto again (true);
	system.deadline_set (10s);
	while (again)
	{
		...
		{
			again = vxldollar::dev::network_params.work.difficulty (vxldollar::work_version::work_1, block2->hash (), work1) < threshold;
		}
	}
	ASSERT_GE (vxldollar::dev::network_params.work.difficulty (vxldollar::work_version::work_1, block2->hash (), work1), threshold);
}
...
TEST (wallet, insert_locked)
{
	...
	ASSERT_TRUE (wallet->insert_adhoc (vxldollar::keypair ().prv).is_zero ());
}
...
TEST (wallet, reseed)
{
	...
	vxldollar::kdf kdf{ vxldollar::dev::network_params.kdf_work };
	vxldollar::wallet_store wallet (init, kdf, transaction, vxldollar::dev::genesis->account (), 1, "0");
	vxldollar::raw_key seed1;
	seed1 = 1;
	vxldollar::raw_key seed2;
	seed2 = 2;
	wallet.seed_set (transaction, seed1);
	vxldollar::raw_key seed3;
...
TEST (wallet, no_work)
{
	...
	vxldollar::keypair key2;
	auto block (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, std::numeric_limits<vxldollar::uint128_t>::max (), false));
	ASSERT_NE (nullptr, block);
	ASSERT_NE (0, block->block_work ());
	ASSERT_GE (vxldollar::dev::network_params.work.difficulty (*block), vxldollar::dev::network_params.work.threshold (block->work_version (), block->sideband ().details));
	auto transaction (system.wallet (0)->wallets.tx_begin_read ());
	uint64_t cached_work (0);
	system.wallet (0)->store.work_get (transaction, vxldollar::dev::genesis_key.pub, cached_work);
...
TEST (wallet, send_race)
{
	...
	for (auto i (1); i < 60; ++i)
	{
		ASSERT_NE (nullptr, system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, vxldollar::Gxrb_ratio));
		ASSERT_EQ (vxldollar::dev::constants.genesis_amount - vxldollar::Gxrb_ratio * i, system.nodes[0]->balance (vxldollar::dev::genesis_key.pub));
	}
}
...
TEST (wallet, change_seed)
{
	...
	uint32_t index (4);
	auto prv = vxldollar::deterministic_key (seed1, index);
	pub = vxldollar::pub_key (prv);
	wallet->insert_adhoc (vxldollar::dev::genesis_key.prv, false);
	auto block (wallet->send_action (vxldollar::dev::genesis_key.pub, pub, 100));
	ASSERT_NE (nullptr, block);
...
TEST (wallet, epoch_2_validation)
{
	...
	{
		auto send = wallet.send_action (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.pub, amount, 1);
		ASSERT_NE (nullptr, send);
		ASSERT_EQ (vxldollar::epoch::epoch_2, send->sideband ().details.epoch);
		ASSERT_EQ (vxldollar::epoch::epoch_0, send->sideband ().source_epoch); // Not used for send state blocks

		auto receive = wallet.receive_action (send->hash (), vxldollar::dev::genesis_key.pub, amount, send->link ().as_account (), 1);
		ASSERT_NE (nullptr, receive);
		if (vxldollar::dev::network_params.work.difficulty (*receive) < node.network_params.work.base)
		{
			ASSERT_GE (vxldollar::dev::network_params.work.difficulty (*receive), node.network_params.work.epoch_2_receive);
			ASSERT_EQ (vxldollar::epoch::epoch_2, receive->sideband ().details.epoch);
			ASSERT_EQ (vxldollar::epoch::epoch_2, receive->sideband ().source_epoch);
			break;
		}
	}
	...
	// Test a change block
	ASSERT_NE (nullptr, wallet.change_action (vxldollar::dev::genesis_key.pub, vxldollar::keypair ().pub, 1));
}
...
TEST (wallet, epoch_2_receive_propagation)
{
	...
	while (++tries < max_tries)
	{
		...
		auto amount = node.config.receive_minimum.number ();
		auto send1 = wallet.send_action (vxldollar::dev::genesis_key.pub, key.pub, amount, 1);
		ASSERT_NE (nullptr, send1);
		ASSERT_NE (nullptr, wallet.receive_action (send1->hash (), vxldollar::dev::genesis_key.pub, amount, send1->link ().as_account (), 1));
		...
		// Send a block
		auto send2 = wallet.send_action (vxldollar::dev::genesis_key.pub, key.pub, amount, 1);
		ASSERT_NE (nullptr, send2);

		auto receive2 = wallet.receive_action (send2->hash (), key.pub, amount, send2->link ().as_account (), 1);
		ASSERT_NE (nullptr, receive2);
		if (vxldollar::dev::network_params.work.difficulty (*receive2) < node.network_params.work.base)
		{
			ASSERT_GE (vxldollar::dev::network_params.work.difficulty (*receive2), node.network_params.work.epoch_2_receive);
			ASSERT_EQ (vxldollar::epoch::epoch_2, node.store.block.version (node.store.tx_begin_read (), receive2->hash ()));
			ASSERT_EQ (vxldollar::epoch::epoch_2, receive2->sideband ().source_epoch);
			break;
		}
	}
	ASSERT_LT (tries, max_tries);
...
TEST (wallet, epoch_2_receive_unopened)
{
	...
	while (++tries < max_tries)
	{
		...
		auto epoch2_unopened = vxldollar::state_block (key.pub, 0, 0, 0, node.network_params.ledger.epochs.link (vxldollar::epoch::epoch_2), vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system.work.generate (key.pub, node.network_params.work.epoch_2));
		ASSERT_EQ (vxldollar::process_result::progress, node.process (epoch2_unopened).code);

		wallet.insert_adhoc (key.prv, false);

		auto receive1 = wallet.receive_action (send1->hash (), key.pub, amount, send1->link ().as_account (), 1);
		ASSERT_NE (nullptr, receive1);
		if (vxldollar::dev::network_params.work.difficulty (*receive1) < node.network_params.work.base)
		{
			ASSERT_GE (vxldollar::dev::network_params.work.difficulty (*receive1), node.network_params.work.epoch_2_receive);
			ASSERT_EQ (vxldollar::epoch::epoch_2, node.store.block.version (node.store.tx_begin_read (), receive1->hash ()));
			ASSERT_EQ (vxldollar::epoch::epoch_1, receive1->sideband ().source_epoch);
			break;
		}
	}
	ASSERT_LT (tries, max_tries);
...
TEST (wallet, foreach_representative_deadlock)
{
	...
	bool set = false;
	node.wallets.foreach_representative ([&node, &set, &system] (vxldollar::public_key const & pub, vxldollar::raw_key const & prv) {
		node.wallets.foreach_representative ([&node, &set, &system] (vxldollar::public_key const & pub, vxldollar::raw_key const & prv) {
...
TEST (wallet, receive_pruned)
{
	...
	// Send
	wallet1.insert_adhoc (vxldollar::dev::genesis_key.prv, false);
	auto amount = node2.config.receive_minimum.number ();
	auto send1 = wallet1.send_action (vxldollar::dev::genesis_key.pub, key.pub, amount, 1);
	auto send2 = wallet1.send_action (vxldollar::dev::genesis_key.pub, key.pub, 1, 1);

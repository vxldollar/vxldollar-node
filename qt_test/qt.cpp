...
#include <algorithm>
#include <vxldollar/qt_test/QTest>
#include <thread>
...
TEST (wallet, construction)
{
	...
	auto wallet_l (system.nodes[0]->wallets.create (vxldollar::random_wallet_id ()));
	auto key (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key));
	wallet->start ();
...
// https://github.com/vxldollarcurrency/vxldollar-node/issues/3235
TEST (wallet, DISABLED_status)
{
	...
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub));
	wallet->start ();
	auto wallet_has = [wallet] (vxldollar_qt::status_types status_ty) {
	...
	system.nodes[0]->network.udp_channels.insert (vxldollar::endpoint (boost::asio::ip::address_v6::loopback (), 10000), 0);
	...
	ASSERT_FALSE (wallet_has (vxldollar_qt::status_types::synchronizing));
	system.deadline_set (25s);
	while (!wallet_has (vxldollar_qt::status_types::synchronizing))
	{
	...
	ASSERT_TRUE (wallet_has (vxldollar_qt::status_types::disconnected));
}
...
TEST (wallet, status_with_peer)
{
	...
	ASSERT_TRUE (wallet_has (vxldollar_qt::status_types::nominal));
}
...
TEST (wallet, startup_balance)
{
	...
	ASSERT_EQ ("Balance: 0 vxldollar", wallet->self.balance_label->text ().toStdString ());
}
...
TEST (wallet, select_account)
{
	...
	vxldollar::system system (1);
	auto wallet_l (system.nodes[0]->wallets.create (vxldollar::random_wallet_id ()));
	vxldollar::public_key key1 (wallet_l->deterministic_insert ());
	vxldollar::public_key key2 (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key1));
	wallet->start ();
...
TEST (wallet, password_change)
{
	...
	system.wallet (0)->insert_adhoc (vxldollar::keypair ().prv);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		account = system.account (transaction, 0);
	}
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->settings_button, Qt::LeftButton);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
...
TEST (wallet, send)
{
	...
	vxldollar::public_key key1 (system.wallet (1)->insert_adhoc (vxldollar::keypair ().prv));
	auto account (vxldollar::dev::genesis_key.pub);
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	ASSERT_NE (wallet->rendering_ratio, vxldollar::raw_ratio);
	QTest::mouseClick (wallet->send_blocks, Qt::LeftButton);
	...
	vxldollar::uint128_t amount (wallet->node.balance (key1));
...
// Issue for investigating it: https://github.com/vxldollarcurrency/vxldollar-node/issues/3642
TEST (wallet, DISABLED_process_block)
{
	vxldollar_qt::eventloop_processor processor;
	vxldollar::system system (1);
	vxldollar::account account;
	vxldollar::block_hash latest (system.nodes[0]->latest (vxldollar::dev::genesis->account ()));
	system.wallet (0)->insert_adhoc (vxldollar::keypair ().prv);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		account = system.account (transaction, 0);
	...
	vxldollar::send_block send (latest, key1.pub, 0, vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system.work.generate (latest));
	std::string previous;
...
TEST (wallet, create_send)
{
	...
	QTest::keyClicks (wallet->block_creation.account, vxldollar::dev::genesis_key.pub.to_account ().c_str ());
	...
	bool error (false);
	vxldollar::state_block send (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (vxldollar::process_result::progress, system.nodes[0]->process (send).code);
	ASSERT_EQ (vxldollar::process_result::old, system.nodes[0]->process (send).code);
}
...
TEST (wallet, create_open_receive)
{
	...
	vxldollar::system system (1);
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key.pub, 100);
	vxldollar::block_hash latest1 (system.nodes[0]->latest (vxldollar::dev::genesis_key.pub));
	system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key.pub, 100);
	vxldollar::block_hash latest2 (system.nodes[0]->latest (vxldollar::dev::genesis_key.pub));
	...
	QTest::keyClicks (wallet->block_creation.representative, vxldollar::dev::genesis_key.pub.to_account ().c_str ());
	QTest::mouseClick (wallet->block_creation.create, Qt::LeftButton);
	...
	bool error (false);
	vxldollar::state_block open (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (vxldollar::process_result::progress, system.nodes[0]->process (open).code);
	ASSERT_EQ (vxldollar::process_result::old, system.nodes[0]->process (open).code);
	...
	bool error2 (false);
	vxldollar::state_block receive (error2, tree2);
	ASSERT_FALSE (error2);
	ASSERT_EQ (vxldollar::process_result::progress, system.nodes[0]->process (receive).code);
	ASSERT_EQ (vxldollar::process_result::old, system.nodes[0]->process (receive).code);
}
...
TEST (wallet, create_change)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	vxldollar::state_block change (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (vxldollar::process_result::progress, system.nodes[0]->process (change).code);
	ASSERT_EQ (vxldollar::process_result::old, system.nodes[0]->process (change).code);
}
...
TEST (history, short_text)
{
	...
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	auto store = vxldollar::make_store (system.nodes[0]->logger, vxldollar::unique_path (), vxldollar::dev::constants);
	ASSERT_TRUE (!store->init_error ());
	vxldollar::ledger ledger (*store, system.nodes[0]->stats, vxldollar::dev::constants);
	{
		auto transaction (store->tx_begin_write ());
		store->initialize (transaction, ledger.cache);
		vxldollar::keypair key;
		auto latest (ledger.latest (transaction, vxldollar::dev::genesis_key.pub));
		vxldollar::send_block send (latest, vxldollar::dev::genesis_key.pub, 0, vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (vxldollar::process_result::progress, ledger.process (transaction, send).code);
		vxldollar::receive_block receive (send.hash (), send.hash (), vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system.work.generate (send.hash ()));
		ASSERT_EQ (vxldollar::process_result::progress, ledger.process (transaction, receive).code);
		vxldollar::change_block change (receive.hash (), key.pub, vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system.work.generate (receive.hash ()));
		ASSERT_EQ (vxldollar::process_result::progress, ledger.process (transaction, change).code);
	}
	vxldollar_qt::history history (ledger, vxldollar::dev::genesis_key.pub, *wallet);
	history.refresh ();
...
TEST (history, pruned_source)
{
	...
	vxldollar::ledger ledger (*store, system.nodes[0]->stats, vxldollar::dev::constants);
	ledger.pruning = true;
	vxldollar::block_hash next_pruning;
	// Basic pruning for legacy blocks. Previous block is pruned, source is pruned
	{
		auto transaction (store->tx_begin_write ());
		store->initialize (transaction, ledger.cache);
		auto latest (ledger.latest (transaction, vxldollar::dev::genesis_key.pub));
		vxldollar::send_block send1 (latest, vxldollar::dev::genesis_key.pub, vxldollar::dev::constants.genesis_amount - 100, vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (vxldollar::process_result::progress, ledger.process (transaction, send1).code);
		vxldollar::send_block send2 (send1.hash (), key.pub, vxldollar::dev::constants.genesis_amount - 200, vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system.work.generate (send1.hash ()));
		ASSERT_EQ (vxldollar::process_result::progress, ledger.process (transaction, send2).code);
		vxldollar::receive_block receive (send2.hash (), send1.hash (), vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system.work.generate (send2.hash ()));
		ASSERT_EQ (vxldollar::process_result::progress, ledger.process (transaction, receive).code);
		vxldollar::open_block open (send2.hash (), key.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
	...
	vxldollar_qt::history history1 (ledger, vxldollar::dev::genesis_key.pub, *wallet);
	history1.refresh ();
	ASSERT_EQ (2, history1.model->rowCount ());
	vxldollar_qt::history history2 (ledger, key.pub, *wallet);
	history2.refresh ();
	...
	// Pruning for state blocks. Previous block is pruned, source is pruned
	{
		auto transaction (store->tx_begin_write ());
		auto latest (ledger.latest (transaction, vxldollar::dev::genesis_key.pub));
		vxldollar::state_block send (vxldollar::dev::genesis_key.pub, latest, vxldollar::dev::genesis_key.pub, vxldollar::dev::constants.genesis_amount - 200, key.pub, vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (vxldollar::process_result::progress, ledger.process (transaction, send).code);
		auto latest_key (ledger.latest (transaction, key.pub));
		vxldollar::state_block receive (key.pub, latest_key, key.pub, 200, send.hash (), key.prv, key.pub, *system.work.generate (latest_key));
		ASSERT_EQ (vxldollar::process_result::progress, ledger.process (transaction, receive).code);
...
TEST (wallet, startup_work)
{
	...
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		ASSERT_TRUE (wallet->wallet_m->store.work_get (transaction, vxldollar::dev::genesis_key.pub, work1));
	}
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	while (again)
	{
		ASSERT_NO_ERROR (system.poll ());
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		again = wallet->wallet_m->store.work_get (transaction, vxldollar::dev::genesis_key.pub, work1);
	}
}
...
TEST (wallet, import)
{
	...
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system.nodes[1], system.wallet (1), key2.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, republish)
{
	...
	vxldollar::block_hash hash;
	{
		...
		auto latest (system.nodes[0]->ledger.latest (transaction, vxldollar::dev::genesis_key.pub));
		vxldollar::send_block block (latest, key.pub, 0, vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system.work.generate (latest));
		hash = block.hash ();
		ASSERT_EQ (vxldollar::process_result::progress, system.nodes[0]->ledger.process (transaction, block).code);
	}
	auto account (vxldollar::dev::genesis_key.pub);
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	...
	ASSERT_FALSE (system.nodes[1]->balance (vxldollar::dev::genesis_key.pub).is_zero ());
	system.deadline_set (10s);
	while (system.nodes[1]->balance (vxldollar::dev::genesis_key.pub).is_zero ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (wallet, ignore_empty_adhoc)
{
	...
	vxldollar::keypair key1;
	system.wallet (0)->insert_adhoc (key1.prv);
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	ASSERT_EQ (wallet->accounts.window, wallet->main_stack->currentWidget ());
	QTest::keyClicks (wallet->accounts.account_key_line, vxldollar::dev::genesis_key.prv.to_string ().c_str ());
	QTest::mouseClick (wallet->accounts.account_key_button, Qt::LeftButton);
	ASSERT_EQ (1, wallet->accounts.model->rowCount ());
...
TEST (wallet, change_seed)
{
	...
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), wallet_key));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, seed_work_generation)
{
	...
	vxldollar::system system (1);
	auto key1 (system.wallet (0)->deterministic_insert ());
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	auto transaction (system.nodes[0]->store.tx_begin_read ());
	ASSERT_GE (vxldollar::dev::network_params.work.difficulty (vxldollar::work_version::work_1, system.nodes[0]->ledger.latest_root (transaction, pub), work), system.nodes[0]->default_difficulty (vxldollar::work_version::work_1));
}
...
TEST (wallet, DISABLED_synchronizing)
{
	...
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *system0.nodes[0], system0.wallet (0), key1));
	wallet->start ();
	{
		auto transaction (system1.nodes[0]->store.tx_begin_write ());
		auto latest (system1.nodes[0]->ledger.latest (transaction, vxldollar::dev::genesis->account ()));
		vxldollar::send_block send (latest, key1, 0, vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system1.work.generate (latest));
		system1.nodes[0]->ledger.process (transaction, send);
	}
	ASSERT_EQ (0, wallet->active_status.active.count (vxldollar_qt::status_types::synchronizing));
	system0.nodes[0]->bootstrap_initiator.bootstrap (system1.nodes[0]->network.endpoint ());
	system1.deadline_set (10s);
	while (wallet->active_status.active.count (vxldollar_qt::status_types::synchronizing) == 0)
	{
	...
	while (wallet->active_status.active.count (vxldollar_qt::status_types::synchronizing) == 1)
	{
		ASSERT_NO_ERROR (system0.poll ());
		ASSERT_NO_ERROR (system1.poll ());
		test_application->processEvents ();
...
TEST (wallet, epoch_2_validation)
{
	...
	auto account (vxldollar::dev::genesis_key.pub);
	auto wallet (std::make_shared<vxldollar_qt::wallet> (*test_application, processor, *node, system.wallet (0), account));
	wallet->start ();
	wallet->client_window->show ();
	...
	auto create_and_process = [&] () -> vxldollar::block_hash {
		wallet->block_creation.create->click ();
		std::string json (wallet->block_creation.block->toPlainText ().toStdString ());
		...
		boost::property_tree::read_json (istream, tree1);
		bool error (false);
		vxldollar::state_block block (error, tree1);
		EXPECT_FALSE (error);
		EXPECT_EQ (vxldollar::process_result::progress, node->process (block).code);
		return block.hash ();
	};

	auto do_send = [&] (vxldollar::public_key const & destination) -> vxldollar::block_hash {
		wallet->block_creation.send->click ();
		wallet->block_creation.account->setText (vxldollar::dev::genesis_key.pub.to_account ().c_str ());
	...
	auto do_open = [&] (vxldollar::block_hash const & source, vxldollar::public_key const & account) -> vxldollar::block_hash {
		wallet->block_creation.open->click ();
	...
	auto do_receive = [&] (vxldollar::block_hash const & source) -> vxldollar::block_hash {
		wallet->block_creation.receive->click ();
		wallet->block_creation.source->setText (source.to_string ().c_str ());
		return create_and_process ();
	};

	auto do_change = [&] (vxldollar::public_key const & account, vxldollar::public_key const & representative) -> vxldollar::block_hash {
		wallet->block_creation.change->click ();
	...
	while (++tries < max_tries)
	{
		...
		do_receive (send2);
		do_change (key.pub, vxldollar::dev::genesis_key.pub);
	}
}

...
bool vxldollar::inactive_cache_status::operator!= (inactive_cache_status const other) const
{
...
std::string vxldollar::inactive_cache_status::to_string () const
{
	std::stringstream ss;
	ss << "bootstrap_started=" << bootstrap_started;
	ss << ", election_started=" << election_started;
	ss << ", confirmed=" << confirmed;
	ss << ", tally=" << vxldollar::uint128_union (tally).to_string ();
	return ss.str ();
}

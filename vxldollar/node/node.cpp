...
double constexpr vxldollar::node::price_max;
double constexpr vxldollar::node::free_cutoff;
std::size_t constexpr vxldollar::block_arrival::arrival_size_min;
std::chrono::seconds constexpr vxldollar::block_arrival::arrival_time_min;

namespace vxldollar
{
extern unsigned char vxldollar_bootstrap_weights_live[];
extern std::size_t vxldollar_bootstrap_weights_live_size;
extern unsigned char vxldollar_bootstrap_weights_beta[];
extern std::size_t vxldollar_bootstrap_weights_beta_size;
}

void vxldollar::node::keepalive (std::string const & address_a, uint16_t port_a)
{
	...
	network.resolver.async_resolve (boost::asio::ip::udp::resolver::query (address_a, std::to_string (port_a)), [node_l, address_a, port_a] (boost::system::error_code const & ec, boost::asio::ip::udp::resolver::iterator i_a) {
		if (!ec)
		{
			...
			{
				auto endpoint (vxldollar::transport::map_endpoint_to_v6 (i->endpoint ()));
				std::weak_ptr<vxldollar::node> node_w (node_l);
...
std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (rep_crawler & rep_crawler, std::string const & name)
{
	std::size_t count;
	{
		vxldollar::lock_guard<vxldollar::mutex> guard (rep_crawler.active_mutex);
...
vxldollar::node::node (boost::asio::io_context & io_ctx_a, uint16_t peering_port_a, boost::filesystem::path const & application_path_a, vxldollar::logging const & logging_a, vxldollar::work_pool & work_a, vxldollar::node_flags flags_a, unsigned seq) :
	node (io_ctx_a, application_path_a, vxldollar::node_config (peering_port_a, logging_a), work_a, flags_a, seq)
{
}

vxldollar::node::node (boost::asio::io_context & io_ctx_a, boost::filesystem::path const & application_path_a, vxldollar::node_config const & config_a, vxldollar::work_pool & work_a, vxldollar::node_flags flags_a, unsigned seq) :
	...
	workers (std::max (3u, config.io_threads / 4), vxldollar::thread_role::name::worker),
	flags (flags_a),
	work (work_a),
	distributed_work (*this),
	logger (config_a.logging.min_time_between_log_output),
	store_impl (vxldollar::make_store (logger, application_path_a, network_params.ledger, flags.read_only, true, config_a.rocksdb_config, config_a.diagnostics_config.txn_tracking, config_a.block_processor_batch_max_time, config_a.lmdb_config, config_a.backup_before_upgrade)),
	store (*store_impl),
	unchecked{ store, flags.disable_block_processor_unchecked_deletion },
	wallets_store_impl (std::make_unique<vxldollar::mdb_wallets_store> (application_path_a / "wallets.ldb", config_a.lmdb_config)),
	...
	telemetry (std::make_shared<vxldollar::telemetry> (network, workers, observers.telemetry, stats, network_params, flags.disable_ongoing_telemetry_requests)),
	...
	node_seq (seq)
{
	unchecked.satisfied = [this] (vxldollar::unchecked_info const & info) {
	...
	if (!init_error ())
	{
		...
		{
			auto endpoint_l (vxldollar::tcp_endpoint (boost::asio::ip::make_address_v6 (config.websocket_config.address), config.websocket_config.port));
			websocket_server = std::make_shared<vxldollar::websocket::listener> (config.websocket_config.tls_config, logger, wallets, io_ctx, endpoint_l);
		...
		network.channel_observer = [this] (std::shared_ptr<vxldollar::transport::channel> const & channel_a) {
		...
		{
			observers.blocks.add ([this] (vxldollar::election_status const & status_a, std::vector<vxldollar::vote_with_weight_info> const & votes_a, vxldollar::account const & account_a, vxldollar::amount const & amount_a, bool is_state_send_a, bool is_state_epoch_a) {
				auto block_a (status_a.winner);
				if ((status_a.type == vxldollar::election_status_type::active_confirmed_quorum || status_a.type == vxldollar::election_status_type::active_confirmation_height) && this->block_arrival.recent (block_a->hash ()))
				{
					auto node_l (shared_from_this ());
					background ([node_l, block_a, account_a, amount_a, is_state_send_a, is_state_epoch_a] () {
						...
						// Subtype field
						else if (block_a->type () == vxldollar::block_type::state)
						{
						...
						resolver->async_resolve (boost::asio::ip::tcp::resolver::query (address, std::to_string (port)), [node_l, address, port, target, body, resolver] (boost::system::error_code const & ec, boost::asio::ip::tcp::resolver::iterator i_a) {
							...
							else
							{
								...
								node_l->stats.inc (vxldollar::stat::type::error, vxldollar::stat::detail::http_callback, vxldollar::stat::dir::out);
							}
						});
					});
				}
		...
		{
			observers.blocks.add ([this] (vxldollar::election_status const & status_a, std::vector<vxldollar::vote_with_weight_info> const & votes_a, vxldollar::account const & account_a, vxldollar::amount const & amount_a, bool is_state_send_a, bool is_state_epoch_a) {
				debug_assert (status_a.type != vxldollar::election_status_type::ongoing);

				if (this->websocket_server->any_subscriber (vxldollar::websocket::topic::confirmation))
				{
					auto block_a (status_a.winner);
					std::string subtype;
					if (is_state_send_a)
			...
			observers.active_stopped.add ([this] (vxldollar::block_hash const & hash_a) {
				if (this->websocket_server->any_subscriber (vxldollar::websocket::topic::stopped_election))
				{
			...
			observers.telemetry.add ([this] (vxldollar::telemetry_data const & telemetry_data, vxldollar::endpoint const & endpoint) {
				if (this->websocket_server->any_subscriber (vxldollar::websocket::topic::telemetry))
				{
					vxldollar::websocket::message_builder builder;
		...
		observers.blocks.add ([this] (vxldollar::election_status const & status_a, std::vector<vxldollar::vote_with_weight_info> const & votes_a, vxldollar::account const & account_a, vxldollar::amount const & amount_a, bool is_state_send_a, bool is_state_epoch_a) {
			...
			{
				case vxldollar::election_status_type::active_confirmed_quorum:
					this->stats.inc (vxldollar::stat::type::confirmation_observer, vxldollar::stat::detail::active_quorum, vxldollar::stat::dir::out);
					break;
				case vxldollar::election_status_type::active_confirmation_height:
					this->stats.inc (vxldollar::stat::type::confirmation_observer, vxldollar::stat::detail::active_conf_height, vxldollar::stat::dir::out);
					break;
				case vxldollar::election_status_type::inactive_confirmation_height:
					this->stats.inc (vxldollar::stat::type::confirmation_observer, vxldollar::stat::detail::inactive_conf_height, vxldollar::stat::dir::out);
					break;
				default:
					break;
			}
		});
		observers.endpoint.add ([this] (std::shared_ptr<vxldollar::transport::channel> const & channel_a) {
			if (channel_a->get_type () == vxldollar::transport::transport_type::udp)
			{
		...
		observers.vote.add ([this] (std::shared_ptr<vxldollar::vote> vote_a, std::shared_ptr<vxldollar::transport::channel> const & channel_a, vxldollar::vote_code code_a) {
			debug_assert (code_a != vxldollar::vote_code::invalid);
			// The vote_code::vote is handled inside the election
			if (code_a == vxldollar::vote_code::indeterminate)
			{
		...
		{
			observers.vote.add ([this] (std::shared_ptr<vxldollar::vote> vote_a, std::shared_ptr<vxldollar::transport::channel> const & channel_a, vxldollar::vote_code code_a) {
				if (this->websocket_server->any_subscriber (vxldollar::websocket::topic::vote))
				{
					vxldollar::websocket::message_builder builder;
		...
		observers.work_cancel.add ([this] (vxldollar::root const & root_a) {
			this->work.cancel (root_a);
			this->distributed_work.cancel (root_a);
		});

		logger.always_log ("Node starting, version: ", VXLDOLLAR_VERSION_STRING);
		...
		if ((network_params.network.is_live_network () || network_params.network.is_beta_network ()) && !flags.inactive_node)
		{
			...
			if (use_bootstrap_weight)
			{
				...
				{
					logger.always_log ("Using bootstrap rep weight: ", rep.first.to_account (), " -> ", vxldollar::uint128_union (rep.second).format_balance (Mxrb_ratio, 0, true), " XRB");
				}
			}
...
void vxldollar::node::do_rpc_callback (boost::asio::ip::tcp::resolver::iterator i_a, std::string const & address, uint16_t port, std::shared_ptr<std::string> const & target, std::shared_ptr<std::string> const & body, std::shared_ptr<boost::asio::ip::tcp::resolver> const & resolver)
{
					...
					if (!ec)
					{
						...
						boost::beast::http::async_read (*sock, *sb, *resp, [node_l, sb, resp, sock, address, port, i_a, target, body, resolver] (boost::system::error_code const & ec, std::size_t bytes_transferred) mutable {
							if (!ec)
							{
								...
								{
									node_l->stats.inc (vxldollar::stat::type::http_callback, vxldollar::stat::detail::initiate, vxldollar::stat::dir::out);
								}
								else
								{
									if (node_l->config.logging.callback_logging ())
...
bool vxldollar::node::copy_with_compaction (boost::filesystem::path const & destination)
{
	return store.copy_db (destination);
}

std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (node & node, std::string const & name)
{
	auto composite = std::make_unique<container_info_composite> (name);
...
void vxldollar::node::process_active (std::shared_ptr<vxldollar::block> const & incoming)
{
	block_arrival.add (incoming->hash ());
	block_processor.add (incoming);
}

vxldollar::process_return vxldollar::node::process (vxldollar::block & block_a)
{
...
vxldollar::process_return vxldollar::node::process_local (std::shared_ptr<vxldollar::block> const & block_a)
{
	...
	vxldollar::unchecked_info info (block_a, block_a->account (), vxldollar::signature_verification::unknown);
	...
	auto const transaction (store.tx_begin_write ({ tables::accounts, tables::blocks, tables::frontiers, tables::pending }));
	return block_processor.process_one (transaction, post_events, info, false, vxldollar::block_origin::local);
}

void vxldollar::node::process_local_async (std::shared_ptr<vxldollar::block> const & block_a)
{
	// Add block hash as recently arrived to trigger automatic rebroadcast and election
	block_arrival.add (block_a->hash ());
	// Set current time to trigger automatic rebroadcast and election
...
void vxldollar::node::start ()
{
	...
	wallets.start ();
	if (config.frontiers_confirmation != vxldollar::frontiers_confirmation_mode::disabled)
	{
...
void vxldollar::node::keepalive_preconfigured (std::vector<std::string> const & peers_a)
{
...
vxldollar::block_hash vxldollar::node::latest (vxldollar::account const & account_a)
{
	auto const transaction (store.tx_begin_read ());
	return ledger.latest (transaction, account_a);
}

vxldollar::uint128_t vxldollar::node::balance (vxldollar::account const & account_a)
{
	auto const transaction (store.tx_begin_read ());
	return ledger.account_balance (transaction, account_a);
}

std::shared_ptr<vxldollar::block> vxldollar::node::block (vxldollar::block_hash const & hash_a)
{
	auto const transaction (store.tx_begin_read ());
	return store.block.get (transaction, hash_a);
}

std::pair<vxldollar::uint128_t, vxldollar::uint128_t> vxldollar::node::balance_pending (vxldollar::account const & account_a, bool only_confirmed_a)
{
	std::pair<vxldollar::uint128_t, vxldollar::uint128_t> result;
	auto const transaction (store.tx_begin_read ());
...
vxldollar::uint128_t vxldollar::node::weight (vxldollar::account const & account_a)
{
	return ledger.weight (account_a);
}

vxldollar::block_hash vxldollar::node::rep_block (vxldollar::account const & account_a)
{
	auto const transaction (store.tx_begin_read ());
...
vxldollar::uint128_t vxldollar::node::minimum_principal_weight ()
{
	return minimum_principal_weight (online_reps.trended ());
}

vxldollar::uint128_t vxldollar::node::minimum_principal_weight (vxldollar::uint128_t const & online_stake)
{
	return online_stake / network_params.network.principal_weight_factor;
}

void vxldollar::node::long_inactivity_cleanup ()
{
...
void vxldollar::node::ongoing_rep_calculation ()
{
	auto now (std::chrono::steady_clock::now ());
	vote_processor.calculate_weights ();
	std::weak_ptr<vxldollar::node> node_w (shared_from_this ());
...
void vxldollar::node::ongoing_bootstrap ()
{
	...
	auto previous_bootstrap_count (stats.count (vxldollar::stat::type::bootstrap, vxldollar::stat::detail::initiate, vxldollar::stat::dir::out) + stats.count (vxldollar::stat::type::bootstrap, vxldollar::stat::detail::initiate_legacy_age, vxldollar::stat::dir::out));
	/* 
...
void vxldollar::node::ongoing_peer_store ()
{
...
void vxldollar::node::backup_wallet ()
{
	...
	for (auto i (wallets.items.begin ()), n (wallets.items.end ()); i != n; ++i)
	{
		...
		vxldollar::set_secure_perm_directory (backup_path, error_chmod);
...
void vxldollar::node::search_receivable_all ()
{
...
void vxldollar::node::bootstrap_wallet ()
{
	std::deque<vxldollar::account> accounts;
	{
		vxldollar::lock_guard<vxldollar::mutex> lock (wallets.mutex);
		...
		for (auto i (wallets.items.begin ()), n (wallets.items.end ()); i != n && accounts.size () < 128; ++i)
		{
			...
			vxldollar::lock_guard<std::recursive_mutex> wallet_lock (wallet.store.mutex);
			for (auto j (wallet.store.begin (transaction)), m (wallet.store.end ()); j != m && accounts.size () < 128; ++j)
			{
				vxldollar::account account (j->first);
...
void vxldollar::node::unchecked_cleanup ()
{
	std::vector<vxldollar::uint128_t> digests;
	std::deque<vxldollar::unchecked_key> cleaning_list;
	...
	{
		auto const now (vxldollar::seconds_since_epoch ());
		auto const transaction (store.tx_begin_read ());
		// Max 1M records to clean, max 2 minutes reading to prevent slow i/o systems issues
		for (auto [i, n] = unchecked.full_range (transaction); i != n && cleaning_list.size () < 1024 * 1024 && vxldollar::seconds_since_epoch () - now < 120; ++i)
		{
...
void vxldollar::node::ongoing_unchecked_cleanup ()
{
	unchecked_cleanup ();
...
void vxldollar::node::ongoing_backlog_population ()
{
	populate_backlog ();
...
bool vxldollar::node::collect_ledger_pruning_targets (std::deque<vxldollar::block_hash> & pruning_targets_a, vxldollar::account & last_account_a, uint64_t const batch_read_size_a, uint64_t const max_depth_a, uint64_t const cutoff_time_a)
{
	...
	for (auto i (store.confirmation_height.begin (transaction, last_account_a)), n (store.confirmation_height.end ()); i != n && !finish_transaction;)
	{
		...
		vxldollar::block_hash hash (i->second.frontier);
		uint64_t depth (0);
...
void vxldollar::node::ledger_pruning (uint64_t const batch_size_a, bool bootstrap_weight_reached_a, bool log_to_cout_a)
{
	uint64_t const max_depth (config.max_pruning_depth != 0 ? config.max_pruning_depth : std::numeric_limits<uint64_t>::max ());
	uint64_t const cutoff_time (bootstrap_weight_reached_a ? vxldollar::seconds_since_epoch () - config.max_pruning_age.count () : std::numeric_limits<uint64_t>::max ());
	uint64_t pruned_count (0);
	uint64_t transaction_write_count (0);
	vxldollar::account last_account (1); // 0 Burn account is never opened. So it can be used to break loop
	std::deque<vxldollar::block_hash> pruning_targets;
	...
	while ((transaction_write_count != 0 || !target_finished) && !stopped)
	{
		...
		{
			auto scoped_write_guard = write_database_queue.wait (vxldollar::writer::pruning);
...
void vxldollar::node::ongoing_ledger_pruning ()
{
	auto bootstrap_weight_reached (ledger.cache.block_count >= ledger.bootstrap_weight_max_blocks);
...
int vxldollar::node::price (vxldollar::uint128_t const & balance_a, int amount_a)
{
	debug_assert (balance_a >= amount_a * vxldollar::Gxrb_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= vxldollar::Gxrb_ratio;
		auto balance_scaled ((balance_l / vxldollar::Mxrb_ratio).convert_to<double> ());
...
uint64_t vxldollar::node::default_difficulty (vxldollar::work_version const version_a) const
{
...
uint64_t vxldollar::node::default_receive_difficulty (vxldollar::work_version const version_a) const
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
...
uint64_t vxldollar::node::max_work_generate_difficulty (vxldollar::work_version const version_a) const
{
	return vxldollar::difficulty::from_multiplier (config.max_work_generate_multiplier, default_difficulty (version_a));
}

bool vxldollar::node::local_work_generation_enabled () const
{
	return config.work_threads > 0 || work.opencl;
}

bool vxldollar::node::work_generation_enabled () const
{
	return work_generation_enabled (config.work_peers);
}

bool vxldollar::node::work_generation_enabled (std::vector<std::pair<std::string, uint16_t>> const & peers_a) const
{
	return !peers_a.empty () || local_work_generation_enabled ();
}

boost::optional<uint64_t> vxldollar::node::work_generate_blocking (vxldollar::block & block_a, uint64_t difficulty_a)
{
...
void vxldollar::node::work_generate (vxldollar::work_version const version_a, vxldollar::root const & root_a, uint64_t difficulty_a, std::function<void (boost::optional<uint64_t>)> callback_a, boost::optional<vxldollar::account> const & account_a, bool secondary_work_peers_a)
{
...
boost::optional<uint64_t> vxldollar::node::work_generate_blocking (vxldollar::work_version const version_a, vxldollar::root const & root_a, uint64_t difficulty_a, boost::optional<vxldollar::account> const & account_a)
{
...
boost::optional<uint64_t> vxldollar::node::work_generate_blocking (vxldollar::block & block_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (block_a, default_difficulty (vxldollar::work_version::work_1));
}

boost::optional<uint64_t> vxldollar::node::work_generate_blocking (vxldollar::root const & root_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (root_a, default_difficulty (vxldollar::work_version::work_1));
}

boost::optional<uint64_t> vxldollar::node::work_generate_blocking (vxldollar::root const & root_a, uint64_t difficulty_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (vxldollar::work_version::work_1, root_a, difficulty_a);
}

void vxldollar::node::add_initial_peers ()
{
	...
	{
		vxldollar::endpoint endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ());
...
void vxldollar::node::block_confirm (std::shared_ptr<vxldollar::block> const & block_a)
{
...
bool vxldollar::node::block_confirmed (vxldollar::block_hash const & hash_a)
{
	auto transaction (store.tx_begin_read ());
	return ledger.block_confirmed (transaction, hash_a);
}

bool vxldollar::node::block_confirmed_or_being_confirmed (vxldollar::transaction const & transaction_a, vxldollar::block_hash const & hash_a)
{
	return confirmation_height_processor.is_processing_block (hash_a) || ledger.block_confirmed (transaction_a, hash_a);
}

void vxldollar::node::ongoing_online_weight_calculation_queue ()
{
	std::weak_ptr<vxldollar::node> node_w (shared_from_this ());
...
bool vxldollar::node::online () const
{
	return rep_crawler.total_weight () > online_reps.delta ();
}

void vxldollar::node::ongoing_online_weight_calculation ()
{
	online_reps.sample ();
	ongoing_online_weight_calculation_queue ();
}

void vxldollar::node::receive_confirmed (vxldollar::transaction const & block_transaction_a, vxldollar::block_hash const & hash_a, vxldollar::account const & destination_a)
{
	vxldollar::unique_lock<vxldollar::mutex> lk (wallets.mutex);
	...
	for ([[maybe_unused]] auto const & [id, wallet] : wallets_l)
	{
		if (wallet->store.exists (wallet_transaction, destination_a))
		{
			...
			auto error (store.pending.get (block_transaction_a, vxldollar::pending_key (destination_a, hash_a), pending));
			if (!error)
			{
				auto amount (pending.amount.number ());
				wallet->receive_async (hash_a, representative, amount, destination_a, [] (std::shared_ptr<vxldollar::block> const &) {});
			}
			else
			{
...
void vxldollar::node::process_confirmed_data (vxldollar::transaction const & transaction_a, std::shared_ptr<vxldollar::block> const & block_a, vxldollar::block_hash const & hash_a, vxldollar::account & account_a, vxldollar::uint128_t & amount_a, bool & is_state_send_a, bool & is_state_epoch_a, vxldollar::account & pending_account_a)
{
	...
	{
		amount_a = vxldollar::dev::constants.genesis_amount;
	}
	if (auto state = dynamic_cast<vxldollar::state_block *> (block_a.get ()))
	{
	...
	if (auto send = dynamic_cast<vxldollar::send_block *> (block_a.get ()))
	{
		pending_account_a = send->hashables.destination;
	}
}

void vxldollar::node::process_confirmed (vxldollar::election_status const & status_a, uint64_t iteration_a)
{
	...
	{
		iteration_a++;
		std::weak_ptr<vxldollar::node> node_w (shared ());
...
bool vxldollar::block_arrival::add (vxldollar::block_hash const & hash_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
	auto inserted (arrival.get<tag_sequence> ().emplace_back (vxldollar::block_arrival_info{ now, hash_a }));
	auto result (!inserted.second);
	return result;
}

bool vxldollar::block_arrival::recent (vxldollar::block_hash const & hash_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
...
std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (block_arrival & block_arrival, std::string const & name)
{
	std::size_t count = 0;
	{
		vxldollar::lock_guard<vxldollar::mutex> guard (block_arrival.mutex);
...
std::shared_ptr<vxldollar::node> vxldollar::node::shared ()
{
	return shared_from_this ();
}

int vxldollar::node::store_version ()
{
	auto transaction (store.tx_begin_read ());
	return store.version.get (transaction);
}

bool vxldollar::node::init_error () const
{
	return store.init_error () || wallets_store.init_error ();
}

bool vxldollar::node::epoch_upgrader (vxldollar::raw_key const & prv_a, vxldollar::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	...
	if (!error)
	{
		...
		if (!error)
		{
			*epoch_upgrade = std::async (std::launch::async, &vxldollar::node::epoch_upgrader_impl, this, prv_a, epoch_a, count_limit, threads);
		}
	}
	return error;
}

void vxldollar::node::set_bandwidth_params (std::size_t limit, double ratio)
{
...
void vxldollar::node::epoch_upgrader_impl (vxldollar::raw_key const & prv_a, vxldollar::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	vxldollar::thread_role::set (vxldollar::thread_role::name::epoch_upgrader);
	auto upgrader_process = [] (vxldollar::node & node_a, std::atomic<uint64_t> & counter, std::shared_ptr<vxldollar::block> const & epoch, uint64_t difficulty, vxldollar::public_key const & signer_a, vxldollar::root const & root_a, vxldollar::account const & account_a) {
		epoch->block_work_set (node_a.work_generate_blocking (vxldollar::work_version::work_1, root_a, difficulty).value_or (0));
		bool valid_signature (!vxldollar::validate_message (signer_a, epoch->hash (), epoch->block_signature ()));
		bool valid_work (node_a.network_params.work.difficulty (*epoch) >= difficulty);
		vxldollar::process_result result (vxldollar::process_result::old);
		if (valid_signature && valid_work)
		{
			result = node_a.process_local (epoch).code;
		}
		if (result == vxldollar::process_result::progress)
		{
			++counter;
		}
		else
		{
			bool fork (result == vxldollar::process_result::fork);
	...
	vxldollar::raw_key raw_key;
	raw_key = prv_a;
	auto signer (vxldollar::pub_key (prv_a));
	debug_assert (signer == ledger.epoch_signer (link));

	vxldollar::mutex upgrader_mutex;
	vxldollar::condition_variable upgrader_condition;
	...
	boost::multi_index::indexed_by<
		...
		boost::multi_index::hashed_unique<boost::multi_index::tag<account_tag>,
			boost::multi_index::member<account_upgrade_item, vxldollar::account, &account_upgrade_item::account>>>>
	accounts_list;
	// clang-format on
	...
	while (!finished_upgrade && !stopped)
	{
		...
		{
			{
				...
				for (auto i (store.account.begin (transaction)), n (store.account.end ()); i != n && accounts_list.size () < count_limit; ++i)
				{
					...
					{
						release_assert (vxldollar::epochs::is_sequential (info.epoch (), epoch_a));
			...
			for (auto i (accounts_list.get<modified_tag> ().begin ()), n (accounts_list.get<modified_tag> ().end ()); i != n && attempts < upgrade_batch_size && attempts < count_limit && !stopped; ++i)
			{
				auto transaction (store.tx_begin_read ());
				vxldollar::account_info info;
				vxldollar::account const & account (i->account);
				if (!store.account.get (transaction, account, info) && info.epoch () < epoch_a)
				{
					++attempts;
					auto difficulty (network_params.work.threshold (vxldollar::work_version::work_1, vxldollar::block_details (epoch_a, false, false, true)));
					vxldollar::root const & root (info.head);
					std::shared_ptr<vxldollar::block> epoch = builder.state ()
														 .account (account)
														 .previous (info.head)
					...
					{
						{
							vxldollar::unique_lock<vxldollar::mutex> lock (upgrader_mutex);
							++workers;
						...
						this->workers.push_task ([node_l = shared_from_this (), &upgrader_process, &upgrader_mutex, &upgrader_condition, &upgraded_accounts, &workers, epoch, difficulty, signer, root, account] () {
							...
							{
								vxldollar::lock_guard<vxldollar::mutex> lock (upgrader_mutex);
								--workers;
							}
		...
		while (!finished_pending && count_limit != 0 && !stopped)
		{
			...
			uint64_t attempts (0);
			auto transaction (store.tx_begin_read ());
			for (auto i (store.pending.begin (transaction, vxldollar::pending_key (1, 0))), n (store.pending.end ()); i != n && attempts < upgrade_batch_size && attempts < count_limit && !stopped;)
			{
				...
				if (!store.account.exists (transaction, key.account))
				{
					...
					if (info.epoch < epoch_a)
					{
						...
						release_assert (vxldollar::epochs::is_sequential (info.epoch, epoch_a));
						auto difficulty (network_params.work.threshold (vxldollar::work_version::work_1, vxldollar::block_details (epoch_a, false, false, true)));
						vxldollar::root const & root (key.account);
						vxldollar::account const & account (key.account);
						std::shared_ptr<vxldollar::block> epoch = builder.state ()
															 .account (key.account)
															 .previous (0)
															 .representative (0)
				...
				if (to_next_account)
				{
					...
					if (key.account.number () == std::numeric_limits<vxldollar::uint256_t>::max ())
					{
						break;
					}
					else
					{
						i = store.pending.begin (transaction, vxldollar::pending_key (key.account.number () + 1, 0));
					}
				}
				else
				{
...
std::pair<uint64_t, decltype (vxldollar::ledger::bootstrap_weights)> vxldollar::node::get_bootstrap_weights () const
{
	std::unordered_map<vxldollar::account, vxldollar::uint128_t> weights;
	uint8_t const * weight_buffer = network_params.network.is_live_network () ? vxldollar_bootstrap_weights_live : vxldollar_bootstrap_weights_beta;
	std::size_t weight_size = network_params.network.is_live_network () ? vxldollar_bootstrap_weights_live_size : vxldollar_bootstrap_weights_beta_size;
	vxldollar::bufferstream weight_stream ((uint8_t const *)weight_buffer, weight_size);
	vxldollar::uint128_union block_height;
	uint64_t max_blocks = 0;
	if (!vxldollar::try_read (weight_stream, block_height))
	{
		max_blocks = vxldollar::narrow_cast<uint64_t> (block_height.number ());
		while (true)
		{
			vxldollar::account account;
			if (vxldollar::try_read (weight_stream, account.bytes))
			{
				break;
			}
			vxldollar::amount weight;
			if (vxldollar::try_read (weight_stream, weight.bytes))
			{
				break;
			}
...
void vxldollar::node::populate_backlog ()
{
	auto done = false;
	uint64_t const chunk_size = 65536;
	vxldollar::account next = 0;
	uint64_t total = 0;
	while (!stopped && !done)
	{
		auto transaction = store.tx_begin_read ();
...
uint64_t vxldollar::node::get_confirmation_height (vxldollar::transaction const & transaction_a, vxldollar::account & account_a)
{
...
vxldollar::node_wrapper::node_wrapper (boost::filesystem::path const & path_a, boost::filesystem::path const & config_path_a, vxldollar::node_flags const & node_flags_a) :
	network_params{ vxldollar::network_constants::active_network },
	...
	work{ network_params.network, 1 }
{
	...
	vxldollar::set_secure_perm_directory (path_a, error_chmod);
	vxldollar::daemon_config daemon_config{ path_a, network_params };
	auto error = vxldollar::read_node_config_toml (config_path_a, daemon_config, node_flags_a.config_overrides);
	if (error)
	{
	...
	node = std::make_shared<vxldollar::node> (*io_context, path_a, node_config, work, node_flags_a);
}
...
vxldollar::inactive_node::inactive_node (boost::filesystem::path const & path_a, boost::filesystem::path const & config_path_a, vxldollar::node_flags const & node_flags_a) :
...
vxldollar::inactive_node::inactive_node (boost::filesystem::path const & path_a, vxldollar::node_flags const & node_flags_a) :
	inactive_node (path_a, path_a, node_flags_a)
{
}

vxldollar::node_flags const & vxldollar::inactive_node_flag_defaults ()
{
	static vxldollar::node_flags node_flags;
...
std::unique_ptr<vxldollar::store> vxldollar::make_store (vxldollar::logger_mt & logger, boost::filesystem::path const & path, vxldollar::ledger_constants & constants, bool read_only, bool add_db_postfix, vxldollar::rocksdb_config const & rocksdb_config, vxldollar::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a, vxldollar::lmdb_config const & lmdb_config_a, bool backup_before_upgrade)
{
	if (rocksdb_config.enable)
	{
		return std::make_unique<vxldollar::rocksdb_store> (logger, add_db_postfix ? path / "rocksdb" : path, constants, rocksdb_config, read_only);
	}

	return std::make_unique<vxldollar::mdb_store> (logger, add_db_postfix ? path / "data.ldb" : path, constants, txn_tracking_config_a, block_processor_batch_max_time_a, lmdb_config_a, backup_before_upgrade);
}

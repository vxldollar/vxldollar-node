...
vxldollar::rocksdb_store::rocksdb_store (vxldollar::logger_mt & logger_a, boost::filesystem::path const & path_a, vxldollar::ledger_constants & constants, vxldollar::rocksdb_config const & rocksdb_config_a, bool open_read_only_a) :
	// clang-format off
	store_partial{
		constants,
		block_store_partial,
	...
	max_block_write_batch_num_m{ vxldollar::narrow_cast<unsigned> (blocks_memtable_size_bytes () / (2 * (sizeof (vxldollar::block_type) + vxldollar::state_block::size + vxldollar::block_sideband::size (vxldollar::block_type::state)))) },
...
std::unordered_map<char const *, vxldollar::tables> vxldollar::rocksdb_store::create_cf_name_table_map () const
{
	std::unordered_map<char const *, vxldollar::tables> map{ { rocksdb::kDefaultColumnFamilyName.c_str (), tables::default_unused },
...
void vxldollar::rocksdb_store::open (bool & error_a, boost::filesystem::path const & path_a, bool open_read_only_a)
{
...
void vxldollar::rocksdb_store::generate_tombstone_map ()
{
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (vxldollar::tables::unchecked), std::forward_as_tuple (0, 50000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (vxldollar::tables::blocks), std::forward_as_tuple (0, 25000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (vxldollar::tables::accounts), std::forward_as_tuple (0, 25000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (vxldollar::tables::pending), std::forward_as_tuple (0, 25000));
}

rocksdb::ColumnFamilyOptions vxldollar::rocksdb_store::get_common_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a, unsigned long long memtable_size_bytes_a) const
{
...
rocksdb::ColumnFamilyOptions vxldollar::rocksdb_store::get_cf_options (std::string const & cf_name_a) const
{
	rocksdb::ColumnFamilyOptions cf_options;
...
std::vector<rocksdb::ColumnFamilyDescriptor> vxldollar::rocksdb_store::create_column_families ()
{
...
vxldollar::write_transaction vxldollar::rocksdb_store::tx_begin_write (std::vector<vxldollar::tables> const & tables_requiring_locks_a, std::vector<vxldollar::tables> const & tables_no_locks_a)
{
	std::unique_ptr<vxldollar::write_rocksdb_txn> txn;
	...
	if (tables_requiring_locks_a.empty () && tables_no_locks_a.empty ())
	{
		...
		txn = std::make_unique<vxldollar::write_rocksdb_txn> (optimistic_db, all_tables (), tables_no_locks_a, write_lock_mutexes);
	}
	else
	{
		txn = std::make_unique<vxldollar::write_rocksdb_txn> (optimistic_db, tables_requiring_locks_a, tables_no_locks_a, write_lock_mutexes);
	}
	...
	return vxldollar::write_transaction{ std::move (txn) };
}

vxldollar::read_transaction vxldollar::rocksdb_store::tx_begin_read () const
{
	return vxldollar::read_transaction{ std::make_unique<vxldollar::read_rocksdb_txn> (db.get ()) };
}

std::string vxldollar::rocksdb_store::vendor_get () const
{
	return boost::str (boost::format ("RocksDB %1%.%2%.%3%") % ROCKSDB_MAJOR % ROCKSDB_MINOR % ROCKSDB_PATCH);
}

rocksdb::ColumnFamilyHandle * vxldollar::rocksdb_store::table_to_column_family (tables table_a) const
{
...
bool vxldollar::rocksdb_store::exists (vxldollar::transaction const & transaction_a, tables table_a, vxldollar::rocksdb_val const & key_a) const
{
...
int vxldollar::rocksdb_store::del (vxldollar::write_transaction const & transaction_a, tables table_a, vxldollar::rocksdb_val const & key_a)
{
...
void vxldollar::rocksdb_store::flush_tombstones_check (tables table_a)
{
...
void vxldollar::rocksdb_store::flush_table (vxldollar::tables table_a)
{
	db->Flush (rocksdb::FlushOptions{}, table_to_column_family (table_a));
}

void vxldollar::version_rocksdb_store::version_put (vxldollar::write_transaction const & transaction_a, int version_a)
{
	...
	auto status (rocksdb_store.put (transaction_a, tables::meta, version_key, vxldollar::rocksdb_val (version_value)));
	release_assert (rocksdb_store.success (status));
}

rocksdb::Transaction * vxldollar::rocksdb_store::tx (vxldollar::transaction const & transaction_a) const
{
	debug_assert (!is_read (transaction_a));
	return static_cast<rocksdb::Transaction *> (transaction_a.get_handle ());
}

int vxldollar::rocksdb_store::get (vxldollar::transaction const & transaction_a, tables table_a, vxldollar::rocksdb_val const & key_a, vxldollar::rocksdb_val & value_a) const
{
	rocksdb::ReadOptions options;
	rocksdb::PinnableSlice slice;
...
int vxldollar::rocksdb_store::put (vxldollar::write_transaction const & transaction_a, tables table_a, vxldollar::rocksdb_val const & key_a, vxldollar::rocksdb_val const & value_a)
{
	debug_assert (transaction_a.contains (table_a));
...
bool vxldollar::rocksdb_store::not_found (int status) const
{
	return (status_code_not_found () == status);
}

bool vxldollar::rocksdb_store::success (int status) const
{
	return (static_cast<int> (rocksdb::Status::Code::kOk) == status);
}

int vxldollar::rocksdb_store::status_code_not_found () const
{
	return static_cast<int> (rocksdb::Status::Code::kNotFound);
}

uint64_t vxldollar::rocksdb_store::count (vxldollar::transaction const & transaction_a, tables table_a) const
{
	uint64_t sum = 0;
...
int vxldollar::rocksdb_store::drop (vxldollar::write_transaction const & transaction_a, tables table_a)
{
	debug_assert (transaction_a.contains (table_a));
	...
	if (success (status))
	{
		...
		if (table_a == tables::peers)
		{
			...
			for (auto i = peer.begin (transaction_a), n = peer.end (); i != n; ++i)
			{
				status = del (transaction_a, tables::peers, vxldollar::rocksdb_val (i->first));
...
int vxldollar::rocksdb_store::clear (rocksdb::ColumnFamilyHandle * column_family)
{
...
vxldollar::unchecked_rocksdb_store::unchecked_rocksdb_store (vxldollar::rocksdb_store & rocksdb_store_a) :
	vxldollar::unchecked_store_partial<rocksdb::Slice, vxldollar::rocksdb_store> (rocksdb_store_a),
	rocksdb_store{ rocksdb_store_a } {};

vxldollar::version_rocksdb_store::version_rocksdb_store (vxldollar::rocksdb_store & rocksdb_store_a) :
	vxldollar::version_store_partial<rocksdb::Slice, vxldollar::rocksdb_store> (rocksdb_store_a),
	rocksdb_store{ rocksdb_store_a } {};

void vxldollar::rocksdb_store::construct_column_family_mutexes ()
{
...
rocksdb::Options vxldollar::rocksdb_store::get_db_options ()
{
...
rocksdb::BlockBasedTableOptions vxldollar::rocksdb_store::get_active_table_options (std::size_t lru_size) const
{
...
rocksdb::BlockBasedTableOptions vxldollar::rocksdb_store::get_small_table_options () const
{
	rocksdb::BlockBasedTableOptions table_options;
	// Improve point lookup performance be using the data block hash index (uses about 5% more space).
	table_options.data_block_index_type = rocksdb::BlockBasedTableOptions::DataBlockIndexType::kDataBlockBinaryAndHash;
...
rocksdb::ColumnFamilyOptions vxldollar::rocksdb_store::get_small_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a) const
{
...
rocksdb::ColumnFamilyOptions vxldollar::rocksdb_store::get_active_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a, unsigned long long memtable_size_bytes_a) const
{
...
void vxldollar::rocksdb_store::on_flush (rocksdb::FlushJobInfo const & flush_job_info_a)
{
...
std::vector<vxldollar::tables> vxldollar::rocksdb_store::all_tables () const
{
	return std::vector<vxldollar::tables>{ tables::accounts, tables::blocks, tables::confirmation_height, tables::final_votes, tables::frontiers, tables::meta, tables::online_weight, tables::peers, tables::pending, tables::pruned, tables::unchecked, tables::vote };
}

bool vxldollar::rocksdb_store::copy_db (boost::filesystem::path const & destination_path)
{
	...
	if (status.ok ())
	{
		vxldollar::rocksdb_store rocksdb_store (logger, destination_path.string (), constants, rocksdb_config, false);
		return !rocksdb_store.init_error ();
	}
	return false;
}

void vxldollar::rocksdb_store::rebuild_db (vxldollar::write_transaction const & transaction_a)
{
	// Not available for RocksDB
}

bool vxldollar::rocksdb_store::init_error () const
{
	return error;
}

void vxldollar::rocksdb_store::serialize_memory_stats (boost::property_tree::ptree & json)
{
	uint64_t val;
...
unsigned long long vxldollar::rocksdb_store::blocks_memtable_size_bytes () const
{
	return base_memtable_size_bytes ();
}

unsigned long long vxldollar::rocksdb_store::base_memtable_size_bytes () const
{
...
unsigned vxldollar::rocksdb_store::max_block_write_batch_num () const
{
	return max_block_write_batch_num_m;
}

std::string vxldollar::rocksdb_store::error_string (int status) const
{
	return std::to_string (status);
}

vxldollar::rocksdb_store::tombstone_info::tombstone_info (uint64_t num_since_last_flush_a, uint64_t const max_a) :
...
template class vxldollar::store_partial<rocksdb::Slice, vxldollar::rocksdb_store>;

...
class request_aggregator final
{
	...
	{
		channel_pool () = delete;
		explicit channel_pool (std::shared_ptr<vxldollar::transport::channel> const & channel_a) :
			channel (channel_a),
			endpoint (vxldollar::transport::map_endpoint_to_v6 (channel_a->get_endpoint ()))
		{
		}
		std::vector<std::pair<vxldollar::block_hash, vxldollar::root>> hashes_roots;
...
public:
	request_aggregator (vxldollar::node_config const & config, vxldollar::stat & stats_a, vxldollar::vote_generator &, vxldollar::vote_generator &, vxldollar::local_vote_history &, vxldollar::ledger &, vxldollar::wallets &, vxldollar::active_transactions &);

	/** Add a new request by \p channel_a for hashes \p hashes_roots_a */
	void add (std::shared_ptr<vxldollar::transport::channel> const & channel_a, std::vector<std::pair<vxldollar::block_hash, vxldollar::root>> const & hashes_roots_a);
	void stop ();
...
private:
	...
	void erase_duplicates (std::vector<std::pair<vxldollar::block_hash, vxldollar::root>> &) const;
	...
	std::pair<std::vector<std::shared_ptr<vxldollar::block>>, std::vector<std::shared_ptr<vxldollar::block>>> aggregate (std::vector<std::pair<vxldollar::block_hash, vxldollar::root>> const & requests_a, std::shared_ptr<vxldollar::transport::channel> & channel_a) const;
	void reply_action (std::shared_ptr<vxldollar::vote> const & vote_a, std::shared_ptr<vxldollar::transport::channel> const & channel_a) const;

	vxldollar::stat & stats;
	vxldollar::local_vote_history & local_votes;
	...
	mi::indexed_by<
		mi::hashed_unique<mi::tag<tag_endpoint>,
			mi::member<channel_pool, vxldollar::endpoint, &channel_pool::endpoint>>,
	...
	vxldollar::mutex mutex{ mutex_identifier (mutexes::request_aggregator) };
	std::thread thread;

...
{
public:
	std::shared_ptr<vxldollar::message> message;
...
{
	class tcp_channels;
	class channel_tcp : public vxldollar::transport::channel
	{
		friend class vxldollar::transport::tcp_channels;

	public:
		channel_tcp (vxldollar::node &, std::weak_ptr<vxldollar::socket>);
		...
		bool operator== (vxldollar::transport::channel_tcp const & other_a) const
		{
			return &node == &other_a.node && socket.lock () == other_a.socket.lock ();
		}
		std::weak_ptr<vxldollar::socket> socket;
		...
		{
			return vxldollar::transport::map_tcp_to_endpoint (get_tcp_endpoint ());
		}
		...
		{
			return vxldollar::transport::transport_type::tcp;
		}

	private:
		vxldollar::tcp_endpoint endpoint{ boost::asio::ip::address_v6::any (), 0 };
	};
	class tcp_channels final
	{
		friend class vxldollar::transport::channel_tcp;
		friend class telemetry_simultaneous_requests_Test;

	public:
		explicit tcp_channels (vxldollar::node &, std::function<void (vxldollar::message const &, std::shared_ptr<vxldollar::transport::channel> const &)> = nullptr);
		bool insert (std::shared_ptr<vxldollar::transport::channel_tcp> const &, std::shared_ptr<vxldollar::socket> const &, std::shared_ptr<vxldollar::bootstrap_server> const &);
		void erase (vxldollar::tcp_endpoint const &);
		std::size_t size () const;
		std::shared_ptr<vxldollar::transport::channel_tcp> find_channel (vxldollar::tcp_endpoint const &) const;
		...
		std::shared_ptr<vxldollar::transport::channel_tcp> find_node_id (vxldollar::account const &);
		...
		void process_message (vxldollar::message const &, vxldollar::tcp_endpoint const &, vxldollar::account const &, std::shared_ptr<vxldollar::socket> const &);
		bool max_ip_connections (vxldollar::tcp_endpoint const & endpoint_a);
		bool max_subnetwork_connections (vxldollar::tcp_endpoint const & endpoint_a);
		bool max_ip_or_subnetwork_connections (vxldollar::tcp_endpoint const & endpoint_a);
		// Should we reach out to this endpoint with a keepalive message
		bool reachout (vxldollar::endpoint const &);
		std::unique_ptr<container_info_component> collect_container_info (std::string const &);
		void purge (std::chrono::steady_clock::time_point const &);
		void ongoing_keepalive ();
		void list (std::deque<std::shared_ptr<vxldollar::transport::channel>> &, uint8_t = 0, bool = true);
		void modify (std::shared_ptr<vxldollar::transport::channel_tcp> const &, std::function<void (std::shared_ptr<vxldollar::transport::channel_tcp> const &)>);
		void update (vxldollar::tcp_endpoint const &);
		// Connection start
		void start_tcp (vxldollar::endpoint const &);
		void start_tcp_receive_node_id (std::shared_ptr<vxldollar::transport::channel_tcp> const &, vxldollar::endpoint const &, std::shared_ptr<std::vector<uint8_t>> const &);
		void udp_fallback (vxldollar::endpoint const &);
		vxldollar::node & node;
	...
	private:
		...
		public:
			...
			std::shared_ptr<vxldollar::socket> socket;
			std::shared_ptr<vxldollar::bootstrap_server> response_server;
			channel_tcp_wrapper (std::shared_ptr<vxldollar::transport::channel_tcp> channel_a, std::shared_ptr<vxldollar::socket> socket_a, std::shared_ptr<vxldollar::bootstrap_server> server_a) :
				channel (std::move (channel_a)), socket (std::move (socket_a)), response_server (std::move (server_a))
			{
			}
			vxldollar::tcp_endpoint endpoint () const
			{
		...
		public:
			...
			explicit tcp_endpoint_attempt (vxldollar::tcp_endpoint const & endpoint_a) :
				endpoint (endpoint_a),
				address (vxldollar::transport::ipv4_address_or_ipv6_subnet (endpoint_a.address ())),
		...
		mi::indexed_by<
			...
			mi::hashed_unique<mi::tag<endpoint_tag>,
				mi::const_mem_fun<channel_tcp_wrapper, vxldollar::tcp_endpoint, &channel_tcp_wrapper::endpoint>>,
			mi::hashed_non_unique<mi::tag<node_id_tag>,
				mi::const_mem_fun<channel_tcp_wrapper, vxldollar::account, &channel_tcp_wrapper::node_id>>,
		...
		mi::indexed_by<
			mi::hashed_unique<mi::tag<endpoint_tag>,
				mi::member<tcp_endpoint_attempt, vxldollar::tcp_endpoint, &tcp_endpoint_attempt::endpoint>>,
			mi::hashed_non_unique<mi::tag<ip_address_tag>,

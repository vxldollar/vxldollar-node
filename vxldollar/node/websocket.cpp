...
#include <vxldollar/boost/asio/dispatch.hpp>
...
vxldollar::websocket::confirmation_options::confirmation_options (vxldollar::wallets & wallets_a) :
	wallets (wallets_a)
{
}

vxldollar::websocket::confirmation_options::confirmation_options (boost::property_tree::ptree const & options_a, vxldollar::wallets & wallets_a, vxldollar::logger_mt & logger_a) :
	wallets (wallets_a),
	logger (logger_a)
{
...
bool vxldollar::websocket::confirmation_options::should_filter (vxldollar::websocket::message const & message_a) const
{
	...
	if (destination_opt_l)
	{
		...
		if (all_local_accounts)
		{
			...
			vxldollar::account source_l{};
			vxldollar::account destination_l{};
...
bool vxldollar::websocket::confirmation_options::update (boost::property_tree::ptree const & options_a)
{
...
void vxldollar::websocket::confirmation_options::check_filter_empty () const
{
...
vxldollar::websocket::vote_options::vote_options (boost::property_tree::ptree const & options_a, vxldollar::logger_mt & logger_a)
{
...
bool vxldollar::websocket::vote_options::should_filter (vxldollar::websocket::message const & message_a) const
{
...
vxldollar::websocket::session::session (vxldollar::websocket::listener & listener_a, socket_type socket_a, boost::asio::ssl::context & ctx_a) :
...
vxldollar::websocket::session::session (vxldollar::websocket::listener & listener_a, socket_type socket_a) :
	ws_listener (listener_a), ws (std::move (socket_a))
{
	ws_listener.get_logger ().try_log ("Websocket: session started");
}

vxldollar::websocket::session::~session ()
{
	{
		vxldollar::unique_lock<vxldollar::mutex> lk (subscriptions_mutex);
...
void vxldollar::websocket::session::handshake ()
{
...
void vxldollar::websocket::session::close ()
{
...
void vxldollar::websocket::session::write (vxldollar::websocket::message message_a)
{
	vxldollar::unique_lock<vxldollar::mutex> lk (subscriptions_mutex);
	auto subscription (subscriptions.find (message_a.topic));
	if (message_a.topic == vxldollar::websocket::topic::ack || (subscription != subscriptions.end () && !subscription->second->should_filter (message_a)))
	{
		lk.unlock ();
		auto this_l (shared_from_this ());
...
void vxldollar::websocket::session::write_queued_messages ()
{
	auto msg (send_queue.front ().to_string ());
	auto this_l (shared_from_this ());

	ws.async_write (vxldollar::shared_const_buffer (msg),
...
void vxldollar::websocket::session::read ()
{
	auto this_l (shared_from_this ());
...
namespace
{
vxldollar::websocket::topic to_topic (std::string const & topic_a)
{
	vxldollar::websocket::topic topic = vxldollar::websocket::topic::invalid;
	if (topic_a == "confirmation")
	{
		topic = vxldollar::websocket::topic::confirmation;
	}
	else if (topic_a == "stopped_election")
	{
		topic = vxldollar::websocket::topic::stopped_election;
	}
	else if (topic_a == "vote")
	{
		topic = vxldollar::websocket::topic::vote;
	}
	else if (topic_a == "ack")
	{
		topic = vxldollar::websocket::topic::ack;
	}
	else if (topic_a == "work")
	{
		topic = vxldollar::websocket::topic::work;
	}
	else if (topic_a == "bootstrap")
	{
		topic = vxldollar::websocket::topic::bootstrap;
	}
	else if (topic_a == "telemetry")
	{
		topic = vxldollar::websocket::topic::telemetry;
	}
	else if (topic_a == "new_unconfirmed_block")
	{
		topic = vxldollar::websocket::topic::new_unconfirmed_block;
	}

	return topic;
}

std::string from_topic (vxldollar::websocket::topic topic_a)
{
	std::string topic = "invalid";
	if (topic_a == vxldollar::websocket::topic::confirmation)
	{
		topic = "confirmation";
	}
	else if (topic_a == vxldollar::websocket::topic::stopped_election)
	{
		topic = "stopped_election";
	}
	else if (topic_a == vxldollar::websocket::topic::vote)
	{
		topic = "vote";
	}
	else if (topic_a == vxldollar::websocket::topic::ack)
	{
		topic = "ack";
	}
	else if (topic_a == vxldollar::websocket::topic::work)
	{
		topic = "work";
	}
	else if (topic_a == vxldollar::websocket::topic::bootstrap)
	{
		topic = "bootstrap";
	}
	else if (topic_a == vxldollar::websocket::topic::telemetry)
	{
		topic = "telemetry";
	}
	else if (topic_a == vxldollar::websocket::topic::new_unconfirmed_block)
	{
...
void vxldollar::websocket::session::send_ack (std::string action_a, std::string id_a)
{
	vxldollar::websocket::message msg (vxldollar::websocket::topic::ack);
	boost::property_tree::ptree & message_l = msg.contents;
	message_l.add ("ack", action_a);
	message_l.add ("time", std::to_string (vxldollar::milliseconds_since_epoch ()));
	if (!id_a.empty ())
	{
...
void vxldollar::websocket::session::handle_message (boost::property_tree::ptree const & message_a)
{
	...
	if (action == "subscribe" && topic_l != vxldollar::websocket::topic::invalid)
	{
		auto options_text_l (message_a.get_child_optional ("options"));
		vxldollar::lock_guard<vxldollar::mutex> lk (subscriptions_mutex);
		std::unique_ptr<vxldollar::websocket::options> options_l{ nullptr };
		if (options_text_l && topic_l == vxldollar::websocket::topic::confirmation)
		{
			options_l = std::make_unique<vxldollar::websocket::confirmation_options> (options_text_l.get (), ws_listener.get_wallets (), ws_listener.get_logger ());
		}
		else if (options_text_l && topic_l == vxldollar::websocket::topic::vote)
		{
			options_l = std::make_unique<vxldollar::websocket::vote_options> (options_text_l.get (), ws_listener.get_logger ());
		}
		else
		{
			options_l = std::make_unique<vxldollar::websocket::options> ();
		}
	...
	else if (action == "unsubscribe" && topic_l != vxldollar::websocket::topic::invalid)
	{
		vxldollar::lock_guard<vxldollar::mutex> lk (subscriptions_mutex);
...
void vxldollar::websocket::listener::stop ()
{
	stopped = true;
	acceptor.close ();

	vxldollar::lock_guard<vxldollar::mutex> lk (sessions_mutex);
...
vxldollar::websocket::listener::listener (std::shared_ptr<vxldollar::tls_config> const & tls_config_a, vxldollar::logger_mt & logger_a, vxldollar::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a) :
...
void vxldollar::websocket::listener::run ()
{
	if (acceptor.is_open ())
	{
		accept ();
...
void vxldollar::websocket::listener::accept ()
{
	auto this_l (shared_from_this ());
...
void vxldollar::websocket::listener::on_accept (boost::system::error_code ec)
{
	if (ec)
	{
	...
	else
	{
		...
		std::shared_ptr<vxldollar::websocket::session> session;
		if (tls_config && tls_config->enable_wss)
		{
#ifdef VXLDOLLAR_SECURE_RPC
			session = std::make_shared<vxldollar::websocket::session> (*this, std::move (socket), tls_config->ssl_context);
#endif
		}
		else
		{
			session = std::make_shared<vxldollar::websocket::session> (*this, std::move (socket));
		}
...
void vxldollar::websocket::listener::broadcast_confirmation (std::shared_ptr<vxldollar::block> const & block_a, vxldollar::account const & account_a, vxldollar::amount const & amount_a, std::string const & subtype, vxldollar::election_status const & election_status_a, std::vector<vxldollar::vote_with_weight_info> const & election_votes_a)
{
	vxldollar::websocket::message_builder builder;

	vxldollar::lock_guard<vxldollar::mutex> lk (sessions_mutex);
	boost::optional<vxldollar::websocket::message> msg_with_block;
	boost::optional<vxldollar::websocket::message> msg_without_block;
	for (auto & weak_session : sessions)
	{
		auto session_ptr (weak_session.lock ());
		if (session_ptr)
		{
			auto subscription (session_ptr->subscriptions.find (vxldollar::websocket::topic::confirmation));
			if (subscription != session_ptr->subscriptions.end ())
			{
				vxldollar::websocket::confirmation_options default_options (wallets);
				auto conf_options (dynamic_cast<vxldollar::websocket::confirmation_options *> (subscription->second.get ()));
...
void vxldollar::websocket::listener::broadcast (vxldollar::websocket::message message_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lk (sessions_mutex);
	for (auto & weak_session : sessions)
	{
...
void vxldollar::websocket::listener::increase_subscriber_count (vxldollar::websocket::topic const & topic_a)
{
	topic_subscriber_count[static_cast<std::size_t> (topic_a)] += 1;
}

void vxldollar::websocket::listener::decrease_subscriber_count (vxldollar::websocket::topic const & topic_a)
{
...
vxldollar::websocket::message vxldollar::websocket::message_builder::stopped_election (vxldollar::block_hash const & hash_a)
{
	vxldollar::websocket::message message_l (vxldollar::websocket::topic::stopped_election);
...
vxldollar::websocket::message vxldollar::websocket::message_builder::block_confirmed (std::shared_ptr<vxldollar::block> const & block_a, vxldollar::account const & account_a, vxldollar::amount const & amount_a, std::string subtype, bool include_block_a, vxldollar::election_status const & election_status_a, std::vector<vxldollar::vote_with_weight_info> const & election_votes_a, vxldollar::websocket::confirmation_options const & options_a)
{
	vxldollar::websocket::message message_l (vxldollar::websocket::topic::confirmation);
	set_common_fields (message_l);
...
vxldollar::websocket::message vxldollar::websocket::message_builder::vote_received (std::shared_ptr<vxldollar::vote> const & vote_a, vxldollar::vote_code code_a)
{
	vxldollar::websocket::message message_l (vxldollar::websocket::topic::vote);
	set_common_fields (message_l);
...
vxldollar::websocket::message vxldollar::websocket::message_builder::work_generation (vxldollar::work_version const version_a, vxldollar::block_hash const & root_a, uint64_t work_a, uint64_t difficulty_a, uint64_t publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool completed_a, bool cancelled_a)
{
	vxldollar::websocket::message message_l (vxldollar::websocket::topic::work);
	set_common_fields (message_l);
	...
	request_l.put ("version", vxldollar::to_string (version_a));
	request_l.put ("hash", root_a.to_string ());
	request_l.put ("difficulty", vxldollar::to_string_hex (difficulty_a));
	auto request_multiplier_l (vxldollar::difficulty::to_multiplier (difficulty_a, publish_threshold_a));
	request_l.put ("multiplier", vxldollar::to_string (request_multiplier_l));
	...
	if (completed_a)
	{
		...
		result_l.put ("work", vxldollar::to_string_hex (work_a));
		auto result_difficulty_l (vxldollar::dev::network_params.work.difficulty (version_a, root_a, work_a));
		result_l.put ("difficulty", vxldollar::to_string_hex (result_difficulty_l));
		auto result_multiplier_l (vxldollar::difficulty::to_multiplier (result_difficulty_l, publish_threshold_a));
		result_l.put ("multiplier", vxldollar::to_string (result_multiplier_l));
...
vxldollar::websocket::message vxldollar::websocket::message_builder::work_cancelled (vxldollar::work_version const version_a, vxldollar::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (version_a, root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, true);
}

vxldollar::websocket::message vxldollar::websocket::message_builder::work_failed (vxldollar::work_version const version_a, vxldollar::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (version_a, root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, false);
}

vxldollar::websocket::message vxldollar::websocket::message_builder::bootstrap_started (std::string const & id_a, std::string const & mode_a)
{
	vxldollar::websocket::message message_l (vxldollar::websocket::topic::bootstrap);
	set_common_fields (message_l);
...
vxldollar::websocket::message vxldollar::websocket::message_builder::bootstrap_exited (std::string const & id_a, std::string const & mode_a, std::chrono::steady_clock::time_point const start_time_a, uint64_t const total_blocks_a)
{
	vxldollar::websocket::message message_l (vxldollar::websocket::topic::bootstrap);
	set_common_fields (message_l);
...
vxldollar::websocket::message vxldollar::websocket::message_builder::telemetry_received (vxldollar::telemetry_data const & telemetry_data_a, vxldollar::endpoint const & endpoint_a)
{
	vxldollar::websocket::message message_l (vxldollar::websocket::topic::telemetry);
	set_common_fields (message_l);

	// Telemetry information
	vxldollar::jsonconfig telemetry_l;
...
vxldollar::websocket::message vxldollar::websocket::message_builder::new_block_arrived (vxldollar::block const & block_a)
{
	vxldollar::websocket::message message_l (vxldollar::websocket::topic::new_unconfirmed_block);
	set_common_fields (message_l);

	boost::property_tree::ptree block_l;
	block_a.serialize_json (block_l);
	auto subtype (vxldollar::state_subtype (block_a.sideband ().details));
...
void vxldollar::websocket::message_builder::set_common_fields (vxldollar::websocket::message & message_a)
{
	// Common message information
	message_a.contents.add ("topic", from_topic (message_a.topic));
	message_a.contents.add ("time", std::to_string (vxldollar::milliseconds_since_epoch ()));
}

std::string vxldollar::websocket::message::to_string () const
{

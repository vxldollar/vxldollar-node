...
std::size_t constexpr vxldollar::active_transactions::max_active_elections_frontier_insertion;

constexpr std::chrono::minutes vxldollar::active_transactions::expired_optimistic_election_info_cutoff;

vxldollar::active_transactions::active_transactions (vxldollar::node & node_a, vxldollar::confirmation_height_processor & confirmation_height_processor_a) :
	...
	thread ([this] () {
		vxldollar::thread_role::set (vxldollar::thread_role::name::request_loop);
		request_loop ();
	})
{
	// Register a callback which will get called after a block is cemented
	confirmation_height_processor.add_cemented_observer ([this] (std::shared_ptr<vxldollar::block> const & callback_block_a) {
		this->block_cemented_callback (callback_block_a);
	});

	// Register a callback which will get called if a block is already cemented
	confirmation_height_processor.add_block_already_cemented_observer ([this] (vxldollar::block_hash const & hash_a) {
...
vxldollar::active_transactions::~active_transactions ()
{
	stop ();
}

bool vxldollar::active_transactions::insert_election_from_frontiers_confirmation (std::shared_ptr<vxldollar::block> const & block_a, vxldollar::account const & account_a, vxldollar::uint128_t previous_balance_a, vxldollar::election_behavior election_behavior_a)
{
	bool inserted{ false };
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	if (roots.get<tag_root> ().find (block_a->qualified_root ()) == roots.get<tag_root> ().end ())
	{
		std::function<void (std::shared_ptr<vxldollar::block> const &)> election_confirmation_cb;
		if (election_behavior_a == vxldollar::election_behavior::optimistic)
		{
			election_confirmation_cb = [this] (std::shared_ptr<vxldollar::block> const & block_a) {
...
vxldollar::frontiers_confirmation_info vxldollar::active_transactions::get_frontiers_confirmation_info ()
{
	...
	bool agressive_mode (half_princpal_representative || node.config.frontiers_confirmation == vxldollar::frontiers_confirmation_mode::always);
	...
	return vxldollar::frontiers_confirmation_info{ max_elections, agressive_mode };
}

void vxldollar::active_transactions::set_next_frontier_check (bool agressive_mode_a)
{
...
void vxldollar::active_transactions::confirm_prioritized_frontiers (vxldollar::transaction const & transaction_a, uint64_t max_elections_a, uint64_t & elections_count_a)
{
				...
				if (!error)
				{
					if (!this->confirmation_height_processor.is_processing_block (info.head))
					{
						...
						if (info.block_count > confirmation_height_info.height)
						{
							...
							auto inserted_election = this->insert_election_from_frontiers_confirmation (block, cementable_account.account, previous_balance, vxldollar::election_behavior::optimistic);
...
void vxldollar::active_transactions::block_cemented_callback (std::shared_ptr<vxldollar::block> const & block_a)
{
	auto transaction = node.store.tx_begin_read ();

	boost::optional<vxldollar::election_status_type> election_status_type;
	...
	else
	{
		...
		election_status_type = vxldollar::election_status_type::active_confirmed_quorum;
	}

	if (election_status_type.is_initialized ())
	{
		if (election_status_type == vxldollar::election_status_type::inactive_confirmation_height)
		{
			vxldollar::account account{};
			vxldollar::uint128_t amount (0);
			bool is_state_send (false);
			bool is_state_epoch (false);
			vxldollar::account pending_account{};
			node.process_confirmed_data (transaction, block_a, block_a->hash (), account, amount, is_state_send, is_state_epoch, pending_account);
			node.observers.blocks.notify (vxldollar::election_status{ block_a, 0, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), 0, 1, 0, vxldollar::election_status_type::inactive_confirmation_height }, {}, account, amount, is_state_send, is_state_epoch);
		}
		else
		{
			auto hash (block_a->hash ());
			vxldollar::unique_lock<vxldollar::mutex> election_winners_lk (election_winner_details_mutex);
			...
			if (existing != election_winner_details.end ())
			{
				...
				{
					vxldollar::unique_lock<vxldollar::mutex> election_lk (election->mutex);
		...
		bool const was_active{ *election_status_type == vxldollar::election_status_type::active_confirmed_quorum || *election_status_type == vxldollar::election_status_type::active_confirmation_height };
...
void vxldollar::active_transactions::add_election_winner_details (vxldollar::block_hash const & hash_a, std::shared_ptr<vxldollar::election> const & election_a)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (election_winner_details_mutex);
	election_winner_details.emplace (hash_a, election_a);
}

void vxldollar::active_transactions::remove_election_winner_details (vxldollar::block_hash const & hash_a)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (election_winner_details_mutex);
	election_winner_details.erase (hash_a);
}

void vxldollar::active_transactions::block_already_cemented_callback (vxldollar::block_hash const & hash_a)
{
...
int64_t vxldollar::active_transactions::vacancy () const
{
...
void vxldollar::active_transactions::request_confirm (vxldollar::unique_lock<vxldollar::mutex> & lock_a)
{
	...
	vxldollar::confirmation_solicitor solicitor (node.network, node.config);
	solicitor.prepare (node.rep_crawler.principal_representatives (std::numeric_limits<std::size_t>::max ()));
	vxldollar::vote_generator_session generator_session (generator);
	vxldollar::vote_generator_session final_generator_session (generator);

	std::size_t unconfirmed_count_l (0);
	vxldollar::timer<std::chrono::milliseconds> elapsed (vxldollar::timer_state::started);
	...
	for (auto const & election_l : elections_l)
	{
		...
		if (election_l->transition_time (solicitor))
		{
			...
			if (!confirmed_l)
			{
				node.stats.inc (vxldollar::stat::type::election, vxldollar::stat::detail::election_drop_expired);
			}
...
void vxldollar::active_transactions::cleanup_election (vxldollar::unique_lock<vxldollar::mutex> & lock_a, vxldollar::election const & election)
{
	if (!election.confirmed ())
	{
		node.stats.inc (vxldollar::stat::type::election, vxldollar::stat::detail::election_drop_all);
	}
	...
	node.stats.inc (vxldollar::stat::type::election, election.confirmed () ? vxldollar::stat::detail::election_confirmed : vxldollar::stat::detail::election_not_confirmed);
...
std::vector<std::shared_ptr<vxldollar::election>> vxldollar::active_transactions::list_active (std::size_t max_a)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	return list_active_impl (max_a);
}

std::vector<std::shared_ptr<vxldollar::election>> vxldollar::active_transactions::list_active_impl (std::size_t max_a) const
{
	std::vector<std::shared_ptr<vxldollar::election>> result_l;
...
void vxldollar::active_transactions::add_expired_optimistic_election (vxldollar::election const & election_a)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
...
unsigned vxldollar::active_transactions::max_optimistic ()
{
	return node.ledger.cache.cemented_count < node.ledger.bootstrap_weight_max_blocks ? std::numeric_limits<unsigned>::max () : 50u;
}

void vxldollar::active_transactions::frontiers_confirmation (vxldollar::unique_lock<vxldollar::mutex> & lock_a)
{
 ...
 * confirmed.
 */
void vxldollar::active_transactions::confirm_expired_frontiers_pessimistically (vxldollar::transaction const & transaction_a, uint64_t max_elections_a, uint64_t & elections_count_a)
{
	auto i{ node.store.account.begin (transaction_a, next_frontier_account) };
	auto n{ node.store.account.end () };
	vxldollar::timer<std::chrono::milliseconds> timer (vxldollar::timer_state::started);
	vxldollar::confirmation_height_info confirmation_height_info;

	// Loop through any expired optimistic elections which have not been started yet. This tag stores already started ones first
	std::vector<vxldollar::account> elections_started_for_account;
	for (auto i = expired_optimistic_election_infos.get<tag_election_started> ().lower_bound (false); i != expired_optimistic_election_infos.get<tag_election_started> ().end ();)
	{
		...
		if (!node.store.account.get (transaction_a, account, account_info))
		{
			...
			{
				should_delete = false;
				std::shared_ptr<vxldollar::block> previous_block;
				...
				{
					vxldollar::uint128_t previous_balance{ 0 };
					if (previous_block && previous_block->balance ().is_zero ())
					{
						previous_balance = previous_block->sideband ().balance.number ();
					}

					auto inserted_election = insert_election_from_frontiers_confirmation (block, account, previous_balance, vxldollar::election_behavior::normal);
					if (inserted_election)
					{
						++elections_count_a;
					}
...
bool vxldollar::active_transactions::should_do_frontiers_confirmation () const
{
	auto pending_confirmation_height_size (confirmation_height_processor.awaiting_processing_size ());
	auto disabled_confirmation_mode = (node.config.frontiers_confirmation == vxldollar::frontiers_confirmation_mode::disabled);
...
void vxldollar::active_transactions::request_loop ()
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	started = true;
	lock.unlock ();
...
bool vxldollar::active_transactions::prioritize_account_for_confirmation (vxldollar::active_transactions::prioritize_num_uncemented & cementable_frontiers_a, std::size_t & cementable_frontiers_size_a, vxldollar::account const & account_a, vxldollar::account_info const & info_a, uint64_t confirmation_height_a)
{
	...
	if (info_a.block_count > confirmation_height_a && !confirmation_height_processor.is_processing_block (info_a.head))
	{
		...
		if (it != cementable_frontiers_a.get<tag_account> ().end ())
		{
			if (it->blocks_uncemented != num_uncemented)
			{
				...
				cementable_frontiers_a.get<tag_account> ().modify (it, [num_uncemented] (vxldollar::cementable_account & info) {
...
void vxldollar::active_transactions::prioritize_frontiers_for_confirmation (vxldollar::transaction const & transaction_a, std::chrono::milliseconds ledger_account_traversal_max_time_a, std::chrono::milliseconds wallet_account_traversal_max_time_a)
{
	...
	if (confirmation_height_processor.awaiting_processing_size () < confirmed_frontiers_max_pending_size)
	{
		...
		vxldollar::timer<std::chrono::milliseconds> wallet_account_timer (vxldollar::timer_state::started);
		...
		if (!skip_wallets)
		{
			...
			{
				vxldollar::lock_guard<vxldollar::mutex> lock (node.wallets.mutex);
...
void vxldollar::active_transactions::stop ()
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	if (!started)
	{
...
vxldollar::election_insertion_result vxldollar::active_transactions::insert_impl (vxldollar::unique_lock<vxldollar::mutex> & lock_a, std::shared_ptr<vxldollar::block> const & block_a, boost::optional<vxldollar::uint128_t> const & previous_balance_a, vxldollar::election_behavior election_behavior_a, std::function<void (std::shared_ptr<vxldollar::block> const &)> const & confirmation_action_a)
{
	debug_assert (lock_a.owns_lock ());
	debug_assert (block_a->has_sideband ());
	vxldollar::election_insertion_result result;
	if (!stopped)
	{
		...
		if (existing == roots.get<tag_root> ().end ())
		{
			if (recently_confirmed.get<tag_root> ().find (root) == recently_confirmed.get<tag_root> ().end ())
			{
				...
				vxldollar::uint128_t previous_balance (previous_balance_a.value_or (0));
				...
				result.election = vxldollar::make_shared<vxldollar::election> (
				node, block_a, confirmation_action_a, [&node = node] (auto const & rep_a) {
					// Representative is defined as online if replying to live votes or rep_crawler queries
					node.online_reps.observe (rep_a);
				},
				election_behavior_a);
				roots.get<tag_root> ().emplace (vxldollar::active_transactions::conflict_info{ root, result.election, epoch, previous_balance });
				blocks.emplace (hash, result.election);
				auto const cache = find_inactive_votes_cache_impl (hash);
				lock_a.unlock ();
				result.election->insert_inactive_votes_cache (cache);
				node.stats.inc (vxldollar::stat::type::election, vxldollar::stat::detail::election_start);
				vacancy_update ();
			}
		}
		else
...
vxldollar::vote_code vxldollar::active_transactions::vote (std::shared_ptr<vxldollar::vote> const & vote_a)
{
	vxldollar::vote_code result{ vxldollar::vote_code::indeterminate };
	// If all hashes were recently confirmed then it is a replay
	unsigned recently_confirmed_counter (0);
	std::vector<std::pair<std::shared_ptr<vxldollar::election>, vxldollar::block_hash>> process;
	{
		vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
		for (auto vote_block : vote_a->blocks)
		{
			...
			if (vote_block.which ())
			{
				auto const & block_hash (boost::get<vxldollar::block_hash> (vote_block));
			...
			{
				auto block (boost::get<std::shared_ptr<vxldollar::block>> (vote_block));
	...
	if (!process.empty ())
	{
		...
		result = replay ? vxldollar::vote_code::replay : vxldollar::vote_code::vote;
	}
	else if (recently_confirmed_counter == vote_a->blocks.size ())
	{
		result = vxldollar::vote_code::replay;
	}
	return result;
}

bool vxldollar::active_transactions::active (vxldollar::qualified_root const & root_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	return roots.get<tag_root> ().find (root_a) != roots.get<tag_root> ().end ();
}

bool vxldollar::active_transactions::active (vxldollar::block const & block_a)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	return roots.get<tag_root> ().find (block_a.qualified_root ()) != roots.get<tag_root> ().end () && blocks.find (block_a.hash ()) != blocks.end ();
}

std::shared_ptr<vxldollar::election> vxldollar::active_transactions::election (vxldollar::qualified_root const & root_a) const
{
	std::shared_ptr<vxldollar::election> result;
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
...
std::shared_ptr<vxldollar::block> vxldollar::active_transactions::winner (vxldollar::block_hash const & hash_a) const
{
...
std::deque<vxldollar::election_status> vxldollar::active_transactions::list_recently_cemented ()
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	return recently_cemented;
}

void vxldollar::active_transactions::add_recently_cemented (vxldollar::election_status const & status_a)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
...
void vxldollar::active_transactions::add_recently_confirmed (vxldollar::qualified_root const & root_a, vxldollar::block_hash const & hash_a)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
...
void vxldollar::active_transactions::erase_recently_confirmed (vxldollar::block_hash const & hash_a)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	recently_confirmed.get<tag_hash> ().erase (hash_a);
}

void vxldollar::active_transactions::erase (vxldollar::block const & block_a)
{
	erase (block_a.qualified_root ());
}

void vxldollar::active_transactions::erase (vxldollar::qualified_root const & root_a)
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
...
void vxldollar::active_transactions::erase_hash (vxldollar::block_hash const & hash_a)
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
...
void vxldollar::active_transactions::erase_oldest ()
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	if (!roots.empty ())
	{
		node.stats.inc (vxldollar::stat::type::election, vxldollar::stat::detail::election_drop_overflow);
		auto item = roots.get<tag_random_access> ().front ();
		cleanup_election (lock, *item.election);
	}
}

bool vxldollar::active_transactions::empty ()
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	return roots.empty ();
}

std::size_t vxldollar::active_transactions::size ()
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	return roots.size ();
}

bool vxldollar::active_transactions::publish (std::shared_ptr<vxldollar::block> const & block_a)
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	...
	if (existing != roots.get<tag_root> ().end ())
	{
		...
		if (!result)
		{
			...
			node.stats.inc (vxldollar::stat::type::election, vxldollar::stat::detail::election_block_conflict);
		}
	}
	return result;
}
...
boost::optional<vxldollar::election_status_type> vxldollar::active_transactions::confirm_block (vxldollar::transaction const & transaction_a, std::shared_ptr<vxldollar::block> const & block_a)
{
	auto hash (block_a->hash ());
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	auto existing (blocks.find (hash));
	boost::optional<vxldollar::election_status_type> status_type;
	if (existing != blocks.end ())
	{
		lock.unlock ();
		vxldollar::unique_lock<vxldollar::mutex> election_lock (existing->second->mutex);
		if (existing->second->status.winner && existing->second->status.winner->hash () == hash)
		{
			if (!existing->second->confirmed ())
			{
				existing->second->confirm_once (election_lock, vxldollar::election_status_type::active_confirmation_height);
				status_type = vxldollar::election_status_type::active_confirmation_height;
			}
			else
			{
#ifndef NDEBUG
...
#endif
				status_type = vxldollar::election_status_type::active_confirmed_quorum;
			}
		}
		else
		{
			status_type = boost::optional<vxldollar::election_status_type>{};
		}
	}
	else
	{
		status_type = vxldollar::election_status_type::inactive_confirmation_height;
	}

	return status_type;
}

std::size_t vxldollar::active_transactions::priority_cementable_frontiers_size ()
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	return priority_cementable_frontiers.size ();
}

std::size_t vxldollar::active_transactions::priority_wallet_cementable_frontiers_size ()
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	return priority_wallet_cementable_frontiers.size ();
}

std::size_t vxldollar::active_transactions::inactive_votes_cache_size ()
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	return inactive_votes_cache.size ();
}

void vxldollar::active_transactions::add_inactive_votes_cache (vxldollar::unique_lock<vxldollar::mutex> & lock_a, vxldollar::block_hash const & hash_a, vxldollar::account const & representative_a, uint64_t const timestamp_a)
{
	...
	if (node.ledger.weight (representative_a) > node.minimum_principal_weight ())
	{
		...
		if (existing != inactive_by_hash.end ())
		{
			...
			{
				auto is_new (false);
				inactive_by_hash.modify (existing, [representative_a, timestamp_a, &is_new] (vxldollar::inactive_cache_information & info) {
				...
				if (is_new)
				{
					...
					if (status != old_status)
					{
						...
						if (existing != inactive_by_hash.end ())
						{
							inactive_by_hash.modify (existing, [status] (vxldollar::inactive_cache_information & info) {
								info.status = status;
							});
						}
					}
		...
		else
		{
			...
			inactive_by_arrival.emplace (vxldollar::inactive_cache_information{ std::chrono::steady_clock::now (), hash_a, representative_a, timestamp_a, default_status });
...
void vxldollar::active_transactions::trigger_inactive_votes_cache_election (std::shared_ptr<vxldollar::block> const & block_a)
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
...
vxldollar::inactive_cache_information vxldollar::active_transactions::find_inactive_votes_cache (vxldollar::block_hash const & hash_a)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	return find_inactive_votes_cache_impl (hash_a);
}

vxldollar::inactive_cache_information vxldollar::active_transactions::find_inactive_votes_cache_impl (vxldollar::block_hash const & hash_a)
{
	auto & inactive_by_hash (inactive_votes_cache.get<tag_hash> ());
	auto existing (inactive_by_hash.find (hash_a));
	if (existing != inactive_by_hash.end ())
	...
	{
		return vxldollar::inactive_cache_information{};
	}
}

void vxldollar::active_transactions::erase_inactive_votes_cache (vxldollar::block_hash const & hash_a)
{
	inactive_votes_cache.get<tag_hash> ().erase (hash_a);
}

vxldollar::inactive_cache_status vxldollar::active_transactions::inactive_votes_bootstrap_check (vxldollar::unique_lock<vxldollar::mutex> & lock_a, vxldollar::account const & voter_a, vxldollar::block_hash const & hash_a, vxldollar::inactive_cache_status const & previously_a)
{
	debug_assert (lock_a.owns_lock ());
	lock_a.unlock ();
...
vxldollar::inactive_cache_status vxldollar::active_transactions::inactive_votes_bootstrap_check (vxldollar::unique_lock<vxldollar::mutex> & lock_a, std::vector<std::pair<vxldollar::account, uint64_t>> const & voters_a, vxldollar::block_hash const & hash_a, vxldollar::inactive_cache_status const & previously_a)
{
...
vxldollar::inactive_cache_status vxldollar::active_transactions::inactive_votes_bootstrap_check_impl (vxldollar::unique_lock<vxldollar::mutex> & lock_a, vxldollar::uint128_t const & tally_a, std::size_t voters_size_a, vxldollar::block_hash const & hash_a, vxldollar::inactive_cache_status const & previously_a)
{
	debug_assert (!lock_a.owns_lock ());
	vxldollar::inactive_cache_status status (previously_a);
...
bool vxldollar::purge_singleton_inactive_votes_cache_pool_memory ()
{
	return boost::singleton_pool<boost::fast_pool_allocator_tag, sizeof (vxldollar::active_transactions::ordered_cache::node_type)>::purge_memory ();
}

std::size_t vxldollar::active_transactions::election_winner_details_size ()
{
	vxldollar::lock_guard<vxldollar::mutex> guard (election_winner_details_mutex);
	return election_winner_details.size ();
}

vxldollar::cementable_account::cementable_account (vxldollar::account const & account_a, std::size_t blocks_uncemented_a) :
	account (account_a), blocks_uncemented (blocks_uncemented_a)
{
}

vxldollar::expired_optimistic_election_info::expired_optimistic_election_info (std::chrono::steady_clock::time_point expired_time_a, vxldollar::account account_a) :
	expired_time (expired_time_a),
	account (account_a)
{
}

bool vxldollar::frontiers_confirmation_info::can_start_elections () const
{
	return max_elections > 0;
}

std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (active_transactions & active_transactions, std::string const & name)
{
	...
	{
		vxldollar::lock_guard<vxldollar::mutex> guard (active_transactions.mutex);
	...
	composite->add_component (std::make_unique<container_info_leaf> (container_info{ "priority_wallet_cementable_frontiers", active_transactions.priority_wallet_cementable_frontiers_size (), sizeof (vxldollar::cementable_account) }));
	composite->add_component (std::make_unique<container_info_leaf> (container_info{ "priority_cementable_frontiers", active_transactions.priority_cementable_frontiers_size (), sizeof (vxldollar::cementable_account) }));
	...
	composite->add_component (std::make_unique<container_info_leaf> (container_info{ "inactive_votes_cache", active_transactions.inactive_votes_cache_size (), sizeof (vxldollar::gap_information) }));

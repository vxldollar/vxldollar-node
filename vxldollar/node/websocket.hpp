...
#include <vxldollar/node/websocket_stream.hpp>
...
namespace websocket
{
	...
	{
	public:
		message (vxldollar::websocket::topic topic_a) :
			topic (topic_a)
		{
		}
		message (vxldollar::websocket::topic topic_a, boost::property_tree::ptree & tree_a) :
			topic (topic_a), contents (tree_a)
		{
		}

		std::string to_string () const;
		vxldollar::websocket::topic topic;
	...
	{
	public:
		message block_confirmed (std::shared_ptr<vxldollar::block> const & block_a, vxldollar::account const & account_a, vxldollar::amount const & amount_a, std::string subtype, bool include_block, vxldollar::election_status const & election_status_a, std::vector<vxldollar::vote_with_weight_info> const & election_votes_a, vxldollar::websocket::confirmation_options const & options_a);
		message stopped_election (vxldollar::block_hash const & hash_a);
		message vote_received (std::shared_ptr<vxldollar::vote> const & vote_a, vxldollar::vote_code code_a);
		message work_generation (vxldollar::work_version const version_a, vxldollar::block_hash const & root_a, uint64_t const work_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool const completed_a = true, bool const cancelled_a = false);
		message work_cancelled (vxldollar::work_version const version_a, vxldollar::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		message work_failed (vxldollar::work_version const version_a, vxldollar::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		...
		message telemetry_received (vxldollar::telemetry_data const &, vxldollar::endpoint const &);
		message new_block_arrived (vxldollar::block const & block_a);
	...
	{
	public:
		confirmation_options (vxldollar::wallets & wallets_a);
		confirmation_options (boost::property_tree::ptree const & options_a, vxldollar::wallets & wallets_a, vxldollar::logger_mt & logger_a);
	...
	private:
		...
		boost::optional<vxldollar::logger_mt &> logger;
	...
	{
	public:
		vote_options (boost::property_tree::ptree const & options_a, vxldollar::logger_mt & logger_a);
	...
	public:
		...
		explicit session (vxldollar::websocket::listener & listener_a, socket_type socket_a, boost::asio::ssl::context & ctx_a);
#endif
		/** Constructor that takes ownership over \p socket_a */
		explicit session (vxldollar::websocket::listener & listener_a, socket_type socket_a);
		...
		void write (vxldollar::websocket::message message_a);

	private:
		/** The owning listener */
		vxldollar::websocket::listener & ws_listener;
		/** Websocket stream, supporting both plain and tls connections */
		vxldollar::websocket::stream ws;
		...
		vxldollar::mutex subscriptions_mutex;
	...
	{
	public:
		listener (std::shared_ptr<vxldollar::tls_config> const & tls_config_a, vxldollar::logger_mt & logger_a, vxldollar::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a);
		...
		void broadcast_confirmation (std::shared_ptr<vxldollar::block> const & block_a, vxldollar::account const & account_a, vxldollar::amount const & amount_a, std::string const & subtype, vxldollar::election_status const & election_status_a, std::vector<vxldollar::vote_with_weight_info> const & election_votes_a);

		/** Broadcast \p message to all session subscribing to the message topic. */
		void broadcast (vxldollar::websocket::message message_a);

		vxldollar::logger_mt & get_logger () const
		{
			return logger;
		}
		...
		vxldollar::wallets & get_wallets () const
		{
			return wallets;
		}
		 ...
		 */
		bool any_subscriber (vxldollar::websocket::topic const & topic_a) const
		{
			return subscriber_count (topic_a) > 0;
		}
		/** Getter for subscriber count of a specific topic*/
		std::size_t subscriber_count (vxldollar::websocket::topic const & topic_a) const
		{
	...
	private:
		...
		friend vxldollar::websocket::session;

		/** Adds to subscription count of a specific topic*/
		void increase_subscriber_count (vxldollar::websocket::topic const & topic_a);
		/** Removes from subscription count of a specific topic*/
		void decrease_subscriber_count (vxldollar::websocket::topic const & topic_a);
		...
		socket_type socket;
		vxldollar::mutex sessions_mutex;

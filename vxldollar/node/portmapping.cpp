...
std::string vxldollar::mapping_protocol::to_string ()
{
	std::stringstream ss;
...
std::string vxldollar::port_mapping::get_config_port (std::string const & node_port_a)
{
	return node.config.external_port != 0 ? std::to_string (node.config.external_port) : node_port_a;
}

std::string vxldollar::port_mapping::to_string ()
{
	std::stringstream ss;
...
void vxldollar::port_mapping::refresh_devices ()
{
...
vxldollar::endpoint vxldollar::port_mapping::external_address ()
{
	vxldollar::endpoint result_l (boost::asio::ip::address_v6{}, 0);
	vxldollar::lock_guard<vxldollar::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
	{
		if (protocol.external_port != 0)
		{
			result_l = vxldollar::endpoint (protocol.external_address, protocol.external_port);
		}
	}
	return result_l;
}

void vxldollar::port_mapping::refresh_mapping ()
{
	...
	if (on)
	{
		...
		for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
		{
			auto upnp_description = std::string ("Vxldollar Node (") + node.network_params.network.get_current_network_as_string () + ")";
...
bool vxldollar::port_mapping::check_lost_or_old_mapping ()
{
...
void vxldollar::port_mapping::check_mapping_loop ()
{
...
void vxldollar::port_mapping::stop ()
{
	on = false;
	vxldollar::lock_guard<vxldollar::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
...
std::string vxldollar::upnp_state::to_string ()
{
	std::stringstream ss;
...
vxldollar::upnp_state::~upnp_state ()
{
	if (devices)
	{
...
vxldollar::upnp_state & vxldollar::upnp_state::operator= (vxldollar::upnp_state && other_a)
{
	if (this == &other_a)
	{
		return *this;

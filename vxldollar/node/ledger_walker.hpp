...
{
public:
	using should_visit_callback = std::function<bool (std::shared_ptr<vxldollar::block> const &)>;
	using visitor_callback = std::function<void (std::shared_ptr<vxldollar::block> const &)>;

	explicit ledger_walker (vxldollar::ledger const & ledger_a);
	...
	void walk_backward (vxldollar::block_hash const & start_block_hash_a, should_visit_callback const & should_visit_callback_a, visitor_callback const & visitor_callback_a);
	...
	void walk (vxldollar::block_hash const & end_block_hash_a, should_visit_callback const & should_visit_callback_a, visitor_callback const & visitor_callback_a);
	...
	void walk_backward (vxldollar::block_hash const & start_block_hash_a, visitor_callback const & visitor_callback_a);
	void walk (vxldollar::block_hash const & end_block_hash_a, visitor_callback const & visitor_callback_a);
...
private:
	...
	std::unordered_set<vxldollar::block_hash> walked_blocks;
	std::optional<dht::DiskHash<bool>> walked_blocks_disk;
	std::stack<vxldollar::block_hash> blocks_to_walk;

	void enqueue_block (vxldollar::block_hash block_hash_a);
	void enqueue_block (std::shared_ptr<vxldollar::block> const & block_a);
	bool add_to_walked_blocks (vxldollar::block_hash const & block_hash_a);
	bool add_to_walked_blocks_disk (vxldollar::block_hash const & block_hash_a);
	void clear_queue ();
	std::shared_ptr<vxldollar::block> dequeue_block (vxldollar::transaction const & transaction_a);

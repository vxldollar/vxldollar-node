...
std::string const default_live_peer_network = vxldollar::get_env_or_default ("VXLDOLLAR_DEFAULT_PEER", "live.vxldollar.com");//
std::string const default_beta_peer_network = vxldollar::get_env_or_default ("VXLDOLLAR_DEFAULT_PEER", "beta.vxldollar.com");
std::string const default_test_peer_network = vxldollar::get_env_or_default ("VXLDOLLAR_DEFAULT_PEER", "test.vxldollar.com");
}

vxldollar::node_config::node_config (vxldollar::network_params & network_params) :
	node_config (std::nullopt, vxldollar::logging (), network_params)
{
}

vxldollar::node_config::node_config (const std::optional<uint16_t> & peering_port_a, vxldollar::logging const & logging_a, vxldollar::network_params & network_params) :
	...
	external_address{ boost::asio::ip::address_v6{}.to_string () }
{
	...
	switch (network_params.network.network ())
	{
		...
		case vxldollar::networks::vxldollar_beta_network:
		{
			...
			vxldollar::account offline_representative;
			release_assert (!offline_representative.decode_account ("vxld_1beta5mgf3rn4nyrheqnzbrsbcytcg6kfsootzocfb3gmdpdsd45x91zm4xx"));
		...
		case vxldollar::networks::vxldollar_live_network:
			...
			preconfigured_representatives.emplace_back ("6196C33418B5E39D3E8940D968E252E0C04EBB23AF6E03EDE3871F1A4A1BF969");//vxld_1repret3jfh5mnzaki8sf5j77r81btxk9dug1hpy93rz5b73qydbxw94oih6
			//preconfigured_representatives.emplace_back ("6196C33A5AF1CAC771B8E277BBFBD2CB75A366681F8F330F80B8B38874044357");//vxld_1reprex7owgcrxrujrmqqhxx7kuonfm8i9wh8e9r3g7mj3t1aitqbthmttki
			//preconfigured_representatives.emplace_back ("6196C339D568AB5A3736BA30E939DCC1E010F5BFA5A5B2A343FA3F965C856BB2");//vxld_1reprewxct7ddaumfgjix6wxsih145tuzbf7pcjn9yjzksgactxktpt7bmwf
			break;
...
vxldollar::error vxldollar::node_config::serialize_toml (vxldollar::tomlconfig & toml) const
{
	...
	vxldollar::tomlconfig experimental_l;
	...
	vxldollar::tomlconfig callback_l;
...
vxldollar::error vxldollar::node_config::deserialize_toml (vxldollar::tomlconfig & toml)
{
	try
	{
		...
		if (frontiers_confirmation == vxldollar::frontiers_confirmation_mode::invalid)
		{
...
std::string vxldollar::node_config::serialize_frontiers_confirmation (vxldollar::frontiers_confirmation_mode mode_a) const
{
	switch (mode_a)
	{
		case vxldollar::frontiers_confirmation_mode::always:
			return "always";
		case vxldollar::frontiers_confirmation_mode::automatic:
			return "auto";
		case vxldollar::frontiers_confirmation_mode::disabled:
			return "disabled";
		default:
			return "auto";
	}
...
vxldollar::frontiers_confirmation_mode vxldollar::node_config::deserialize_frontiers_confirmation (std::string const & string_a)
{
	if (string_a == "always")
	{
		return vxldollar::frontiers_confirmation_mode::always;
	}
	else if (string_a == "auto")
	{
		return vxldollar::frontiers_confirmation_mode::automatic;
	}
	else if (string_a == "disabled")
	{
		return vxldollar::frontiers_confirmation_mode::disabled;
	}
	else
	{
		return vxldollar::frontiers_confirmation_mode::invalid;
	}
}

void vxldollar::node_config::deserialize_address (std::string const & entry_a, std::vector<std::pair<std::string, uint16_t>> & container_a) const
{
...
vxldollar::account vxldollar::node_config::random_representative () const
{
	debug_assert (!preconfigured_representatives.empty ());
	std::size_t index (vxldollar::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (preconfigured_representatives.size () - 1)));

...
vxldollar::ipc::access_permission from_string (std::string permission)
{
	if (permission == "unrestricted")
		return vxldollar::ipc::access_permission::unrestricted;
	if (permission == "api_account_weight")
		return vxldollar::ipc::access_permission::api_account_weight;
	if (permission == "api_service_register")
		return vxldollar::ipc::access_permission::api_service_register;
	if (permission == "api_service_stop")
		return vxldollar::ipc::access_permission::api_service_stop;
	if (permission == "api_topic_service_stop")
		return vxldollar::ipc::access_permission::api_topic_service_stop;
	if (permission == "api_topic_confirmation")
		return vxldollar::ipc::access_permission::api_topic_confirmation;
	if (permission == "account_query")
		return vxldollar::ipc::access_permission::account_query;
	if (permission == "epoch_upgrade")
		return vxldollar::ipc::access_permission::epoch_upgrade;
	if (permission == "service")
		return vxldollar::ipc::access_permission::service;
	if (permission == "wallet")
		return vxldollar::ipc::access_permission::wallet;
	if (permission == "wallet_read")
		return vxldollar::ipc::access_permission::wallet_read;
	if (permission == "wallet_write")
		return vxldollar::ipc::access_permission::wallet_write;
	if (permission == "wallet_seed_change")
		return vxldollar::ipc::access_permission::wallet_seed_change;

	return vxldollar::ipc::access_permission::invalid;
}
}

void vxldollar::ipc::access::set_effective_permissions (vxldollar::ipc::access_subject & subject_a, std::shared_ptr<cpptoml::table> const & config_subject_a)
{
	...
	for (auto const & permission : allow_strings_l)
	{
		if (!permission.empty ())
		{
			...
			if (permission_enum != vxldollar::ipc::access_permission::invalid)
			{
...
void vxldollar::ipc::access::clear ()
{
	users.clear ();
	roles.clear ();
	...
	default_user.permissions.insert (vxldollar::ipc::access_permission::api_account_weight);
}

vxldollar::error vxldollar::ipc::access::deserialize_toml (vxldollar::tomlconfig & toml)
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	clear ();

	vxldollar::error error;
	if (toml.has_key ("role"))
	{
		auto get_role = [this] (std::shared_ptr<cpptoml::table> const & role_a) {
			vxldollar::ipc::access_role role;
	...
	if (!error && toml.has_key ("user"))
	{
		auto get_user = [this, &error] (std::shared_ptr<cpptoml::table> const & user_a) {
			vxldollar::ipc::access_user user;
...
bool vxldollar::ipc::access::has_access (std::string const & credentials_a, vxldollar::ipc::access_permission permssion_a) const
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	...
	if (user != users.end ())
	{
		...
		if (!permitted)
		{
			permitted = user->second.permissions.find (vxldollar::ipc::access_permission::unrestricted) != user->second.permissions.end ();
		}
	}
	return permitted;
}

bool vxldollar::ipc::access::has_access_to_all (std::string const & credentials_a, std::initializer_list<vxldollar::ipc::access_permission> permissions_a) const
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
bool vxldollar::ipc::access::has_access_to_oneof (std::string const & credentials_a, std::initializer_list<vxldollar::ipc::access_permission> permissions_a) const
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
void vxldollar::ipc::access_subject::clear ()
{
	permissions.clear ();
}

void vxldollar::ipc::access_user::clear ()
{
...
namespace ipc
{
	vxldollar::error read_access_config_toml (boost::filesystem::path const & data_path_a, vxldollar::ipc::access & config_a)
	{
		vxldollar::error error;
		auto toml_config_path = vxldollar::get_access_toml_config_path (data_path_a);

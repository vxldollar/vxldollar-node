...
public:
	...
	std::vector<vxldollar::account> voters;
...
{
public:
	explicit gap_cache (vxldollar::node &);
	void add (vxldollar::block_hash const &, std::chrono::steady_clock::time_point = std::chrono::steady_clock::now ());
	void erase (vxldollar::block_hash const & hash_a);
	void vote (std::shared_ptr<vxldollar::vote> const &);
	bool bootstrap_check (std::vector<vxldollar::account> const &, vxldollar::block_hash const &);
	void bootstrap_start (vxldollar::block_hash const & hash_a);
	vxldollar::uint128_t bootstrap_threshold ();
	std::size_t size ();
	// clang-format off
	class tag_arrival {};
	class tag_hash {};
	using ordered_gaps = boost::multi_index_container<vxldollar::gap_information,
	boost::multi_index::indexed_by<
		...
		boost::multi_index::hashed_unique<boost::multi_index::tag<tag_hash>,
			boost::multi_index::member<gap_information, vxldollar::block_hash, &gap_information::hash>>>>;
	ordered_gaps blocks;
	// clang-format on
	std::size_t const max = 256;
	vxldollar::mutex mutex{ mutex_identifier (mutexes::gap_cache) };
	vxldollar::node & node;
};

...
void vxldollar::vote_spacing::trim ()
{
	recent.get<tag_time> ().erase (recent.get<tag_time> ().begin (), recent.get<tag_time> ().upper_bound (std::chrono::steady_clock::now () - delay));
}

bool vxldollar::vote_spacing::votable (vxldollar::root const & root_a, vxldollar::block_hash const & hash_a) const
{
	bool result = true;
...
void vxldollar::vote_spacing::flag (vxldollar::root const & root_a, vxldollar::block_hash const & hash_a)
{
	trim ();
...
std::size_t vxldollar::vote_spacing::size () const
{
	return recent.size ();
}

bool vxldollar::local_vote_history::consistency_check (vxldollar::root const & root_a) const
{
...
void vxldollar::local_vote_history::add (vxldollar::root const & root_a, vxldollar::block_hash const & hash_a, std::shared_ptr<vxldollar::vote> const & vote_a)
{
...
void vxldollar::local_vote_history::erase (vxldollar::root const & root_a)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	auto & history_by_root (history.get<tag_root> ());
	auto range (history_by_root.equal_range (root_a));
...
std::vector<std::shared_ptr<vxldollar::vote>> vxldollar::local_vote_history::votes (vxldollar::root const & root_a) const
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	std::vector<std::shared_ptr<vxldollar::vote>> result;
...
std::vector<std::shared_ptr<vxldollar::vote>> vxldollar::local_vote_history::votes (vxldollar::root const & root_a, vxldollar::block_hash const & hash_a, bool const is_final_a) const
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	std::vector<std::shared_ptr<vxldollar::vote>> result;
	auto range (history.get<tag_root> ().equal_range (root_a));
	// clang-format off
	vxldollar::transform_if (range.first, range.second, std::back_inserter (result),
...
bool vxldollar::local_vote_history::exists (vxldollar::root const & root_a) const
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	return history.get<tag_root> ().find (root_a) != history.get<tag_root> ().end ();
}

void vxldollar::local_vote_history::clean ()
{
...
std::size_t vxldollar::local_vote_history::size () const
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	return history.size ();
}

std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (vxldollar::local_vote_history & history, std::string const & name)
{
...
vxldollar::vote_generator::vote_generator (vxldollar::node_config const & config_a, vxldollar::ledger & ledger_a, vxldollar::wallets & wallets_a, vxldollar::vote_processor & vote_processor_a, vxldollar::local_vote_history & history_a, vxldollar::network & network_a, vxldollar::stat & stats_a, bool is_final_a) :
	config (config_a),
	ledger (ledger_a),
	wallets (wallets_a),
...
void vxldollar::vote_generator::add (vxldollar::root const & root_a, vxldollar::block_hash const & hash_a)
{
	...
	else
	{
		...
		if (should_vote)
		{
			...
			if (candidates.size () >= vxldollar::network::confirm_ack_hashes_max)
			{
				lock.unlock ();
...
void vxldollar::vote_generator::stop ()
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	stopped = true;
...
std::size_t vxldollar::vote_generator::generate (std::vector<std::shared_ptr<vxldollar::block>> const & blocks_a, std::shared_ptr<vxldollar::transport::channel> const & channel_a)
{
	request_t::first_type req_candidates;
	{
		...
		vxldollar::transform_if (blocks_a.begin (), blocks_a.end (), std::back_inserter (req_candidates), dependents_confirmed, as_candidate);
	}
	...
	while (requests.size () > max_requests)
	{
		...
		requests.pop_front ();
		stats.inc (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_replies_discarded);
	}
	return result;
}

void vxldollar::vote_generator::set_reply_action (std::function<void (std::shared_ptr<vxldollar::vote> const &, std::shared_ptr<vxldollar::transport::channel> const &)> action_a)
{
	release_assert (!reply_action);
	reply_action = action_a;
}

void vxldollar::vote_generator::broadcast (vxldollar::unique_lock<vxldollar::mutex> & lock_a)
{
	debug_assert (lock_a.owns_lock ());
	std::unordered_set<std::shared_ptr<vxldollar::vote>> cached_sent;
	std::vector<vxldollar::block_hash> hashes;
	std::vector<vxldollar::root> roots;
	hashes.reserve (vxldollar::network::confirm_ack_hashes_max);
	roots.reserve (vxldollar::network::confirm_ack_hashes_max);
	while (!candidates.empty () && hashes.size () < vxldollar::network::confirm_ack_hashes_max)
	{
		...
		if (cached_votes.empty () && std::find (roots.begin (), roots.end (), root) == roots.end ())
		{
			...
			{
				stats.inc (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_spacing);
			}
		}
	...
	if (!hashes.empty ())
	{
		...
		vote (hashes, roots, [this] (auto const & vote_a) {
			...
			this->stats.inc (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_broadcasts);
		});
		lock_a.lock ();
	}
}

void vxldollar::vote_generator::reply (vxldollar::unique_lock<vxldollar::mutex> & lock_a, request_t && request_a)
{
	lock_a.unlock ();
	std::unordered_set<std::shared_ptr<vxldollar::vote>> cached_sent;
	...
	while (i != n && !stopped)
	{
		...
		hashes.reserve (vxldollar::network::confirm_ack_hashes_max);
		roots.reserve (vxldollar::network::confirm_ack_hashes_max);
		for (; i != n && hashes.size () < vxldollar::network::confirm_ack_hashes_max; ++i)
		{
			...
			for (auto const & cached_vote : cached_votes)
			{
				if (cached_sent.insert (cached_vote).second)
				{
					stats.add (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_cached_late_hashes, stat::dir::in, cached_vote->blocks.size ());
					stats.inc (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_cached_late_votes, stat::dir::in);
		...
		if (!hashes.empty ())
		{
			stats.add (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_generated_hashes, stat::dir::in, hashes.size ());
			vote (hashes, roots, [this, &channel = request_a.second] (std::shared_ptr<vxldollar::vote> const & vote_a) {
				this->reply_action (vote_a, channel);
				this->stats.inc (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_generated_votes, stat::dir::in);
			});
		}
	}
	stats.inc (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_replies);
	lock_a.lock ();
}

void vxldollar::vote_generator::vote (std::vector<vxldollar::block_hash> const & hashes_a, std::vector<vxldollar::root> const & roots_a, std::function<void (std::shared_ptr<vxldollar::vote> const &)> const & action_a)
{
	debug_assert (hashes_a.size () == roots_a.size ());
	std::vector<std::shared_ptr<vxldollar::vote>> votes_l;
	wallets.foreach_representative ([this, &hashes_a, &votes_l] (vxldollar::public_key const & pub_a, vxldollar::raw_key const & prv_a) {
		auto timestamp = this->is_final ? vxldollar::vote::timestamp_max : vxldollar::milliseconds_since_epoch ();
		uint8_t duration = this->is_final ? vxldollar::vote::duration_max : /*8192ms*/ 0x9;
		votes_l.emplace_back (std::make_shared<vxldollar::vote> (pub_a, prv_a, timestamp, duration, hashes_a));
	});
...
void vxldollar::vote_generator::broadcast_action (std::shared_ptr<vxldollar::vote> const & vote_a) const
{
	network.flood_vote_pr (vote_a);
	network.flood_vote (vote_a, 2.0f);
	vote_processor.vote (vote_a, std::make_shared<vxldollar::transport::channel_loopback> (network.node));
}

void vxldollar::vote_generator::run ()
{
	vxldollar::thread_role::set (vxldollar::thread_role::name::voting);
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	started = true;
	lock.unlock ();
	condition.notify_all ();
	...
	while (!stopped)
	{
		...
		{
			condition.wait_for (lock, config.vote_generator_delay, [this] () { return this->candidates.size () >= vxldollar::network::confirm_ack_hashes_max; });
			if (candidates.size () >= config.vote_generator_threshold && candidates.size () < vxldollar::network::confirm_ack_hashes_max)
			{
				condition.wait_for (lock, config.vote_generator_delay, [this] () { return this->candidates.size () >= vxldollar::network::confirm_ack_hashes_max; });
			}
			if (!candidates.empty ())
...
vxldollar::vote_generator_session::vote_generator_session (vxldollar::vote_generator & vote_generator_a) :
	generator (vote_generator_a)
{
}

void vxldollar::vote_generator_session::add (vxldollar::root const & root_a, vxldollar::block_hash const & hash_a)
{
	debug_assert (vxldollar::thread_role::get () == vxldollar::thread_role::name::request_loop);
	items.emplace_back (root_a, hash_a);
}

void vxldollar::vote_generator_session::flush ()
{
	debug_assert (vxldollar::thread_role::get () == vxldollar::thread_role::name::request_loop);
...
std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (vxldollar::vote_generator & vote_generator, std::string const & name)
{
	std::size_t candidates_count = 0;
	std::size_t requests_count = 0;
	{
		vxldollar::lock_guard<vxldollar::mutex> guard (vote_generator.mutex);

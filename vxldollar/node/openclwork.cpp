...
bool vxldollar::opencl_loaded{ true };
#else
bool vxldollar::opencl_loaded{ false };
#endif
...
enum IV_Derived {
    vxldollar_xor_iv0 = 0x6a09e667f2bdc900UL,  // iv1 ^ 0x1010000 ^ outlen
    vxldollar_xor_iv4 = 0x510e527fade682f9UL,  // iv4 ^ inbytes
    vxldollar_xor_iv6 = 0xe07c265404be4294UL,  // iv6 ^ ~0
};
...
{
    ulong2 vv[8] = {
        {vxldollar_xor_iv0, iv1}, {iv2, iv3},          {iv4, iv5},
        {iv6, iv7},          {iv0, iv1},          {iv2, iv3},
        {vxldollar_xor_iv4, iv5}, {vxldollar_xor_iv6, iv7},
    };
    ...
    return vxldollar_xor_iv0 ^ vv[0].s0 ^ vv[4].s0;
}
#undef G32
#undef G2v
#undef G2v_split
...
__kernel void vxldollar_work(__constant ulong *attempt,
...
vxldollar::opencl_environment::opencl_environment (bool & error_a)
{
	if (vxldollar::opencl_loaded)
	{
		...
		{
			vxldollar::opencl_platform platform;
			platform.platform = *i;
...
void vxldollar::opencl_environment::dump (std::ostream & stream)
{
	if (vxldollar::opencl_loaded)
	{
		auto index (0);
...
vxldollar::opencl_work::opencl_work (bool & error_a, vxldollar::opencl_config const & config_a, vxldollar::opencl_environment & environment_a, vxldollar::logger_mt & logger_a, vxldollar::work_thresholds & work) :
	config (config_a),
	context (0),
	attempt_buffer (0),
	result_buffer (0),
	...
	work{ work }
{
	...
	if (!error_a)
	{
		...
		if (!error_a)
		{
			vxldollar::random_pool::generate_block (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
			...
			if (!error_a)
			{
				...
				if (!error_a)
				{
					...
					if (!error_a)
					{
						...
						{
							cl_int item_error (0);
							std::size_t item_size (sizeof (vxldollar::uint256_union));
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										if (!error_a)
										{
											...
											kernel = clCreateKernel (program, "vxldollar_work", &kernel_error);
...
vxldollar::opencl_work::~opencl_work ()
{
	if (kernel != 0)
	{
...
boost::optional<uint64_t> vxldollar::opencl_work::generate_work (vxldollar::work_version const version_a, vxldollar::root const & root_a, uint64_t const difficulty_a)
{
	std::atomic<int> ticket_l{ 0 };
	return generate_work (version_a, root_a, difficulty_a, ticket_l);
}

boost::optional<uint64_t> vxldollar::opencl_work::generate_work (vxldollar::work_version const version_a, vxldollar::root const & root_a, uint64_t const difficulty_a, std::atomic<int> & ticket_a)
{
	...
	while (work.difficulty (version_a, root_a, result) < difficulty_a && !error && ticket_a == ticket_l)
	{
		...
		{
			cl_int write_error2 = clEnqueueWriteBuffer (queue, item_buffer, false, 0, sizeof (vxldollar::root), root_a.bytes.data (), 0, nullptr, nullptr);
...
std::unique_ptr<vxldollar::opencl_work> vxldollar::opencl_work::create (bool create_a, vxldollar::opencl_config const & config_a, vxldollar::logger_mt & logger_a, vxldollar::work_thresholds & work)
{
	std::unique_ptr<vxldollar::opencl_work> result;
	if (create_a)
	{
		auto error (false);
		...
		if (!error)
		{
			result.reset (new vxldollar::opencl_work (error, config_a, environment, logger_a, work));
			if (error)
			{
				result.reset ();
			}

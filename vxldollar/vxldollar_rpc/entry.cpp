...
void run (boost::filesystem::path const & data_path, std::vector<std::string> const & config_overrides)
{
	...
	vxldollar::rpc_config rpc_config{ network_params.network };
	auto error = vxldollar::read_rpc_config_toml (data_path, rpc_config, config_overrides);
	if (!error)
	{
		...
		try
		{
			vxldollar::ipc_rpc_processor ipc_rpc_processor (io_ctx, rpc_config);
			auto rpc = vxldollar::get_rpc (io_ctx, rpc_config, ipc_rpc_processor);
			rpc->start ();
			...
			runner = std::make_unique<vxldollar::thread_runner> (io_ctx, rpc_config.rpc_process.io_threads);
			runner->join ();
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("config", boost::program_options::value<std::vector<vxldollar::config_key_value_pair>>()->multitoken(), "Pass RPC configuration values. This takes precedence over any values in the configuration file. This option can be repeated multiple times.")

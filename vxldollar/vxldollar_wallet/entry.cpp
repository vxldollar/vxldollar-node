...
#include <vxldollar/lib/walletconfig.hpp>
#include <vxldollar/vxldollar_wallet/icon.hpp>
...
{
	QMessageBox message (QMessageBox::Critical, "Error starting Vxldollar", message_a.c_str ());
...
{
	QMessageBox message (QMessageBox::NoIcon, "Help", "see <a href=\"https://docs.vxldollar.org/commands/command-line-interface/#launch-options\">launch options</a> ");
...
vxldollar::error write_wallet_config (vxldollar::wallet_config & config_a, boost::filesystem::path const & data_path_a)
{
	vxldollar::tomlconfig wallet_config_toml;
	auto wallet_path (vxldollar::get_qtwallet_toml_config_path (data_path_a));
...
vxldollar::error read_wallet_config (vxldollar::wallet_config & config_a, boost::filesystem::path const & data_path_a)
{
	vxldollar::tomlconfig wallet_config_toml;
	auto wallet_path (vxldollar::get_qtwallet_toml_config_path (data_path_a));
...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path, vxldollar::node_flags const & flags)
{
	int result (0);
	...
	vxldollar::wallet_config wallet_config;
	...
	if (!error)
	{
		...
		vxldollar::thread_runner runner (io_ctx, config.node.io_threads);

		std::shared_ptr<vxldollar::node> node;
		std::shared_ptr<vxldollar_qt::wallet> gui;
		vxldollar::set_application_icon (application);
		auto opencl (vxldollar::opencl_work::create (config.opencl_enable, config.opencl, logger, config.node.network_params.work));
		vxldollar::work_pool work{ config.node.network_params.network, config.node.work_threads, config.node.pow_sleep_interval, opencl ? [&opencl] (vxldollar::work_version const version_a, vxldollar::root const & root_a, uint64_t difficulty_a, std::atomic<int> &) {
		...
		node = std::make_shared<vxldollar::node> (io_ctx, data_path, config.node, work, flags);
		if (!node->init_error ())
		{
			...
			node->start ();
			vxldollar::ipc::ipc_server ipc (*node, config.rpc);
			...
			if (config.pow_server.enable)
			{
				...
				{
					splash->hide ();
					show_error (std::string ("vxldollar_pow_server is configured to start as a child process, however the file cannot be found at: ") + config.pow_server.pow_server_path);
					std::exit (1);
				}
			...
			if (config.rpc_enable)
			{
				if (!config.rpc.child_process.enable)
				{
					...
					error = vxldollar::read_rpc_config_toml (data_path, rpc_config, flags.rpc_config_overrides);
					if (error)
					{
						splash->hide ();
						show_error (error.get_message ());
					...
					rpc_handler = std::make_unique<vxldollar::inprocess_rpc_handler> (*node, ipc, config.rpc);
			...
			QObject::connect (&application, &QApplication::aboutToQuit, [&] () {
				...
				if (vxldollar_pow_server_process)
				{
					vxldollar_pow_server_process->terminate ();
				}
#endif
				runner.stop_event_processing ();
			});
			QApplication::postEvent (&processor, new vxldollar_qt::eventloop_event ([&] () {
				gui = std::make_shared<vxldollar_qt::wallet> (application, processor, *node, wallet, wallet_config.account);
				splash->close ();
				gui->start ();
...
int main (int argc, char * const * argv)
{
	...
	try
	{
		...
		description.add_options()
			...
			("config", boost::program_options::value<std::vector<vxldollar::config_key_value_pair>>()->multitoken(), "Pass configuration values. This takes precedence over any values in the node configuration file. This option can be repeated multiple times.")
			("rpcconfig", boost::program_options::value<std::vector<vxldollar::config_key_value_pair>>()->multitoken(), "Pass RPC configuration values. This takes precedence over any values in the RPC configuration file. This option can be repeated multiple times.");
		...
		if (network != vm.end ())
		{
			...
			if (err)
			{
				show_error (vxldollar::network_constants::active_network_err_msg);
				std::exit (1);
			}
		}
		...
		{
			config_overrides = vxldollar::config_overrides (configItr->second.as<std::vector<vxldollar::config_key_value_pair>> ());
		}
		...
		if (ec == vxldollar::error_cli::unknown_command)
		{
			...
			else
			{
				try
				{
					...
					{
						data_path = vxldollar::working_path ();
					}

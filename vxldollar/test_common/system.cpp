...
std::string vxldollar::error_system_messages::message (int ev) const
{
	switch (static_cast<vxldollar::error_system> (ev))
	{
		case vxldollar::error_system::generic:
			return "Unknown error";
		case vxldollar::error_system::deadline_expired:
...
std::shared_ptr<vxldollar::node> vxldollar::system::add_node (vxldollar::node_flags node_flags_a, vxldollar::transport::transport_type type_a)
{
	return add_node (vxldollar::node_config (vxldollar::get_available_port (), logging), node_flags_a, type_a);
}

/** Returns the node added. */
std::shared_ptr<vxldollar::node> vxldollar::system::add_node (vxldollar::node_config const & node_config_a, vxldollar::node_flags node_flags_a, vxldollar::transport::transport_type type_a)
{
	auto node (std::make_shared<vxldollar::node> (io_ctx, vxldollar::unique_path (), node_config_a, work, node_flags_a, node_sequence++));
	for (auto i : initialization_blocks)
	{
		auto result = node->ledger.process (node->store.tx_begin_write (), *i);
		debug_assert (result.code == vxldollar::process_result::progress);
	}
	...
	if (nodes.size () > 1)
	{
		...
		for (auto i (begin), j (begin + 1), n (nodes.end ()); j != n; ++i, ++j)
		{
			...
			if (type_a == vxldollar::transport::transport_type::tcp)
			{
			...
			{
				// UDP connection
				auto channel (std::make_shared<vxldollar::transport::channel_udp> ((*j)->network.udp_channels, (*i)->network.endpoint (), node1->network_params.network.protocol_version));
			...
			if (type_a == vxldollar::transport::transport_type::tcp && node_config_a.tcp_incoming_connections_max != 0 && !node_flags_a.disable_tcp_realtime)
			{
		...
		auto iterations1 (0);
		while (std::any_of (begin, nodes.end (), [] (std::shared_ptr<vxldollar::node> const & node_a) { return node_a->bootstrap_initiator.in_progress (); }))
		{
			poll ();
			++iterations1;
...
vxldollar::system::system (uint16_t count_a, vxldollar::transport::transport_type type_a, vxldollar::node_flags flags_a) :
	system ()
{
	nodes.reserve (count_a);
	for (uint16_t i (0); i < count_a; ++i)
	{
		vxldollar::node_config config (vxldollar::get_available_port (), logging);
...
void vxldollar::system::ledger_initialization_set (std::vector<vxldollar::keypair> const & reps, vxldollar::amount const & reserve)
{
	vxldollar::block_hash previous = vxldollar::dev::genesis->hash ();
	auto amount = (vxldollar::dev::constants.genesis_amount - reserve.number ()) / reps.size ();
	auto balance = vxldollar::dev::constants.genesis_amount;
	for (auto const & i : reps)
	{
		balance -= amount;
		vxldollar::state_block_builder builder;
		builder.account (vxldollar::dev::genesis_key.pub)
		.previous (previous)
...
std::shared_ptr<vxldollar::wallet> vxldollar::system::wallet (size_t index_a)
{
...
vxldollar::account vxldollar::system::account (vxldollar::transaction const & transaction_a, size_t index_a)
{
	...
	debug_assert (++keys == wallet_l->store.end ());
	return vxldollar::account (result);
}

uint64_t vxldollar::system::work_generate_limited (vxldollar::block_hash const & root_a, uint64_t min_a, uint64_t max_a)
{
	...
	} while (work.network_constants.work.difficulty (vxldollar::work_version::work_1, root_a, result) >= max_a);
	return result;
}
 ...
 */
std::unique_ptr<vxldollar::state_block> vxldollar::upgrade_epoch (vxldollar::work_pool & pool_a, vxldollar::ledger & ledger_a, vxldollar::epoch epoch_a)
{
	auto transaction (ledger_a.store.tx_begin_write ());
	auto dev_genesis_key = vxldollar::dev::genesis_key;
	...
	auto epoch = builder
				 ...
				 .work (*pool_a.generate (latest, pool_a.network_constants.work.threshold (vxldollar::work_version::work_1, vxldollar::block_details (epoch_a, false, false, true))))
				 .build (ec);

	bool error{ true };
	if (!ec && epoch)
	{
		error = ledger_a.process (transaction, *epoch).code != vxldollar::process_result::progress;
	}

	return !error ? std::move (epoch) : nullptr;
}

void vxldollar::blocks_confirm (vxldollar::node & node_a, std::vector<std::shared_ptr<vxldollar::block>> const & blocks_a, bool const forced_a)
{
...
std::unique_ptr<vxldollar::state_block> vxldollar::system::upgrade_genesis_epoch (vxldollar::node & node_a, vxldollar::epoch const epoch_a)
{
	return upgrade_epoch (work, node_a.ledger, epoch_a);
}

void vxldollar::system::deadline_set (std::chrono::duration<double, std::nano> const & delta_a)
{
	deadline = std::chrono::steady_clock::now () + delta_a * deadline_scaling_factor;
}

std::error_code vxldollar::system::poll (std::chrono::nanoseconds const & wait_time)
{
#if VXLDOLLAR_ASIO_HANDLER_TRACKING == 0
	...
	else if (count == 1 && timer.since_start ().count () >= VXLDOLLAR_ASIO_HANDLER_TRACKING)
	{
	...
	{
		ec = vxldollar::error_system::deadline_expired;
		stop ();
	}
	return ec;
}

std::error_code vxldollar::system::poll_until_true (std::chrono::nanoseconds deadline_a, std::function<bool ()> predicate_a)
{
	std::error_code ec;
 ...
 */
void vxldollar::system::delay_ms (std::chrono::milliseconds const & delay)
{
...
{
public:
	traffic_generator (uint32_t count_a, uint32_t wait_a, std::shared_ptr<vxldollar::node> const & node_a, vxldollar::system & system_a) :
		count (count_a),
		wait (wait_a),
		node (node_a),
		system (system_a)
...
void vxldollar::system::generate_usage_traffic (uint32_t count_a, uint32_t wait_a)
{
...
void vxldollar::system::generate_usage_traffic (uint32_t count_a, uint32_t wait_a, size_t index_a)
{
	debug_assert (nodes.size () > index_a);
...
void vxldollar::system::generate_rollback (vxldollar::node & node_a, std::vector<vxldollar::account> & accounts_a)
{
...
void vxldollar::system::generate_receive (vxldollar::node & node_a)
{
	std::shared_ptr<vxldollar::block> send_block;
	{
		auto transaction (node_a.store.tx_begin_read ());
		vxldollar::account random_account;
		random_pool::generate_block (random_account.bytes.data (), sizeof (random_account.bytes));
		auto i (node_a.store.pending.begin (transaction, vxldollar::pending_key (random_account, 0)));
		if (i != node_a.store.pending.end ())
		{
			vxldollar::pending_key const & send_hash (i->first);
			send_block = node_a.store.block.get (transaction, send_hash.hash);
		}
	}
	if (send_block != nullptr)
	{
		auto receive_error (wallet (0)->receive_sync (send_block, vxldollar::dev::genesis->account (), std::numeric_limits<vxldollar::uint128_t>::max ()));
		(void)receive_error;
	}
}

void vxldollar::system::generate_activity (vxldollar::node & node_a, std::vector<vxldollar::account> & accounts_a)
{
...
vxldollar::account vxldollar::system::get_random_account (std::vector<vxldollar::account> & accounts_a)
{
	debug_assert (std::numeric_limits<CryptoPP::word32>::max () > accounts_a.size ());
	auto index (random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (accounts_a.size () - 1)));
...
vxldollar::uint128_t vxldollar::system::get_random_amount (vxldollar::transaction const & transaction_a, vxldollar::node & node_a, vxldollar::account const & account_a)
{
	vxldollar::uint128_t balance (node_a.ledger.account_balance (transaction_a, account_a));
	vxldollar::uint128_union random_amount;
	vxldollar::random_pool::generate_block (random_amount.bytes.data (), sizeof (random_amount.bytes));
	return (((vxldollar::uint256_t{ random_amount.number () } * balance) / vxldollar::uint256_t{ std::numeric_limits<vxldollar::uint128_t>::max () }).convert_to<vxldollar::uint128_t> ());
}

void vxldollar::system::generate_send_existing (vxldollar::node & node_a, std::vector<vxldollar::account> & accounts_a)
{
	...
	vxldollar::account source;
	{
		...
		auto transaction (node_a.store.tx_begin_read ());
		vxldollar::store_iterator<vxldollar::account, vxldollar::account_info> entry (node_a.store.account.begin (transaction, account));
		if (entry == node_a.store.account.end ())
		{
			entry = node_a.store.account.begin (transaction);
		}
		debug_assert (entry != node_a.store.account.end ());
		destination = vxldollar::account (entry->first);
...
void vxldollar::system::generate_change_known (vxldollar::node & node_a, std::vector<vxldollar::account> & accounts_a)
{
	vxldollar::account source (get_random_account (accounts_a));
	if (!node_a.latest (source).is_zero ())
	{
		vxldollar::account destination (get_random_account (accounts_a));
...
void vxldollar::system::generate_change_unknown (vxldollar::node & node_a, std::vector<vxldollar::account> & accounts_a)
{
	vxldollar::account source (get_random_account (accounts_a));
	if (!node_a.latest (source).is_zero ())
	{
		vxldollar::keypair key;
		vxldollar::account destination (key.pub);
		auto change_error (wallet (0)->change_sync (source, destination));
		(void)change_error;
		debug_assert (!change_error);
	}
...
void vxldollar::system::generate_send_new (vxldollar::node & node_a, std::vector<vxldollar::account> & accounts_a)
{
...
void vxldollar::system::generate_mass_activity (uint32_t count_a, vxldollar::node & node_a)
{
	std::vector<vxldollar::account> accounts;
	auto dev_genesis_key = vxldollar::dev::genesis_key;
...
uint16_t vxldollar::get_available_port ()
{
...
void vxldollar::cleanup_dev_directories_on_exit ()
{
	// Makes sure everything is cleaned up
	vxldollar::logging::release_file_sink ();
	// Clean up tmp directories created by the tests. Since it's sometimes useful to
	// see log files after test failures, an environment variable is supported to
	// retain the files.
	if (std::getenv ("TEST_KEEP_TMPDIRS") == nullptr)

...
{
public:
	rpc_process_config (vxldollar::network_constants & network_constants);
...
{
public:
	explicit rpc_config (vxldollar::network_constants & network_constants);
	explicit rpc_config (vxldollar::network_constants & network_constants, uint16_t, bool);
	vxldollar::error serialize_toml (vxldollar::tomlconfig &) const;
	vxldollar::error deserialize_toml (vxldollar::tomlconfig &);

	vxldollar::rpc_process_config rpc_process;
	std::string address;
	...
	vxldollar::rpc_logging_config rpc_logging;
	/** Optional TLS config */
	std::shared_ptr<vxldollar::tls_config> tls_config;
};

vxldollar::error read_rpc_config_toml (boost::filesystem::path const & data_path_a, vxldollar::rpc_config & config_a, std::vector<std::string> const & config_overrides = std::vector<std::string> ());

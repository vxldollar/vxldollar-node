...
*/
char const * const VXLDOLLAR_VERSION_STRING = xstr (TAG_VERSION_STRING);
char const * const VXLDOLLAR_MAJOR_VERSION_STRING = xstr (MAJOR_VERSION_STRING);
char const * const VXLDOLLAR_MINOR_VERSION_STRING = xstr (MINOR_VERSION_STRING);
char const * const VXLDOLLAR_PATCH_VERSION_STRING = xstr (PATCH_VERSION_STRING);
char const * const VXLDOLLAR_PRE_RELEASE_VERSION_STRING = xstr (PRE_RELEASE_VERSION_STRING);
...
enum class networks : uint16_t
{
	...
	vxldollar_dev_network = 0x5241, // 'R', 'A'
	// Normal work parameters, secret beta genesis key, beta IP ports
	vxldollar_beta_network = 0x5242, // 'R', 'B'
	// Normal work parameters, secret live key, live IP ports
	vxldollar_live_network = 0x5243, // 'R', 'C'
	// Normal work parameters, secret test genesis key, test IP ports
	vxldollar_test_network = 0x5258, // 'R', 'X'
};
...
public:
	...
	work_thresholds operator= (vxldollar::work_thresholds const & other_a)
	{
		return other_a;
	}

	uint64_t threshold_entry (vxldollar::work_version const, vxldollar::block_type const) const;
	uint64_t threshold (vxldollar::block_details const &) const;
	// Ledger threshold
	uint64_t threshold (vxldollar::work_version const, vxldollar::block_details const) const;
	uint64_t threshold_base (vxldollar::work_version const) const;
	uint64_t value (vxldollar::root const & root_a, uint64_t work_a) const;
	...
	uint64_t difficulty (vxldollar::work_version const, vxldollar::root const &, uint64_t const) const;
	uint64_t difficulty (vxldollar::block const & block_a) const;
	bool validate_entry (vxldollar::work_version const, vxldollar::root const &, uint64_t const) const;
	bool validate_entry (vxldollar::block const &) const;

	/** Network work thresholds. Define these inline as constexpr when moving to cpp17. */
	static vxldollar::work_thresholds const publish_full;
	static vxldollar::work_thresholds const publish_beta;
	static vxldollar::work_thresholds const publish_dev;
	static vxldollar::work_thresholds const publish_test;
};

class network_constants
{
public:
	network_constants (vxldollar::work_thresholds & work, vxldollar::networks network_a) :
	...
	vxldollar::networks current_network{ vxldollar::network_constants::active_network };
	...
	vxldollar::networks network () const
	{
	 ...
	 */
	static void set_active_network (vxldollar::networks network_a)
	{
	...
	static bool set_active_network (std::string network_a)
	{
		...
		if (network_a == "live")
		{
			active_network = vxldollar::networks::vxldollar_live_network;
		}
		else if (network_a == "beta")
		{
			active_network = vxldollar::networks::vxldollar_beta_network;
		}
		else if (network_a == "dev")
		{
			active_network = vxldollar::networks::vxldollar_dev_network;
		}
		else if (network_a == "test")
		{
			active_network = vxldollar::networks::vxldollar_test_network;
		}
		else
		{
			error = true;
	...
	{
		return current_network == vxldollar::networks::vxldollar_live_network;
	}
	bool is_beta_network () const
	{
		return current_network == vxldollar::networks::vxldollar_beta_network;
	}
	bool is_dev_network () const
	{
		return current_network == vxldollar::networks::vxldollar_dev_network;
	}
	bool is_test_network () const
	{
		return current_network == vxldollar::networks::vxldollar_test_network;
	}

	/** Initial value is ACTIVE_NETWORK compile flag, but can be overridden by a CLI flag */
	static vxldollar::networks active_network;

...
{
	vxldollar::local_vote_history history{ vxldollar::dev::network_params.voting };
	...
	auto vote1a (std::make_shared<vxldollar::vote> ());
	...
	auto vote1b (std::make_shared<vxldollar::vote> ());
	...
	auto vote2 (std::make_shared<vxldollar::vote> ());
	...
	auto vote3 (std::make_shared<vxldollar::vote> ());
...
TEST (vote_generator, cache)
{
	...
	ASSERT_TRUE (std::any_of (votes[0]->begin (), votes[0]->end (), [hash = epoch1->hash ()] (vxldollar::block_hash const & hash_a) { return hash_a == hash; }));
}
...
TEST (vote_generator, multiple_representatives)
{
	...
	auto const amount = 100 * vxldollar::Gxrb_ratio;
	wallet.send_sync (vxldollar::dev::genesis_key.pub, key1.pub, amount);
	wallet.send_sync (vxldollar::dev::genesis_key.pub, key2.pub, amount);
	wallet.send_sync (vxldollar::dev::genesis_key.pub, key3.pub, amount);
	...
	auto hash = wallet.send_sync (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.pub, 1);
	...
	for (auto const & account : { key1.pub, key2.pub, key3.pub, vxldollar::dev::genesis_key.pub })
	{
		auto existing (std::find_if (votes.begin (), votes.end (), [&account] (std::shared_ptr<vxldollar::vote> const & vote_a) -> bool {
...
TEST (vote_generator, session)
{
	...
	auto node (system.nodes[0]);
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	vxldollar::vote_generator_session generator_session (node->active.generator);
	boost::thread thread ([node, &generator_session] () {
		vxldollar::thread_role::set (vxldollar::thread_role::name::request_loop);
		generator_session.add (vxldollar::dev::genesis->account (), vxldollar::dev::genesis->hash ());
		ASSERT_EQ (0, node->stats.count (vxldollar::stat::type::vote, vxldollar::stat::detail::vote_indeterminate));
		generator_session.flush ();
	});
	thread.join ();
	ASSERT_TIMELY (2s, 1 == node->stats.count (vxldollar::stat::type::vote, vxldollar::stat::detail::vote_indeterminate));
}

TEST (vote_spacing, basic)
{
	vxldollar::vote_spacing spacing{ std::chrono::milliseconds{ 100 } };
	vxldollar::root root1{ 1 };
	vxldollar::root root2{ 2 };
	vxldollar::block_hash hash3{ 3 };
	vxldollar::block_hash hash4{ 4 };
	vxldollar::block_hash hash5{ 5 };
...
TEST (vote_spacing, prune)
{
	...
	vxldollar::vote_spacing spacing{ length };
	vxldollar::root root1{ 1 };
	vxldollar::root root2{ 2 };
	vxldollar::block_hash hash3{ 3 };
	vxldollar::block_hash hash4{ 4 };
...
TEST (vote_spacing, vote_generator)
{
	...
	auto send2 = builder.make_block ()
				 ...
				 .previous (vxldollar::dev::genesis->hash ())
				 .representative (vxldollar::dev::genesis_key.pub)
				 .balance (vxldollar::dev::constants.genesis_amount - vxldollar::Gxrb_ratio - 1)
				 .link (vxldollar::dev::genesis_key.pub)
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				 .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
				 .build_shared ();
	ASSERT_EQ (vxldollar::process_result::progress, node.ledger.process (node.store.tx_begin_write (), *send1).code);
	ASSERT_EQ (0, node.stats.count (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_broadcasts));
	node.active.generator.add (vxldollar::dev::genesis->hash (), send1->hash ());
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_broadcasts) == 1);
	...
	node.active.generator.add (vxldollar::dev::genesis->hash (), send2->hash ());
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_spacing) == 1);
	ASSERT_EQ (1, node.stats.count (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_broadcasts));
	std::this_thread::sleep_for (config.network_params.voting.delay);
	node.active.generator.add (vxldollar::dev::genesis->hash (), send2->hash ());
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_broadcasts) == 2);
}
...
TEST (vote_spacing, rapid)
{
	...
	node.active.generator.add (vxldollar::dev::genesis->hash (), send2->hash ());
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_spacing) == 1);
	ASSERT_TIMELY (3s, 1 == node.stats.count (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_broadcasts));
	std::this_thread::sleep_for (config.network_params.voting.delay);
	node.active.generator.add (vxldollar::dev::genesis->hash (), send2->hash ());
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::vote_generator, vxldollar::stat::detail::generator_broadcasts) == 2);
}

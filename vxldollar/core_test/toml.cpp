...
TEST (toml, diff)
{
	vxldollar::tomlconfig defaults, other;
...
{
	vxldollar::tomlconfig t;
	boost::filesystem::path data_path (".");
	vxldollar::daemon_config c{ data_path, vxldollar::dev::network_params };
...
TEST (toml, rpc_config_deserialize_defaults)
{
	...
	vxldollar::tomlconfig t;
	t.read (ss);
	vxldollar::rpc_config conf{ vxldollar::dev::network_params.network };
	vxldollar::rpc_config defaults{ vxldollar::dev::network_params.network };
...
TEST (toml, daemon_config_deserialize_defaults)
{
	...
	vxldollar::tomlconfig t;
	t.read (ss);
	vxldollar::daemon_config conf;
	vxldollar::daemon_config defaults;
	conf.deserialize_toml (t);
...
TEST (toml, base_override)
{
	...
	ASSERT_TRUE (t.get_error () == vxldollar::error_config::missing_value);
	t.get_error ().clear ();

	// Query uint16 that's too big, make sure we have an error
	t.get_required<uint16_t> ("node.too_big", port);
	ASSERT_TRUE (t.get_error ());
	ASSERT_TRUE (t.get_error () == vxldollar::error_config::invalid_value);
}

TEST (toml, put)
{
	vxldollar::tomlconfig config;
	vxldollar::tomlconfig config_node;
...
TEST (toml, daemon_config_deserialize_no_defaults)
{
	...
	preconfigured_representatives = ["vxld_3arg3asgtigae3xckabaaewkx3bzsh7nwz7jkmjos79ihyaxwphhm6qgjps4"]
	receive_minimum = "999"
	...
	ASSERT_EQ (vxldollar::rocksdb_config::using_rocksdb_in_tests (), defaults.node.rocksdb_config.enable);
...
TEST (toml, daemon_config_deserialize_errors)
{
...

	{
		...
		ASSERT_EQ (conf.node.frontiers_confirmation, vxldollar::frontiers_confirmation_mode::invalid);
	}
...
TEST (toml, daemon_read_config)
{
	...
	vxldollar::daemon_config config;
	...
	ASSERT_FALSE (boost::filesystem::exists (vxldollar::get_node_toml_config_path (path)));
	ASSERT_FALSE (vxldollar::read_node_config_toml (path, config));
	{
		auto error = vxldollar::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
		ASSERT_EQ (error.get_message (), expected_message1);
	}
	{
		auto error = vxldollar::read_node_config_toml (path, config, invalid_overrides2);
		ASSERT_TRUE (error);
	...
	vxldollar::tomlconfig toml;
	toml.write (vxldollar::get_node_toml_config_path (path));

	// Reading when there is a config file
	ASSERT_TRUE (boost::filesystem::exists (vxldollar::get_node_toml_config_path (path)));
	ASSERT_FALSE (vxldollar::read_node_config_toml (path, config));
	{
		auto error = vxldollar::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
...
TEST (toml, tls_config_deserialize_no_defaults)
{
	...
	vxldollar::tomlconfig toml;
	toml.read (ss);
	vxldollar::tls_config conf;
	vxldollar::tls_config defaults;
	conf.deserialize_toml (toml);

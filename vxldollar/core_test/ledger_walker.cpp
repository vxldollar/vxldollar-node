...
TEST (ledger_walker, genesis_block)
{
	...
	vxldollar::ledger_walker ledger_walker{ node->ledger };

	std::size_t walked_blocks_count = 0;
	ledger_walker.walk_backward (vxldollar::dev::genesis->hash (),
	[&] (auto const & block) {
		++walked_blocks_count;
		EXPECT_EQ (block->hash (), vxldollar::dev::genesis->hash ());
	});

	EXPECT_EQ (walked_blocks_count, 1);

	walked_blocks_count = 0;
	ledger_walker.walk (vxldollar::dev::genesis->hash (),
	[&] (auto const & block) {
		++walked_blocks_count;
		EXPECT_EQ (block->hash (), vxldollar::dev::genesis->hash ());
	});
...
TEST (ledger_walker, genesis_account_longer)
{
	...
	vxldollar::account_info genesis_account_info{};
	ASSERT_FALSE (node->ledger.store.account.get (transaction, vxldollar::dev::genesis_key.pub, genesis_account_info));
	...
	{
		auto const send = system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.pub, 1);
		ASSERT_TRUE (send);
...
TEST (ledger_walker, cross_account)
{
	...
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	ASSERT_TRUE (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.pub, 1));
	ASSERT_TIMELY (3s, 3 == node->ledger.cache.cemented_count);

	vxldollar::keypair key{};
	system.wallet (0)->insert_adhoc (key.prv);
	ASSERT_TRUE (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key.pub, 1));
...
// PR in which it got disabled: https://github.com/vxldollarcurrency/vxldollar-node/pull/3602
// Issue for investigating it: https://github.com/vxldollarcurrency/vxldollar-node/issues/3603
TEST (ledger_walker, DISABLED_ladder_geometry)
{
	...
	auto const node = system.add_node (node_config);
	std::array<vxldollar::keypair, 3> keys{};

	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	for (auto itr = 0; itr != keys.size (); ++itr)
	{
		system.wallet (0)->insert_adhoc (keys[itr].prv);
		auto const block = system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, keys[itr].pub, 1000);
		ASSERT_TRUE (block);
		ASSERT_TIMELY (3s, 1 + (itr + 1) * 2 == node->ledger.cache.cemented_count);
	}

	std::vector<vxldollar::uint128_t> amounts_to_send (10);
	std::iota (amounts_to_send.begin (), amounts_to_send.end (), 1);

	vxldollar::account const * last_destination{};
	...
	vxldollar::account_info last_destination_info{};
	...
	std::vector<vxldollar::uint128_t> amounts_expected_backwards{ 10, 9, 8, 5, 4, 3, 1000, 1, 1000, 2, 1000, 6, 7 };

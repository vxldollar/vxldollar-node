...
{
	auto const & null_account = vxldollar::account::null ();
	ASSERT_TRUE (null_account == nullptr);
	ASSERT_FALSE (null_account != nullptr);

	vxldollar::account default_account{};
...
TEST (node, work_generate)
{
	...
	vxldollar::block_hash root{ 1 };
	vxldollar::work_version version{ vxldollar::work_version::work_1 };
	{
		auto difficulty = vxldollar::difficulty::from_multiplier (1.5, node.network_params.work.base);
		auto work = node.work_generate_blocking (version, root, difficulty);
		ASSERT_TRUE (work.is_initialized ());
		ASSERT_TRUE (vxldollar::dev::network_params.work.difficulty (version, root, *work) >= difficulty);
	}
	{
		auto difficulty = vxldollar::difficulty::from_multiplier (0.5, node.network_params.work.base);
		boost::optional<uint64_t> work;
		do
		{
			work = node.work_generate_blocking (version, root, difficulty);
		} while (vxldollar::dev::network_params.work.difficulty (version, root, *work) >= node.network_params.work.base);
		ASSERT_TRUE (work.is_initialized ());
		ASSERT_TRUE (vxldollar::dev::network_params.work.difficulty (version, root, *work) >= difficulty);
		ASSERT_FALSE (vxldollar::dev::network_params.work.difficulty (version, root, *work) >= node.network_params.work.base);
	}
}
...
TEST (node, block_store_path_failure)
{
	...
	auto node (std::make_shared<vxldollar::node> (*service, vxldollar::get_available_port (), path, logging, pool));
...
#endif
{
	...
	{
		vxldollar::inactive_node node (vxldollar::unique_path (), vxldollar::inactive_node_flag_defaults ());
		ASSERT_TRUE (node.node->init_error ());
	}
	else
	{
		ASSERT_EXIT (vxldollar::inactive_node node (vxldollar::unique_path (), vxldollar::inactive_node_flag_defaults ()), ::testing::ExitedWithCode (1), "");
	}
}
...
TEST (node, password_fanout)
{
	...
	vxldollar::node node (io_ctx, path, config, pool);
...
TEST (node, balance)
{
	...
	ASSERT_EQ (std::numeric_limits<vxldollar::uint128_t>::max (), system.nodes[0]->ledger.account_balance (transaction, vxldollar::dev::genesis_key.pub));
}

TEST (node, representative)
{
	vxldollar::system system (1);
	auto block1 (system.nodes[0]->rep_block (vxldollar::dev::genesis_key.pub));
	{
...
TEST (node, send_unkeyed)
{
	...
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	system.wallet (0)->store.password.value_set (vxldollar::keypair ().prv);
	ASSERT_EQ (nullptr, system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
}
...
TEST (node, send_self)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
	ASSERT_TIMELY (10s, !system.nodes[0]->balance (key2.pub).is_zero ());
	ASSERT_EQ (std::numeric_limits<vxldollar::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (vxldollar::dev::genesis_key.pub));
}
...
TEST (node, send_single_observing_peer)
{
	...
	ASSERT_EQ (std::numeric_limits<vxldollar::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (vxldollar::dev::genesis_key.pub));
	ASSERT_TRUE (system.nodes[0]->balance (key2.pub).is_zero ());
	ASSERT_TIMELY (10s, std::all_of (system.nodes.begin (), system.nodes.end (), [&] (std::shared_ptr<vxldollar::node> const & node_a) { return !node_a->balance (key2.pub).is_zero (); }));
}

TEST (node, send_single_many_peers)
{
	vxldollar::system system (10);
	vxldollar::keypair key2;
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	system.wallet (1)->insert_adhoc (key2.prv);
	ASSERT_NE (nullptr, system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
	ASSERT_EQ (std::numeric_limits<vxldollar::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (vxldollar::dev::genesis_key.pub));
	ASSERT_TRUE (system.nodes[0]->balance (key2.pub).is_zero ());
	ASSERT_TIMELY (3.5min, std::all_of (system.nodes.begin (), system.nodes.end (), [&] (std::shared_ptr<vxldollar::node> const & node_a) { return !node_a->balance (key2.pub).is_zero (); }));
	system.stop ();
...
TEST (node, send_out_of_order)
{
	...
	auto send1 = builder.make_block ()
				 ...
				 .balance (std::numeric_limits<vxldollar::uint128_t>::max () - node1.config.receive_minimum.number ())
	...
	auto send2 = builder.make_block ()
				 .previous (send1->hash ())
				 .destination (key2.pub)
				 .balance (std::numeric_limits<vxldollar::uint128_t>::max () - 2 * node1.config.receive_minimum.number ())
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
	...
	auto send3 = builder.make_block ()
				 .previous (send2->hash ())
				 .destination (key2.pub)
				 .balance (std::numeric_limits<vxldollar::uint128_t>::max () - 3 * node1.config.receive_minimum.number ())
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
	...
	ASSERT_TIMELY (10s, std::all_of (system.nodes.begin (), system.nodes.end (), [&] (std::shared_ptr<vxldollar::node> const & node_a) { return node_a->balance (vxldollar::dev::genesis_key.pub) == vxldollar::dev::constants.genesis_amount - node1.config.receive_minimum.number () * 3; }));
}
...
TEST (node, quick_confirm)
{
	...
	auto & node1 (*system.nodes[0]);
	vxldollar::keypair key;
	vxldollar::block_hash previous (node1.latest (vxldollar::dev::genesis_key.pub));
	auto genesis_start_balance (node1.balance (vxldollar::dev::genesis_key.pub));
	system.wallet (0)->insert_adhoc (key.prv);
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	auto send = vxldollar::send_block_builder ()
				.previous (previous)
				.destination (key.pub)
	...
	ASSERT_EQ (node1.balance (vxldollar::dev::genesis_key.pub), node1.online_reps.delta () + 1);
...
TEST (node, node_receive_quorum)
{
	...
	ASSERT_TIMELY (10s, node1.active.election (vxldollar::qualified_root (previous, previous)) != nullptr);
	auto election (node1.active.election (vxldollar::qualified_root (previous, previous)));
	...
	system2.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
...
TEST (node, auto_bootstrap)
{
	...
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	system.wallet (0)->insert_adhoc (key2.prv);
	auto send1 (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, node0->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send1);
	ASSERT_TIMELY (10s, node0->balance (key2.pub) == node0->config.receive_minimum.number ());
	auto node1 (std::make_shared<vxldollar::node> (system.io_ctx, vxldollar::get_available_port (), vxldollar::unique_path (), system.logging, system.work, node_flags));
	ASSERT_FALSE (node1->init_error ());
	node1->start ();
	system.nodes.push_back (node1);
	ASSERT_NE (nullptr, vxldollar::establish_tcp (system, *node1, node0->network.endpoint ()));
...
TEST (node, auto_bootstrap_reverse)
{
	...
	auto node1 (std::make_shared<vxldollar::node> (system.io_ctx, vxldollar::get_available_port (), vxldollar::unique_path (), system.logging, system.work, node_flags));
	ASSERT_FALSE (node1->init_error ());
	ASSERT_NE (nullptr, system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, node0->config.receive_minimum.number ()));
	node1->start ();
	system.nodes.push_back (node1);
	ASSERT_NE (nullptr, vxldollar::establish_tcp (system, *node0, node1->network.endpoint ()));
	ASSERT_TIMELY (10s, node1->balance (key2.pub) == node0->config.receive_minimum.number ());
}
...
TEST (node, auto_bootstrap_age)
{
	...
	ASSERT_TIMELY (10s, node0->stats.count (vxldollar::stat::type::bootstrap, vxldollar::stat::detail::initiate_legacy_age, vxldollar::stat::dir::out) >= 3);
	// More attempts with frontiers age
	ASSERT_GE (node0->stats.count (vxldollar::stat::type::bootstrap, vxldollar::stat::detail::initiate_legacy_age, vxldollar::stat::dir::out), node0->stats.count (vxldollar::stat::type::bootstrap, vxldollar::stat::detail::initiate, vxldollar::stat::dir::out));
...
TEST (node, receive_gap)
{
	...
	auto block = vxldollar::send_block_builder ()
				 .previous (5)
				 .destination (1)
				 .balance (2)
				 .sign (vxldollar::keypair ().prv, 4)
				 .work (0)
				 .build_shared ();
...
{
	vxldollar::system system (1);
	std::array<vxldollar::endpoint, 8> endpoints;
	endpoints.fill (vxldollar::endpoint (boost::asio::ip::address_v6::loopback (), vxldollar::get_available_port ()));
	endpoints[0] = vxldollar::endpoint (boost::asio::ip::address_v6::loopback (), vxldollar::get_available_port ());
...
TEST (node, search_receivable)
{
	...
	vxldollar::keypair key2;
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	ASSERT_NE (nullptr, system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	system.wallet (0)->insert_adhoc (key2.prv);
...
TEST (node, search_receivable_multiple)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key3.pub, node->config.receive_minimum.number ()));
...
TEST (node, search_receivable_confirmed)
{
	...
	vxldollar::keypair key2;
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	auto send1 (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send1);
	auto send2 (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	...
	}
	{
		...
		system.wallet (0)->store.erase (transaction, vxldollar::dev::genesis_key.pub);
	}
	system.wallet (0)->insert_adhoc (key2.prv);
	ASSERT_FALSE (system.wallet (0)->search_receivable (system.wallet (0)->wallets.tx_begin_read ()));
	{
...
TEST (node, search_receivable_pruned)
{
	...
	vxldollar::keypair key2;
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	auto send1 (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, node2->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send1);
	auto send2 (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, node2->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send2);
	...
	system.wallet (0)->store.erase (node1->wallets.tx_begin_write (), vxldollar::dev::genesis_key.pub);
...
TEST (node, unlock_search)
{
	...
	auto node (system.nodes[0]);
	vxldollar::keypair key2;
	vxldollar::uint128_t balance (node->balance (vxldollar::dev::genesis_key.pub));
	{
	...
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	ASSERT_NE (nullptr, system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	ASSERT_TIMELY (10s, node->balance (vxldollar::dev::genesis_key.pub) != balance);
	ASSERT_TIMELY (10s, node->active.empty ());
	system.wallet (0)->insert_adhoc (key2.prv);
	{
		vxldollar::lock_guard<std::recursive_mutex> lock (system.wallet (0)->store.mutex);
		system.wallet (0)->store.password.value_set (vxldollar::keypair ().prv);
	}
	{
		auto transaction (system.wallet (0)->wallets.tx_begin_write ());
...
TEST (node, connect_after_junk)
{
	...
	auto channel1 (std::make_shared<vxldollar::transport::channel_udp> (node1->network.udp_channels, node0->network.endpoint (), node1->network_params.network.protocol_version));
	channel1->send_buffer (vxldollar::shared_const_buffer (std::move (junk_buffer)), [] (boost::system::error_code const &, size_t) {});
	ASSERT_TIMELY (10s, node0->stats.count (vxldollar::stat::type::error) != 0);
	node1->start ();
	system.nodes.push_back (node1);
	auto channel2 (std::make_shared<vxldollar::transport::channel_udp> (node1->network.udp_channels, node0->network.endpoint (), node1->network_params.network.protocol_version));
...
TEST (node, working)
{
	auto path (vxldollar::working_path ());
...
{
	vxldollar::system system (1);
	auto price1 (system.nodes[0]->price (vxldollar::Gxrb_ratio, 1));
	ASSERT_EQ (vxldollar::node::price_max * 100.0, price1);
	auto price2 (system.nodes[0]->price (vxldollar::Gxrb_ratio * int (vxldollar::node::free_cutoff + 1), 1));
	ASSERT_EQ (0, price2);
	auto price3 (system.nodes[0]->price (vxldollar::Gxrb_ratio * int (vxldollar::node::free_cutoff + 2) / 2, 1));
	ASSERT_EQ (vxldollar::node::price_max * 100.0 / 2, price3);
	auto price4 (system.nodes[0]->price (vxldollar::Gxrb_ratio * int (vxldollar::node::free_cutoff) * 2, 1));
	ASSERT_EQ (0, price4);
}
...
{
	auto path (vxldollar::unique_path ());
	vxldollar::logging logging1;
	logging1.init (path);
	vxldollar::node_config config1 (100, logging1);
...
TEST (node_flags, disable_tcp_realtime)
{
	...
	ASSERT_EQ (vxldollar::transport::transport_type::udp, list1[0]->get_type ());
	ASSERT_EQ (1, node2->network.size ());
	auto list2 (node2->network.list (2));
	ASSERT_EQ (node1->network.endpoint (), list2[0]->get_endpoint ());
	ASSERT_EQ (vxldollar::transport::transport_type::udp, list2[0]->get_type ());
}
...
TEST (node_flags, disable_tcp_realtime_and_bootstrap_listener)
{
	...
	auto node2 = system.add_node (node_flags);
	ASSERT_EQ (vxldollar::tcp_endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->bootstrap.endpoint ());
	ASSERT_NE (vxldollar::endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->network.endpoint ());
	ASSERT_EQ (1, node1->network.size ());
	auto list1 (node1->network.list (2));
	ASSERT_EQ (node2->network.endpoint (), list1[0]->get_endpoint ());
	ASSERT_EQ (vxldollar::transport::transport_type::udp, list1[0]->get_type ());
...
TEST (node_flags, disable_udp)
{
	...
	node_flags.disable_udp = false;
	auto node1 = system.add_node (node_flags);
	auto node2 (std::make_shared<vxldollar::node> (system.io_ctx, vxldollar::unique_path (), vxldollar::node_config (vxldollar::get_available_port (), system.logging), system.work));
	system.nodes.push_back (node2);
	node2->start ();
	ASSERT_EQ (vxldollar::endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->network.udp_channels.get_local_endpoint ());
	ASSERT_NE (vxldollar::endpoint (boost::asio::ip::address_v6::loopback (), 0), node2->network.endpoint ());
	// Send UDP message
	auto channel (std::make_shared<vxldollar::transport::channel_udp> (node1->network.udp_channels, node2->network.endpoint (), node2->network_params.network.protocol_version));
	...
	auto list1 (node1->network.list (2));
	ASSERT_EQ (node2->network.endpoint (), list1[0]->get_endpoint ());
	ASSERT_EQ (vxldollar::transport::transport_type::tcp, list1[0]->get_type ());
	ASSERT_EQ (1, node2->network.size ());
	auto list2 (node2->network.list (2));
	ASSERT_EQ (node1->network.endpoint (), list2[0]->get_endpoint ());
	ASSERT_EQ (vxldollar::transport::transport_type::tcp, list2[0]->get_type ());
	node2->stop ();
}

TEST (node, fork_publish)
{
	std::weak_ptr<vxldollar::node> node0;
	{
		vxldollar::system system (1);
...
// PR in which it got disabled: https://github.com/vxldollarcurrency/vxldollar-node/pull/3611
// Issue for investigating it: https://github.com/vxldollarcurrency/vxldollar-node/issues/3614
TEST (node, DISABLED_fork_publish_inactive)
{
	...
	ASSERT_EQ (vxldollar::process_result::fork, node.process_local (send2).code);
...
TEST (node, fork_keep)
{
	...
	node2.block_processor.flush ();
	auto election1 (node2.active.election (vxldollar::qualified_root (vxldollar::dev::genesis->hash (), vxldollar::dev::genesis->hash ())));
...
TEST (node, fork_flip)
{
	...
	auto send1 = builder.make_block ()
				 ...
				 .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
				 .build_shared ();
	vxldollar::publish publish1{ vxldollar::dev::network_params.network, send1 };
	vxldollar::keypair key2;
	auto send2 = builder.make_block ()
				 .previous (vxldollar::dev::genesis->hash ())
				 .destination (key2.pub)
				 ...
				 .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
				 .build_shared ();
	vxldollar::publish publish2{ vxldollar::dev::network_params.network, send2 };
...
{
	std::vector<vxldollar::transport::transport_type> types{ vxldollar::transport::transport_type::tcp, vxldollar::transport::transport_type::udp };
	for (auto & type : types)
	{
		vxldollar::system system;
		vxldollar::node_flags node_flags;
		if (type == vxldollar::transport::transport_type::udp)
		{
			node_flags.disable_tcp_realtime = true;
			node_flags.disable_bootstrap_listener = true;
			node_flags.disable_udp = false;
		...
		auto send3 = builder.make_block ()
					 ...
					 .build_shared ();
		vxldollar::publish publish3{ vxldollar::dev::network_params.network, send3 };
...
TEST (node, fork_bootstrap_flip)
{
	...
	vxldollar::system system1;
	vxldollar::node_config config0{ vxldollar::get_available_port (), system0.logging };
	config0.frontiers_confirmation = vxldollar::frontiers_confirmation_mode::disabled;
	vxldollar::node_flags node_flags;
	node_flags.disable_bootstrap_bulk_push_client = true;
	node_flags.disable_lazy_bootstrap = true;
	auto & node1 = *system0.add_node (config0, node_flags);
	vxldollar::node_config config1 (vxldollar::get_available_port (), system1.logging);
	...
	ASSERT_EQ (vxldollar::process_result::progress, node2.ledger.process (node2.store.tx_begin_write (), *send2).code);
...
TEST (node, fork_open)
{
	...
	auto & node1 (*system.nodes[0]);
	vxldollar::keypair key1;
	auto send1 = vxldollar::send_block_builder ()
				 .previous (vxldollar::dev::genesis->hash ())
				 .destination (key1.pub)
				 .balance (0)
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
	...
	vxldollar::open_block_builder builder;
	auto open1 = builder.make_block ()
				 ...
				 .build_shared ();
	vxldollar::publish publish2{ vxldollar::dev::network_params.network, open1 };
	node1.network.inbound (publish2, channel1);
	node1.block_processor.flush ();
	node1.scheduler.flush ();
	ASSERT_EQ (1, node1.active.size ());
	auto open2 = builder.make_block ()
				 ...
				 .work (*system.work.generate (key1.pub))
				 .build_shared ();
	vxldollar::publish publish3{ vxldollar::dev::network_params.network, open2 };
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
...
TEST (node, fork_open_flip)
{
	...
	ASSERT_EQ (1, node1.network.size ());
	vxldollar::keypair key1;
	vxldollar::keypair rep1;
	vxldollar::keypair rep2;
	auto send1 = vxldollar::send_block_builder ()
				 .previous (vxldollar::dev::genesis->hash ())
				 .destination (key1.pub)
	...
	auto send1_copy (std::make_shared<vxldollar::send_block> (*send1));
	node1.process_active (send1);
	...
	ASSERT_EQ (vxldollar::dev::constants.genesis_amount - 1, winner.first);
...
TEST (node, coherent_observer)
{
	vxldollar::system system (1);
	auto & node1 (*system.nodes[0]);
	node1.observers.blocks.add ([&node1] (vxldollar::election_status const & status_a, std::vector<vxldollar::vote_with_weight_info> const &, vxldollar::account const &, vxldollar::uint128_t const &, bool, bool) {
		auto transaction (node1.store.tx_begin_read ());
		ASSERT_TRUE (node1.store.block.exists (transaction, status_a.winner->hash ()));
	});
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	vxldollar::keypair key;
	system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key.pub, 1);
}
...
TEST (node, fork_no_vote_quorum)
{
	...
	system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key4, vxldollar::dev::constants.genesis_amount / 4);
	...
	auto block (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key1, node1.config.receive_minimum.number ()));
	...
	vxldollar::state_block send1 (vxldollar::dev::genesis_key.pub, block->hash (), vxldollar::dev::genesis_key.pub, (vxldollar::dev::constants.genesis_amount / 4) - (node1.config.receive_minimum.number () * 2), key1, vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, *system.work.generate (block->hash ()));
	ASSERT_EQ (vxldollar::process_result::progress, node1.process (send1).code);
	ASSERT_EQ (vxldollar::process_result::progress, node2.process (send1).code);
	ASSERT_EQ (vxldollar::process_result::progress, node3.process (send1).code);
	auto key2 (system.wallet (2)->deterministic_insert ());
	auto send2 = vxldollar::send_block_builder ()
				 .previous (block->hash ())
				 .destination (key2)
				 .balance ((vxldollar::dev::constants.genesis_amount / 4) - (node1.config.receive_minimum.number () * 2))
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				 .work (*system.work.generate (block->hash ()))
				 .build_shared ();
	vxldollar::raw_key key3;
	auto transaction (system.wallet (1)->wallets.tx_begin_read ());
	ASSERT_FALSE (system.wallet (1)->store.fetch (transaction, key1, key3));
	auto vote (std::make_shared<vxldollar::vote> (key1, key3, 0, 0, send2));
	vxldollar::confirm_ack confirm{ vxldollar::dev::network_params.network, vote };
	...
	channel->send_buffer (vxldollar::shared_const_buffer (std::move (buffer)));
	ASSERT_TIMELY (10s, node3.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::in) >= 3);
	ASSERT_TRUE (node1.latest (vxldollar::dev::genesis_key.pub) == send1.hash ());
	ASSERT_TRUE (node2.latest (vxldollar::dev::genesis_key.pub) == send1.hash ());
	ASSERT_TRUE (node3.latest (vxldollar::dev::genesis_key.pub) == send1.hash ());
}
...
TEST (node, DISABLED_fork_pre_confirm)
{
	...
	auto block0 (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key1.pub, vxldollar::dev::constants.genesis_amount / 3));
	ASSERT_NE (nullptr, block0);
	ASSERT_TIMELY (30s, node0.balance (key1.pub) != 0);
	auto block1 (system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, key2.pub, vxldollar::dev::constants.genesis_amount / 3));
	...
	auto block2 = builder.make_block ()
				  ...
				  .previous (node0.latest (vxldollar::dev::genesis_key.pub))
				  .representative (key3.pub)
				  .balance (node0.balance (vxldollar::dev::genesis_key.pub))
				  .link (0)
				  .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				  .work (0)
				  .build_shared ();
	...
	while (!done)
	{
		done |= node0.latest (vxldollar::dev::genesis_key.pub) == block2->hash () && node1.latest (vxldollar::dev::genesis_key.pub) == block2->hash () && node2.latest (vxldollar::dev::genesis_key.pub) == block2->hash ();
		done |= node0.latest (vxldollar::dev::genesis_key.pub) == block3->hash () && node1.latest (vxldollar::dev::genesis_key.pub) == block3->hash () && node2.latest (vxldollar::dev::genesis_key.pub) == block3->hash ();
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, DISABLED_fork_stale)
{
	...
	system1.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	vxldollar::system system2 (1);
	auto & node1 (*system1.nodes[0]);
	auto & node2 (*system2.nodes[0]);
	node2.bootstrap_initiator.bootstrap (node1.network.endpoint (), false);
	std::shared_ptr<vxldollar::transport::channel> channel (std::make_shared<vxldollar::transport::channel_udp> (node2.network.udp_channels, node1.network.endpoint (), node2.network_params.network.protocol_version));
	auto vote = std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, 0, 0, std::vector<vxldollar::block_hash> ());
	...
	auto send3 = builder.make_block ()
				 ...
				 .balance (vxldollar::dev::constants.genesis_amount - vxldollar::Mxrb_ratio)
				 .link (key1.pub)
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				 .work (0)
				 .build_shared ();
	...
	auto send1 = builder.make_block ()
				 ...
				 .previous (send3->hash ())
				 .representative (vxldollar::dev::genesis_key.pub)
				 .balance (vxldollar::dev::constants.genesis_amount - 2 * vxldollar::Mxrb_ratio)
				 .link (key1.pub)
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				 .work (0)
				 .build_shared ();
	...
	node1.work_generate_blocking (*send2);
	{
		...
		ASSERT_EQ (vxldollar::process_result::progress, node1.ledger.process (transaction1, *send1).code);
		auto transaction2 (node2.store.tx_begin_write ());
		ASSERT_EQ (vxldollar::process_result::progress, node2.ledger.process (transaction2, *send2).code);
	}
	node1.process_active (send1);
	node1.process_active (send2);
	node2.process_active (send1);
...
// Issue for investigating it: https://github.com/vxldollarcurrency/vxldollar-node/issues/3516
TEST (node, DISABLED_broadcast_elected)
{
	...
	for (auto & type : types)
	{
		...
		node_config.peering_port = vxldollar::get_available_port ();
		auto node2 = system.add_node (node_config, node_flags, type);
		vxldollar::keypair rep_big;
		vxldollar::keypair rep_small;
		vxldollar::keypair rep_other;
		vxldollar::block_builder builder;
		{
			...
			auto fund_big = *builder.send ()
							 ...
							 .balance (vxldollar::Gxrb_ratio * 5)
							 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
							 .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
							 .build ();
			...
			auto fund_small = *builder.send ()
							   ...
							   .balance (vxldollar::Gxrb_ratio * 2)
							   .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
			...
			auto open_other = *builder.open ()
							   ...
							   .build ();
			ASSERT_EQ (vxldollar::process_result::progress, node0->ledger.process (transaction0, fund_big).code);
			ASSERT_EQ (vxldollar::process_result::progress, node1->ledger.process (transaction1, fund_big).code);
			ASSERT_EQ (vxldollar::process_result::progress, node2->ledger.process (transaction2, fund_big).code);
			ASSERT_EQ (vxldollar::process_result::progress, node0->ledger.process (transaction0, open_big).code);
			ASSERT_EQ (vxldollar::process_result::progress, node1->ledger.process (transaction1, open_big).code);
			ASSERT_EQ (vxldollar::process_result::progress, node2->ledger.process (transaction2, open_big).code);
			ASSERT_EQ (vxldollar::process_result::progress, node0->ledger.process (transaction0, fund_small).code);
			ASSERT_EQ (vxldollar::process_result::progress, node1->ledger.process (transaction1, fund_small).code);
			ASSERT_EQ (vxldollar::process_result::progress, node2->ledger.process (transaction2, fund_small).code);
			ASSERT_EQ (vxldollar::process_result::progress, node0->ledger.process (transaction0, open_small).code);
			ASSERT_EQ (vxldollar::process_result::progress, node1->ledger.process (transaction1, open_small).code);
			ASSERT_EQ (vxldollar::process_result::progress, node2->ledger.process (transaction2, open_small).code);
			ASSERT_EQ (vxldollar::process_result::progress, node0->ledger.process (transaction0, fund_other).code);
			ASSERT_EQ (vxldollar::process_result::progress, node1->ledger.process (transaction1, fund_other).code);
			ASSERT_EQ (vxldollar::process_result::progress, node2->ledger.process (transaction2, fund_other).code);
			ASSERT_EQ (vxldollar::process_result::progress, node0->ledger.process (transaction0, open_other).code);
			ASSERT_EQ (vxldollar::process_result::progress, node1->ledger.process (transaction1, open_other).code);
			ASSERT_EQ (vxldollar::process_result::progress, node2->ledger.process (transaction2, open_other).code);
		}
		// Confirm blocks to allow voting
		for (auto & node : system.nodes)
		{
			auto block (node->block (node->latest (vxldollar::dev::genesis_key.pub)));
			ASSERT_NE (nullptr, block);
		...
		auto fork0 = builder.send ()
					 .previous (node2->latest (vxldollar::dev::genesis_key.pub))
					 .destination (rep_small.pub)
					 .balance (0)
					 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
					 .work (*node0->work_generate_blocking (node2->latest (vxldollar::dev::genesis_key.pub)))
					 .build_shared ();
		// A copy is necessary to avoid data races during ledger processing, which sets the sideband
		auto fork0_copy (std::make_shared<vxldollar::send_block> (*fork0));
		...
		ASSERT_TIMELY (5s, node1->stats.count (vxldollar::stat::type::confirmation_observer, vxldollar::stat::detail::inactive_conf_height, vxldollar::stat::dir::out) != 0);
	}
}
...
TEST (node, rep_self_vote)
{
	...
	auto fund_big = *builder.send ()
					 .previous (vxldollar::dev::genesis->hash ())
					 .destination (rep_big.pub)
					 .balance (vxldollar::uint128_t{ "0xb0000000000000000000000000000000" })
					 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
					 .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
					 .build ();
	auto open_big = *builder.open ()
					 ...
					 .work (*system.work.generate (rep_big.pub))
					 .build ();
	ASSERT_EQ (vxldollar::process_result::progress, node0->process (fund_big).code);
	ASSERT_EQ (vxldollar::process_result::progress, node0->process (open_big).code);
	...
	auto block0 = builder.send ()
				  .previous (fund_big.hash ())
				  .destination (rep_big.pub)
				  .balance (vxldollar::uint128_t ("0x60000000000000000000000000000000"))
				  .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				  .work (*system.work.generate (fund_big.hash ()))
				  .build_shared ();
	ASSERT_EQ (vxldollar::process_result::progress, node0->process (*block0).code);
	auto & active = node0->active;
	auto & scheduler = node0->scheduler;
	scheduler.activate (vxldollar::dev::genesis_key.pub, node0->store.tx_begin_read ());
	scheduler.flush ();
	...
	ASSERT_NE (rep_votes.end (), rep_votes.find (vxldollar::dev::genesis_key.pub));
...
{
	vxldollar::system system0 (1);
	vxldollar::system system1 (1);
	auto node0 (system0.nodes[0]);
	auto node1 (system1.nodes[0]);
	vxldollar::keypair key0;
	// node0 knows about send0 but node1 doesn't.
	vxldollar::send_block send0 (node0->latest (vxldollar::dev::genesis_key.pub), key0.pub, 500, vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub, 0);
	{
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (vxldollar::process_result::progress, node0->ledger.process (transaction, send0).code);
	}
...
// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/vxldollarcurrency/vxldollar-node/pull/3512
// Issue for investigating it: https://github.com/vxldollarcurrency/vxldollar-node/issues/3515
TEST (node, DISABLED_bootstrap_bulk_push)
{
	...
	vxldollar::keypair key0;
	// node0 knows about send0 but node1 doesn't.
	auto send0 = vxldollar::send_block_builder ()
				 .previous (vxldollar::dev::genesis->hash ())
				 .destination (key0.pub)
				 .balance (500)
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				 .work (*node0->work_generate_blocking (vxldollar::dev::genesis->hash ()))
				 .build_shared ();
	ASSERT_EQ (vxldollar::process_result::progress, node0->process (*send0).code);
...
TEST (node, bootstrap_fork_open)
{
	...
	auto send0 = *builder.send ()
				  .previous (vxldollar::dev::genesis->hash ())
				  .destination (key0.pub)
				  .balance (vxldollar::dev::constants.genesis_amount - 500)
				  .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				  .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
				  .build ();
	...
	ASSERT_EQ (vxldollar::process_result::progress, node0->process (send0).code);
	ASSERT_EQ (vxldollar::process_result::progress, node1->process (send0).code);
	// Confirm send0 to allow starting and voting on the following blocks
	for (auto node : system.nodes)
	{
		node->block_confirm (node->block (node->latest (vxldollar::dev::genesis_key.pub)));
	...
	ASSERT_EQ (vxldollar::process_result::progress, node0->process (open0).code);
...
TEST (node, unconfirmed_send)
{
	...
	auto send1 = wallet1->send_action (vxldollar::dev::genesis->account (), key2.pub, 2 * vxldollar::Mxrb_ratio);
	...
	ASSERT_EQ (node2.balance (key2.pub), 2 * vxldollar::Mxrb_ratio);
	...
	auto send2 = vxldollar::state_block_builder{}
				 .make_block ()
				 .account (key2.pub)
				 .previous (recv1->hash ())
				 .representative (vxldollar::dev::genesis_key.pub)
				 .balance (vxldollar::Mxrb_ratio)
				 .link (vxldollar::dev::genesis->account ())
				 .sign (key2.prv, key2.pub)
				 .work (*system.work.generate (recv1->hash ()))
				 .build_shared ();
	ASSERT_EQ (vxldollar::process_result::progress, node2.process (*send2).code);

	auto send3 = wallet2->send_action (key2.pub, vxldollar::dev::genesis->account (), vxldollar::Mxrb_ratio);
	...
	ASSERT_TIMELY (5s, node1.balance (vxldollar::dev::genesis->account ()) == vxldollar::dev::constants.genesis_amount);
}
...
TEST (node, rep_list)
{
	...
	// Node0 has a rep
	wallet0->insert_adhoc (vxldollar::dev::genesis_key.prv);
	vxldollar::keypair key1;
	// Broadcast a confirm so others should know this is a rep node
	wallet0->send_action (vxldollar::dev::genesis_key.pub, key1.pub, vxldollar::Mxrb_ratio);
...
TEST (node, rep_weight)
{
	...
	auto add_node = [&system] {
		auto node = std::make_shared<vxldollar::node> (system.io_ctx, vxldollar::get_available_port (), vxldollar::unique_path (), system.logging, system.work);
		node->start ();
	...
	auto & node2 = *add_node ();
	auto & node3 = *add_node ();
	vxldollar::keypair keypair1;
	vxldollar::keypair keypair2;
	vxldollar::block_builder builder;
	auto amount_pr (node.minimum_principal_weight () + 100);
	auto amount_not_pr (node.minimum_principal_weight () - 100);
	std::shared_ptr<vxldollar::block> block1 = builder
										  .state ()
										  .account (vxldollar::dev::genesis_key.pub)
										  .previous (vxldollar::dev::genesis->hash ())
										  .representative (vxldollar::dev::genesis_key.pub)
										  .balance (vxldollar::dev::constants.genesis_amount - amount_not_pr)
										  .link (keypair1.pub)
										  .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
										  .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
										  .build ();
	std::shared_ptr<vxldollar::block> block2 = builder
										  .state ()
										  ...
										  .build ();
	std::shared_ptr<vxldollar::block> block3 = builder
										  .state ()
										  .account (vxldollar::dev::genesis_key.pub)
										  .previous (block1->hash ())
										  .representative (vxldollar::dev::genesis_key.pub)
										  .balance (vxldollar::dev::constants.genesis_amount - amount_not_pr - amount_pr)
										  .link (keypair2.pub)
										  .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
										  .work (*system.work.generate (block1->hash ()))
										  .build ();
	std::shared_ptr<vxldollar::block> block4 = builder
										  .state ()
										  ...
										  .build ();
	{
		...
		ASSERT_EQ (vxldollar::process_result::progress, node.ledger.process (transaction, *block1).code);
		ASSERT_EQ (vxldollar::process_result::progress, node.ledger.process (transaction, *block2).code);
		ASSERT_EQ (vxldollar::process_result::progress, node.ledger.process (transaction, *block3).code);
		ASSERT_EQ (vxldollar::process_result::progress, node.ledger.process (transaction, *block4).code);
	}
	ASSERT_TRUE (node.rep_crawler.representatives (1).empty ());
	std::shared_ptr<vxldollar::transport::channel> channel1 = vxldollar::establish_tcp (system, node, node1.network.endpoint ());
	ASSERT_NE (nullptr, channel1);
	std::shared_ptr<vxldollar::transport::channel> channel2 = vxldollar::establish_tcp (system, node, node2.network.endpoint ());
	ASSERT_NE (nullptr, channel2);
	std::shared_ptr<vxldollar::transport::channel> channel3 = vxldollar::establish_tcp (system, node, node3.network.endpoint ());
	ASSERT_NE (nullptr, channel3);
	auto vote0 = std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, 0, 0, vxldollar::dev::genesis);
	auto vote1 = std::make_shared<vxldollar::vote> (keypair1.pub, keypair1.prv, 0, 0, vxldollar::dev::genesis);
	auto vote2 = std::make_shared<vxldollar::vote> (keypair2.pub, keypair2.prv, 0, 0, vxldollar::dev::genesis);
	...
	ASSERT_EQ (node.balance (vxldollar::dev::genesis_key.pub), reps[0].weight.number ());
	ASSERT_EQ (vxldollar::dev::genesis_key.pub, reps[0].account);
...
TEST (node, rep_remove)
{
	...
	std::shared_ptr<vxldollar::block> block1 = builder
										  ...
										  .previous (vxldollar::dev::genesis->hash ())
										  .representative (vxldollar::dev::genesis_key.pub)
										  .balance (vxldollar::dev::constants.genesis_amount - node.minimum_principal_weight () * 2)
										  .link (keypair1.pub)
										  .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
										  .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
										  .build ();
	...
	std::shared_ptr<vxldollar::block> block3 = builder
										  ...
										  .previous (block1->hash ())
										  .representative (vxldollar::dev::genesis_key.pub)
										  .balance (vxldollar::dev::constants.genesis_amount - node.minimum_principal_weight () * 4)
										  .link (keypair2.pub)
										  .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
										  .work (*system.work.generate (block1->hash ()))
										  .build ();
	...
	std::shared_ptr<vxldollar::transport::channel> channel0 (std::make_shared<vxldollar::transport::channel_udp> (node.network.udp_channels, endpoint0, node.network_params.network.protocol_version));
	...
	node.network.udp_channels.modify (channel_udp, [fake_timepoint] (std::shared_ptr<vxldollar::transport::channel_udp> const & channel_a) {
	...
	auto node1 = system.add_node (vxldollar::node_config (vxldollar::get_available_port (), system.logging));
	system.wallet (1)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	auto channel1 (node.network.find_channel (node1->network.endpoint ()));
	ASSERT_NE (nullptr, channel1);
	auto vote2 = std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, 0, 0, vxldollar::dev::genesis);
	...
	auto vote3 = std::make_shared<vxldollar::vote> (keypair2.pub, keypair2.prv, 0, 0, vxldollar::dev::genesis);
	node.network.tcp_channels.start_tcp (node2->network.endpoint ());
	std::shared_ptr<vxldollar::transport::channel> channel2;
	ASSERT_TIMELY (10s, (channel2 = node.network.tcp_channels.find_channel (vxldollar::transport::map_endpoint_to_tcp (node2->network.endpoint ()))) != nullptr);
...
TEST (node, no_voting)
{
	...
	// Broadcast a confirm so others should know this is a rep node
	wallet1->send_action (vxldollar::dev::genesis_key.pub, key1.pub, vxldollar::Mxrb_ratio);
	ASSERT_TIMELY (10s, node0.active.empty ());
	ASSERT_EQ (0, node0.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::in));
}
...
TEST (node, send_callback)
{
	...
	ASSERT_EQ (std::numeric_limits<vxldollar::uint128_t>::max () - node0.config.receive_minimum.number (), node0.balance (vxldollar::dev::genesis_key.pub));
}
...
TEST (node, balance_observer)
{
	...
	vxldollar::keypair key;
	node1.observers.account_balance.add ([&key, &balances] (vxldollar::account const & account_a, bool is_pending) {
		if (key.pub == account_a && is_pending)
		{
			balances++;
		}
		else if (vxldollar::dev::genesis_key.pub == account_a && !is_pending)
		{
			balances++;
		}
	});
...
TEST (node, stat_counting)
{
	vxldollar::system system (1);
	auto & node1 (*system.nodes[0]);
	node1.stats.add (vxldollar::stat::type::ledger, vxldollar::stat::dir::in, 1);
	node1.stats.add (vxldollar::stat::type::ledger, vxldollar::stat::dir::in, 5);
	node1.stats.inc (vxldollar::stat::type::ledger, vxldollar::stat::dir::in);
	node1.stats.inc (vxldollar::stat::type::ledger, vxldollar::stat::detail::send, vxldollar::stat::dir::in);
	node1.stats.inc (vxldollar::stat::type::ledger, vxldollar::stat::detail::send, vxldollar::stat::dir::in);
	node1.stats.inc (vxldollar::stat::type::ledger, vxldollar::stat::detail::receive, vxldollar::stat::dir::in);
	ASSERT_EQ (10, node1.stats.count (vxldollar::stat::type::ledger, vxldollar::stat::dir::in));
	ASSERT_EQ (2, node1.stats.count (vxldollar::stat::type::ledger, vxldollar::stat::detail::send, vxldollar::stat::dir::in));
	ASSERT_EQ (1, node1.stats.count (vxldollar::stat::type::ledger, vxldollar::stat::detail::receive, vxldollar::stat::dir::in));
	node1.stats.add (vxldollar::stat::type::ledger, vxldollar::stat::dir::in, 0);
	ASSERT_EQ (10, node1.stats.count (vxldollar::stat::type::ledger, vxldollar::stat::dir::in));
}
...
TEST (node, stat_histogram)
{
	...
	// Specific bins
	node1.stats.define_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_req, vxldollar::stat::dir::in, { 1, 6, 10, 16 });
	node1.stats.update_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_req, vxldollar::stat::dir::in, 1, 50);
	auto histogram_req (node1.stats.get_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_req, vxldollar::stat::dir::in));
	ASSERT_EQ (histogram_req->get_bins ()[0].value, 50);

	// Uniform distribution (12 bins, width 1); also test clamping 100 to the last bin
	node1.stats.define_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::in, { 1, 13 }, 12);
	node1.stats.update_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::in, 1);
	node1.stats.update_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::in, 8, 10);
	node1.stats.update_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::in, 100);

	auto histogram_ack (node1.stats.get_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::in));
	...
	node1.stats.define_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out, { 1, 11 }, 2);
	node1.stats.update_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out, 1, 1);
	node1.stats.update_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out, 6, 1);

	auto histogram_ack_out (node1.stats.get_histogram (vxldollar::stat::type::vote, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out));
...
TEST (node, online_reps)
{
	...
	auto vote (std::make_shared<vxldollar::vote> ());
	ASSERT_EQ (0, node1.online_reps.online ());
	node1.online_reps.observe (vxldollar::dev::genesis_key.pub);
	ASSERT_EQ (vxldollar::dev::constants.genesis_amount, node1.online_reps.online ());
	// 1 minimum, 1 maximum
	ASSERT_EQ (node1.config.online_weight_minimum, node1.online_reps.trended ());
	node1.online_reps.sample ();
	ASSERT_EQ (vxldollar::dev::constants.genesis_amount, node1.online_reps.trended ());
...
TEST (node, online_reps_rep_crawler)
{
	...
	auto vote = std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, vxldollar::milliseconds_since_epoch (), 0, std::vector<vxldollar::block_hash>{ vxldollar::dev::genesis->hash () });
	ASSERT_EQ (0, node1.online_reps.online ());
	// Without rep crawler
	node1.vote_processor.vote_blocking (vote, std::make_shared<vxldollar::transport::channel_loopback> (node1));
	ASSERT_EQ (0, node1.online_reps.online ());
	// After inserting to rep crawler
	{
		vxldollar::lock_guard<vxldollar::mutex> guard (node1.rep_crawler.probable_reps_mutex);
		node1.rep_crawler.active.insert (vxldollar::dev::genesis->hash ());
	}
	node1.vote_processor.vote_blocking (vote, std::make_shared<vxldollar::transport::channel_loopback> (node1));
	ASSERT_EQ (vxldollar::dev::constants.genesis_amount, node1.online_reps.online ());
}
...
TEST (node, online_reps_election)
{
	...
	auto send1 = builder.make_block ()
				 ...
				 .link (key.pub)
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				 .work (*node1.work_generate_blocking (vxldollar::dev::genesis->hash ()))
				 .build_shared ();
	node1.process_active (send1);
	node1.block_processor.flush ();
	node1.scheduler.flush ();
	...
	auto vote = std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, vxldollar::milliseconds_since_epoch (), 0, std::vector<vxldollar::block_hash>{ send1->hash () });
	ASSERT_EQ (0, node1.online_reps.online ());
	node1.vote_processor.vote_blocking (vote, std::make_shared<vxldollar::transport::channel_loopback> (node1));
	ASSERT_EQ (vxldollar::dev::constants.genesis_amount - vxldollar::Gxrb_ratio, node1.online_reps.online ());
}
...
TEST (node, block_confirm)
{
	...
	for (auto & type : types)
	{
		...
		vxldollar::system system (2, type, node_flags);
		auto & node1 (*system.nodes[0]);
		auto & node2 (*system.nodes[1]);
		vxldollar::keypair key;
		vxldollar::state_block_builder builder;
...
TEST (node, block_arrival_size)
{
	vxldollar::system system (1);
	auto & node (*system.nodes[0]);
	auto time (std::chrono::steady_clock::now () - vxldollar::block_arrival::arrival_time_min - std::chrono::seconds (5));
	vxldollar::block_hash hash (0);
	for (auto i (0); i < vxldollar::block_arrival::arrival_size_min * 2; ++i)
	{
		node.block_arrival.arrival.push_back (vxldollar::block_arrival_info{ time, hash });
		++hash.qwords[0];
	}
	ASSERT_EQ (vxldollar::block_arrival::arrival_size_min * 2, node.block_arrival.arrival.size ());
	node.block_arrival.recent (0);
	ASSERT_EQ (vxldollar::block_arrival::arrival_size_min, node.block_arrival.arrival.size ());
}
...
TEST (node, confirm_quorum)
{
	...
	vxldollar::amount new_balance = node1.online_reps.delta () - vxldollar::Gxrb_ratio;
	auto send1 = vxldollar::state_block_builder ()
				 ...
				 .build_shared ();
	ASSERT_EQ (vxldollar::process_result::progress, node1.process (*send1).code);
	system.wallet (0)->send_action (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.pub, new_balance.number ());
	ASSERT_TIMELY (2s, node1.active.election (send1->qualified_root ()));
	auto election = node1.active.election (send1->qualified_root ());
	ASSERT_NE (nullptr, election);
	ASSERT_FALSE (election->confirmed ());
	ASSERT_EQ (1, election->votes ().size ());
	ASSERT_EQ (0, node1.balance (vxldollar::dev::genesis_key.pub));
}
...
TEST (node, local_votes_cache)
{
	...
	vxldollar::node_config node_config (vxldollar::get_available_port (), system.logging);
	node_config.frontiers_confirmation = vxldollar::frontiers_confirmation_mode::disabled;
	node_config.receive_minimum = vxldollar::dev::constants.genesis_amount;
	auto & node (*system.add_node (node_config));
	vxldollar::state_block_builder builder;
	auto send1 = builder.make_block ()
				 .account (vxldollar::dev::genesis_key.pub)
	...
	auto send3 = builder.make_block ()
				 ...
				 .previous (send2->hash ())
				 .representative (vxldollar::dev::genesis_key.pub)
				 .balance (vxldollar::dev::constants.genesis_amount - 3 * vxldollar::Gxrb_ratio)
				 .link (vxldollar::dev::genesis_key.pub)
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
	...
	system.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	vxldollar::confirm_req message1{ vxldollar::dev::network_params.network, send1 };
	vxldollar::confirm_req message2{ vxldollar::dev::network_params.network, send2 };
	auto channel (node.network.udp_channels.create (node.network.endpoint ()));
	node.network.inbound (message1, channel);
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_generated_votes) == 1);
	node.network.inbound (message2, channel);
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_generated_votes) == 2);
	...
	vxldollar::confirm_req message3{ vxldollar::dev::network_params.network, send3 };
	for (auto i (0); i < 100; ++i)
	{
	...
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_generated_votes) == 3);
...
// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/vxldollarcurrency/vxldollar-node/pull/3532
// Issue for investigating it: https://github.com/vxldollarcurrency/vxldollar-node/issues/3481
TEST (node, DISABLED_local_votes_cache_batch)
{
	...
	auto send1 = vxldollar::state_block_builder ()
				 ...
				 .work (*node.work_generate_blocking (vxldollar::dev::genesis->hash ()))
				 .build_shared ();
	ASSERT_EQ (vxldollar::process_result::progress, node.ledger.process (node.store.tx_begin_write (), *send1).code);
	...
	auto send2 = vxldollar::state_block_builder ()
				 ...
				 .work (*node.work_generate_blocking (send1->hash ()))
				 .build_shared ();
	ASSERT_EQ (vxldollar::process_result::progress, node.ledger.process (node.store.tx_begin_write (), *send2).code);
	auto receive1 = vxldollar::state_block_builder ()
					.account (key1.pub)
					.previous (0)
					.representative (vxldollar::dev::genesis_key.pub)
					.balance (vxldollar::Gxrb_ratio)
					...
					.build_shared ();
	ASSERT_EQ (vxldollar::process_result::progress, node.ledger.process (node.store.tx_begin_write (), *receive1).code);
	std::vector<std::pair<vxldollar::block_hash, vxldollar::root>> batch{ { send2->hash (), send2->root () }, { receive1->hash (), receive1->root () } };
	vxldollar::confirm_req message{ vxldollar::dev::network_params.network, batch };
	auto channel (node.network.udp_channels.create (node.network.endpoint ()));
	// Generates and sends one vote for both hashes which is then cached
	node.network.inbound (message, channel);
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out) == 1);
	...
	node.network.inbound (message, channel);
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out) == 2);
	ASSERT_EQ (2, node.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out));
	// Test when votes are different
	node.history.erase (send2->root ());
	node.history.erase (receive1->root ());
	node.network.inbound (vxldollar::confirm_req{ vxldollar::dev::network_params.network, send2->hash (), send2->root () }, channel);
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out) == 3);
	ASSERT_EQ (3, node.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out));
	node.network.inbound (vxldollar::confirm_req{ vxldollar::dev::network_params.network, receive1->hash (), receive1->root () }, channel);
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out) == 4);
	ASSERT_EQ (4, node.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out));
	// There are two different votes, so both should be sent in response
	node.network.inbound (message, channel);
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out) == 6);
	ASSERT_EQ (6, node.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out));
}
...
TEST (node, local_votes_cache_generate_new_vote)
{
	...
	vxldollar::confirm_req message1{ vxldollar::dev::network_params.network, vxldollar::dev::genesis };
	auto channel (node.network.udp_channels.create (node.network.endpoint ()));
	node.network.inbound (message1, channel);
	ASSERT_TIMELY (3s, !node.history.votes (vxldollar::dev::genesis->root (), vxldollar::dev::genesis->hash ()).empty ());
	auto votes1 (node.history.votes (vxldollar::dev::genesis->root (), vxldollar::dev::genesis->hash ()));
	ASSERT_EQ (1, votes1.size ());
	ASSERT_EQ (1, votes1[0]->blocks.size ());
	ASSERT_EQ (vxldollar::dev::genesis->hash (), boost::get<vxldollar::block_hash> (votes1[0]->blocks[0]));
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_generated_votes) == 1);
	auto send1 = vxldollar::state_block_builder ()
				 .account (vxldollar::dev::genesis_key.pub)
				 .previous (vxldollar::dev::genesis->hash ())
	...
	std::vector<std::pair<vxldollar::block_hash, vxldollar::root>> roots_hashes{ std::make_pair (vxldollar::dev::genesis->hash (), vxldollar::dev::genesis->root ()), std::make_pair (send1->hash (), send1->root ()) };
	vxldollar::confirm_req message2{ vxldollar::dev::network_params.network, roots_hashes };
	node.network.inbound (message2, channel);
	...
	ASSERT_TIMELY (3s, node.stats.count (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_generated_votes) == 2);
	ASSERT_FALSE (node.history.votes (vxldollar::dev::genesis->root (), vxldollar::dev::genesis->hash ()).empty ());
	ASSERT_FALSE (node.history.votes (send1->root (), send1->hash ()).empty ());
	// First generated + again cached + new generated
	ASSERT_TIMELY (3s, 3 == node.stats.count (vxldollar::stat::type::message, vxldollar::stat::detail::confirm_ack, vxldollar::stat::dir::out));
}
...
TEST (node, local_votes_cache_fork)
{
	...
	auto send1 = vxldollar::state_block_builder ()
				 ...
				 .work (*node1.work_generate_blocking (vxldollar::dev::genesis->hash ()))
				 .build_shared ();
	auto send1_fork = vxldollar::state_block_builder ()
					  .account (vxldollar::dev::genesis_key.pub)
					  .previous (vxldollar::dev::genesis->hash ())
					  .representative (vxldollar::dev::genesis_key.pub)
					  .balance (vxldollar::dev::constants.genesis_amount - 2 * vxldollar::Gxrb_ratio)
	...
	ASSERT_EQ (vxldollar::process_result::progress, node1.process (*send1).code);
	// Cache vote
	auto vote (std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, 0, 0, std::vector<vxldollar::block_hash> (1, send1->hash ())));
	node1.vote_processor.vote (vote, std::make_shared<vxldollar::transport::channel_loopback> (node1));
...
TEST (node, vote_republish)
{
	...
	auto send2 = builder.make_block ()
				 .previous (vxldollar::dev::genesis->hash ())
				 .destination (key2.pub)
				 .balance (std::numeric_limits<vxldollar::uint128_t>::max () - node1.config.receive_minimum.number () * 2)
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				 .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
				 .build_shared ();
	node1.process_active (send1);
	...
	node1.process_active (send2);
	auto vote (std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, vxldollar::vote::timestamp_max, vxldollar::vote::duration_max, send2));
...
TEST (node, vote_by_hash_bundle)
{
	...
	auto & node = *system.nodes[0];
	vxldollar::state_block_builder builder;
	std::vector<std::shared_ptr<vxldollar::state_block>> blocks;
	...
	ASSERT_EQ (vxldollar::process_result::progress, node.ledger.process (node.store.tx_begin_write (), *blocks.back ()).code);
	for (auto i = 2; i < 200; ++i)
	{
		auto block = builder.make_block ()
					 .from (*blocks.back ())
					 .previous (blocks.back ()->hash ())
					 .balance (vxldollar::dev::constants.genesis_amount - i)
					 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
	...
	system.nodes[0]->observers.vote.add ([&max_hashes] (std::shared_ptr<vxldollar::vote> const & vote_a, std::shared_ptr<vxldollar::transport::channel> const &, vxldollar::vote_code) {
...
TEST (node, vote_by_hash_republish)
{
	...
	auto vote = std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, vxldollar::vote::timestamp_max, vxldollar::vote::duration_max, vote_blocks); // Final vote for confirmation
	ASSERT_TRUE (node1.active.active (*send1));
...
// Issue for investigating it: https://github.com/vxldollarcurrency/vxldollar-node/issues/3638
TEST (node, DISABLED_vote_by_hash_epoch_block_republish)
{
	...
	auto send1 = vxldollar::send_block_builder ()
				 ...
				 .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
				 .build_shared ();
	auto epoch1 = vxldollar::state_block_builder ()
	...
	auto vote (std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, 0, 0, vote_blocks));
	ASSERT_TRUE (node1.active.active (*send1));
	ASSERT_TRUE (node2.active.active (*send1));
	node1.vote_processor.vote (vote, std::make_shared<vxldollar::transport::channel_loopback> (node1));
...
TEST (node, epoch_conflict_confirm)
{
	...
	auto send2 = builder.make_block ()
				 ...
				 .previous (send->hash ())
				 .representative (vxldollar::dev::genesis_key.pub)
				 .balance (vxldollar::dev::constants.genesis_amount - 2)
				 .link (open->hash ())
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
	...
	auto epoch_open = builder.make_block ()
					  ...
					  .representative (0)
					  .balance (0)
					  .link (node0->ledger.epoch_link (vxldollar::epoch::epoch_1))
	...
	ASSERT_EQ (vxldollar::process_result::progress, node0->process (*send).code);
	ASSERT_EQ (vxldollar::process_result::progress, node0->process (*send2).code);
	ASSERT_EQ (vxldollar::process_result::progress, node0->process (*open).code);
	...
	vxldollar::blocks_confirm (*node1, { change, epoch_open }, true /* forced */);
	ASSERT_TIMELY (3s, node1->block_confirmed (change->hash ()) && node1->block_confirmed (epoch_open->hash ()));
	// Start elections for node0
	vxldollar::blocks_confirm (*node0, { change, epoch_open });
	ASSERT_EQ (2, node0->active.size ());
	{
		vxldollar::lock_guard<vxldollar::mutex> lock (node0->active.mutex);
...
// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/vxldollarcurrency/vxldollar-node/pull/3526
// Issue for investigating it: https://github.com/vxldollarcurrency/vxldollar-node/issues/3527
TEST (node, DISABLED_fork_invalid_block_signature)
{
	...
	auto send2 = builder.make_block ()
				 ...
				 .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
				 .build_shared ();
	auto send2_corrupt (std::make_shared<vxldollar::send_block> (*send2));
	send2_corrupt->signature = vxldollar::signature (123);
	auto vote (std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, 0, 0, send2));
	auto vote_corrupt (std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, 0, 0, send2_corrupt));
	...
	ASSERT_TIMELY (5s, node1.stats.count (vxldollar::stat::type::rollback, vxldollar::stat::detail::all));
...
TEST (node, fork_election_invalid_block_signature)
{
	...
	auto send3 = builder.state ()
				 ...
				 .link (vxldollar::dev::genesis_key.pub)
				 .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
				 .sign (vxldollar::dev::genesis_key.prv, 0) // Invalid signature
				 .build_shared ();
	auto channel1 (node1.network.udp_channels.create (node1.network.endpoint ()));
	node1.network.inbound (vxldollar::publish{ vxldollar::dev::network_params.network, send1 }, channel1);
	ASSERT_TIMELY (5s, node1.active.active (send1->qualified_root ()));
	auto election (node1.active.election (send1->qualified_root ()));
	ASSERT_NE (nullptr, election);
	ASSERT_EQ (1, election->blocks ().size ());
	node1.network.inbound (vxldollar::publish{ vxldollar::dev::network_params.network, send3 }, channel1);
	node1.network.inbound (vxldollar::publish{ vxldollar::dev::network_params.network, send2 }, channel1);
...
TEST (node, block_processor_signatures)
{
	...
	system0.wallet (0)->insert_adhoc (vxldollar::dev::genesis_key.prv);
	vxldollar::block_hash latest (system0.nodes[0]->latest (vxldollar::dev::genesis_key.pub));
	vxldollar::state_block_builder builder;
	vxldollar::keypair key1;
	vxldollar::keypair key2;
	vxldollar::keypair key3;
	...
	auto send4 = builder.make_block ()
				 ...
				 .previous (send3->hash ())
				 .representative (vxldollar::dev::genesis_key.pub)
				 .balance (vxldollar::dev::constants.genesis_amount - 4 * vxldollar::Gxrb_ratio)
				 .link (key3.pub)
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
	...
	std::shared_ptr<vxldollar::block> send5 = builder.make_block ()
										 .account (vxldollar::dev::genesis_key.pub)
										 .previous (send3->hash ())
										 .representative (vxldollar::dev::genesis_key.pub)
										 .balance (vxldollar::dev::constants.genesis_amount - 5 * vxldollar::Gxrb_ratio)
										 .link (key3.pub)
										 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
										 .work (*node1.work_generate_blocking (send3->hash ()))
										 .build_shared ();
...
TEST (node, block_processor_full)
{
	...
	auto & node = *system.add_node (vxldollar::node_config (vxldollar::get_available_port (), system.logging), node_flags);
	vxldollar::state_block_builder builder;
	auto send1 = builder.make_block ()
				 .account (vxldollar::dev::genesis_key.pub)
				 .previous (vxldollar::dev::genesis->hash ())
...
TEST (node, confirm_back)
{
	...
	vxldollar::keypair key;
	auto & node (*system.nodes[0]);
	auto genesis_start_balance (node.balance (vxldollar::dev::genesis_key.pub));
	auto send1 = vxldollar::send_block_builder ()
				 .previous (vxldollar::dev::genesis->hash ())
				 .destination (key.pub)
	...
	node.process_active (send2);
	vxldollar::blocks_confirm (node, { send1, open, send2 });
	ASSERT_EQ (3, node.active.size ());
	std::vector<vxldollar::block_hash> vote_blocks;
	vote_blocks.push_back (send2->hash ());
	auto vote (std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, vxldollar::vote::timestamp_max, vxldollar::vote::duration_max, vote_blocks));
	node.vote_processor.vote_blocking (vote, std::make_shared<vxldollar::transport::channel_loopback> (node));
	ASSERT_TIMELY (10s, node.active.empty ());
}
...
TEST (node, peers)
{
	...
	auto node2 (std::make_shared<vxldollar::node> (system.io_ctx, vxldollar::get_available_port (), vxldollar::unique_path (), system.logging, system.work));
	system.nodes.push_back (node2);

	auto endpoint = node1->network.endpoint ();
	vxldollar::endpoint_key endpoint_key{ endpoint.address ().to_v6 ().to_bytes (), endpoint.port () };
	auto & store = node2->store;
	{
		...
		store.peer.put (transaction, vxldollar::endpoint_key{ boost::asio::ip::address_v6::any ().to_bytes (), 55555 });
	}
...
TEST (node, peer_cache_restart)
{
	...
	auto path (vxldollar::unique_path ());
	{
		auto node2 (std::make_shared<vxldollar::node> (system.io_ctx, vxldollar::get_available_port (), path, system.logging, system.work));
		system.nodes.push_back (node2);
		auto & store = node2->store;
		{
			// Add a peer to the database
	...
	// Restart node
	{
		...
		auto node3 (std::make_shared<vxldollar::node> (system.io_ctx, vxldollar::get_available_port (), path, system.logging, system.work, node_flags));
...
TEST (node, unchecked_cleanup)
{
	...
	vxldollar::keypair key{};
	auto & node = *system.add_node (node_flags);
	auto open = vxldollar::state_block_builder ()
				.account (key.pub)
				.previous (0)
				.representative (key.pub)
				.balance (1)
...
TEST (node, dont_write_lock_node)
{
	...
	std::thread ([&path, &write_lock_held_promise, &finished_promise] () {
		...
		auto store = vxldollar::make_store (logger, path, vxldollar::dev::constants, false, true);
		{
	...
	vxldollar::inactive_node node (path, vxldollar::inactive_node_flag_defaults ());
...
TEST (node, aggressive_flooding)
{
	...
	auto & wallet1 (*system.wallet (0));
	wallet1.insert_adhoc (vxldollar::dev::genesis_key.prv);
	std::vector<std::pair<std::shared_ptr<vxldollar::node>, std::shared_ptr<vxldollar::wallet>>> nodes_wallets;
	bool const sanitizer_or_valgrind (is_sanitizer_build || vxldollar::running_within_valgrind ());
	...
	auto large_amount = (vxldollar::dev::constants.genesis_amount / 2) / nodes_wallets.size ();
	std::vector<std::shared_ptr<vxldollar::block>> genesis_blocks;
	for (auto & node_wallet : nodes_wallets)
	{
		...
		auto block (wallet1.send_action (vxldollar::dev::genesis_key.pub, keypair.pub, large_amount));
		ASSERT_NE (nullptr, block);
	...
	for (auto & node_wallet : nodes_wallets)
	{
		for (auto const & block : genesis_blocks)
		{
			...
			ASSERT_TRUE (vxldollar::process_result::progress == process_result.code || vxldollar::process_result::old == process_result.code);
		}
		ASSERT_EQ (node1.latest (vxldollar::dev::genesis_key.pub), node_wallet.first->latest (vxldollar::dev::genesis_key.pub));
		ASSERT_EQ (genesis_blocks.back ()->hash (), node_wallet.first->latest (vxldollar::dev::genesis_key.pub));
		// Confirm blocks for rep crawler & receiving
		vxldollar::blocks_confirm (*node_wallet.first, { genesis_blocks.back () }, true);
	}
	vxldollar::blocks_confirm (node1, { genesis_blocks.back () }, true);
	...
	vxldollar::state_block_builder builder;
	std::shared_ptr<vxldollar::state_block> block{};
	{
		auto transaction (node1.store.tx_begin_read ());
		block = builder.make_block ()
				.account (vxldollar::dev::genesis_key.pub)
				.representative (vxldollar::dev::genesis_key.pub)
				.previous (node1.ledger.latest (transaction, vxldollar::dev::genesis_key.pub))
				.balance (node1.ledger.account_balance (transaction, vxldollar::dev::genesis_key.pub) - 1)
				.link (vxldollar::dev::genesis_key.pub)
				.sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				.work (*node1.work_generate_blocking (node1.ledger.latest (transaction, vxldollar::dev::genesis_key.pub)))
				.build ();
	}
	// Processing locally goes through the aggressive block flooding path
	ASSERT_EQ (vxldollar::process_result::progress, node1.process_local (block).code);

	auto all_have_block = [&nodes_wallets] (vxldollar::block_hash const & hash_a) {
	...
	auto wallet_block = wallet1.send_sync (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.pub, 10);
...
TEST (node, rollback_vote_self)
{
	...
	auto send1 = builder.make_block ()
				 ...
				 .representative (vxldollar::dev::genesis_key.pub)
				 .link (key.pub)
				 .balance (vxldollar::dev::constants.genesis_amount - weight)
				 .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
				 .work (*system.work.generate (vxldollar::dev::genesis->hash ()))
				 .build_shared ();
	auto open = builder.make_block ()
	...
	auto vote (votes.find (vxldollar::dev::genesis_key.pub));
...
TEST (node, rollback_gap_source)
{
	...
	auto open = builder.make_block ()
				...
				.build_shared ();
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*send1).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*fork).code);
	...
	vxldollar::blocks_confirm (node, { fork });
	{
		...
		ASSERT_EQ (1, election->votes ().size ());
		// Confirm open
		auto vote1 (std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, vxldollar::vote::timestamp_max, vxldollar::vote::duration_max, std::vector<vxldollar::block_hash> (1, open->hash ())));
		node.vote_processor.vote (vote1, std::make_shared<vxldollar::transport::channel_loopback> (node));
	...
	ASSERT_TIMELY (5s, node.stats.count (vxldollar::stat::type::rollback, vxldollar::stat::detail::open) == 1);
	...
	ASSERT_TIMELY (5s, node.stats.count (vxldollar::stat::type::rollback, vxldollar::stat::detail::open) == 2);
...
TEST (node, dependency_graph)
{
	...
	auto key3_epoch = builder.make_block ()
					  ...
					  .link (node.ledger.epoch_link (vxldollar::epoch::epoch_1))
					  .balance (key3_receive->balance ())
					  .sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
					  .work (*system.work.generate (key3_receive->hash ()))
					  .build ();

	ASSERT_EQ (vxldollar::process_result::progress, node.process (*gen_send1).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*key1_open).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*key1_send1).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*gen_receive).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*gen_send2).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*key2_open).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*key2_send1).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*key3_open).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*key2_send2).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*key1_receive).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*key1_send2).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*key3_receive).code);
	ASSERT_EQ (vxldollar::process_result::progress, node.process (*key3_epoch).code);
	ASSERT_TRUE (node.active.empty ());

	// Hash -> Ancestors
	std::unordered_map<vxldollar::block_hash, std::vector<vxldollar::block_hash>> dependency_graph{
...
TEST (node, dependency_graph_frontier)
{
	...
	config.peering_port = vxldollar::get_available_port ();
	config.frontiers_confirmation = vxldollar::frontiers_confirmation_mode::always;
	...
	{
		auto transaction (node->store.tx_begin_write ());
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *gen_send1).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *key1_open).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *key1_send1).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *gen_receive).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *gen_send2).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *key2_open).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *key2_send1).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *key3_open).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *key2_send2).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *key1_receive).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *key1_send2).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *key3_receive).code);
		ASSERT_EQ (vxldollar::process_result::progress, node->ledger.process (transaction, *key3_epoch).code);
	}
...
TEST (node, deferred_dependent_elections)
{
	...
	auto fork = builder.make_block ()
				.from (*receive)
				.representative (vxldollar::dev::genesis_key.pub) // was key.pub
				.sign (key.prv, key.pub)
				.build_shared ();
	...
	node.work_generate_blocking (*open, vxldollar::dev::network_params.work.difficulty (*open) + 1);
	node.process_local (open);
	node.block_processor.flush ();
	node.scheduler.flush ();
	ASSERT_FALSE (node.active.active (open->qualified_root ()));
	...
	ASSERT_NO_ERROR (system.poll_until_true (5s, [&node, &send2] {
		vxldollar::unique_lock<vxldollar::mutex> lock{ node.active.mutex };
	...
	ASSERT_EQ (vxldollar::process_result::fork, node.process (*fork).code);
...
TEST (rep_crawler, recently_confirmed)
{
	...
	auto const block = vxldollar::dev::genesis;
...
TEST (rep_crawler, local)
{
	...
	flags.disable_rep_crawler = true;
	auto & node = *system.add_node (flags);
	auto loopback = std::make_shared<vxldollar::transport::channel_loopback> (node);
	auto vote = std::make_shared<vxldollar::vote> (vxldollar::dev::genesis_key.pub, vxldollar::dev::genesis_key.prv, 0, 0, std::vector{ vxldollar::dev::genesis->hash () });
	{
		vxldollar::lock_guard<vxldollar::mutex> guard (node.rep_crawler.probable_reps_mutex);
		node.rep_crawler.active.insert (vxldollar::dev::genesis->hash ());
...
TEST (node, pruning_automatic)
{
	...
	vxldollar::keypair key1{};
	vxldollar::send_block_builder builder{};
	auto latest_hash = vxldollar::dev::genesis->hash ();
	...
	node1.process_confirmed (vxldollar::election_status{ send1 });
	ASSERT_TIMELY (5s, node1.block_confirmed (send1->hash ()));
	node1.process_confirmed (vxldollar::election_status{ send2 });
	ASSERT_TIMELY (5s, node1.block_confirmed (send2->hash ()));
	...
	ASSERT_TRUE (node1.ledger.block_or_pruned_exists (vxldollar::dev::genesis->hash ()));
	ASSERT_TRUE (node1.ledger.block_or_pruned_exists (send1->hash ()));

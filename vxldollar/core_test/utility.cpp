...
TEST (rate, basic)
{
	vxldollar::rate::token_bucket bucket (10, 10);
...
TEST (rate, network)
{
	...
	vxldollar::rate::token_bucket bucket (10, 5);
...
TEST (rate, reset)
{
	vxldollar::rate::token_bucket bucket (0, 0);
...
TEST (optional_ptr, basic)
{
	...
	vxldollar::optional_ptr<valtype> opt;
	ASSERT_FALSE (opt);
...
TEST (thread, thread_pool)
{
	...
	vxldollar::thread_pool workers (1u, vxldollar::thread_role::name::unknown);
...
TEST (thread_pool_alarm, one)
{
	...
	vxldollar::unique_lock<vxldollar::mutex> unique (mutex);
	condition.wait (unique, [&] () { return !!done; });
}

TEST (thread_pool_alarm, many)
{
	vxldollar::thread_pool workers (50u, vxldollar::thread_role::name::unknown);
...
TEST (filesystem, remove_all_files)
{
	...
	vxldollar::remove_all_files_in_dir (path);
...
TEST (filesystem, move_all_files)
{
	...
	vxldollar::move_all_files_to_dir (dummy_directory, path);
...
{
	vxldollar::relaxed_atomic_integral<uint32_t> atomic{ 0 };
	ASSERT_EQ (0, atomic++);
	ASSERT_EQ (1, atomic);
	ASSERT_EQ (2, ++atomic);
	ASSERT_EQ (2, atomic);

...
#ifndef _WIN32
#ifdef VXLDOLLAR_STACKTRACE_BACKTRACE
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("config", boost::program_options::value<std::vector<vxldollar::config_key_value_pair>>()->multitoken(), "Pass node configuration values. This takes precedence over any values in the configuration file. This option can be repeated multiple times.")
		("rpcconfig", boost::program_options::value<std::vector<vxldollar::config_key_value_pair>>()->multitoken(), "Pass rpc configuration values. This takes precedence over any values in the configuration file. This option can be repeated multiple times.")
		...
		("debug_output_last_backtrace_dump", "Displays the contents of the latest backtrace in the event of a vxldollar_node crash")
		("debug_generate_crash_report", "Consolidates the vxldollar_node_backtrace.dump file. Requires addr2line installed on Linux")
		...
		("debug_profile_process", "Profile active blocks processing (only for vxldollar_dev_network)")
		("debug_profile_votes", "Profile votes processing (only for vxldollar_dev_network)")
		("debug_profile_frontiers_confirmation", "Profile frontiers confirmation speed (only for vxldollar_dev_network)")
	...
	vxldollar::add_node_flag_options (description);
	...
	if (network != vm.end ())
	{
		auto err (vxldollar::network_constants::set_active_network (network->second.as<std::string> ()));
		if (err)
		{
			std::cerr << vxldollar::network_constants::active_network_err_msg << std::endl;
			std::exit (1);
		}
	}
	...
	boost::filesystem::path data_path ((data_path_it != vm.end ()) ? data_path_it->second.as<std::string> () : vxldollar::working_path ());
	auto ec = vxldollar::handle_node_options (vm);
	if (ec == vxldollar::error_cli::unknown_command)
	{
		if (vm.count ("daemon") > 0)
		{
			vxldollar_daemon::daemon daemon;
			vxldollar::node_flags flags;
			auto flags_ec = vxldollar::update_flags (flags, vm);
			if (flags_ec)
			{
		...
		{
			if (vxldollar::network_constants::active_network != vxldollar::networks::vxldollar_dev_network)
			{
				auto node_flags = vxldollar::inactive_node_flag_defaults ();
				vxldollar::update_flags (node_flags, vm);
				node_flags.generate_cache.reps = true;
				vxldollar::inactive_node inactive_node (data_path, node_flags);
				...
				auto get_total = [] (decltype (bootstrap_weights.second) const & reps) -> vxldollar::uint128_union {
					return std::accumulate (reps.begin (), reps.end (), vxldollar::uint128_t{ 0 }, [] (auto sum, auto const & rep) { return sum + rep.second; });
				};
				...
				{
					std::vector<std::pair<vxldollar::account, vxldollar::uint128_t>> sorted;
				...
				struct mismatched_t
				{
					vxldollar::account rep;
					vxldollar::uint128_union hardcoded;
					vxldollar::uint128_union ledger;
					vxldollar::uint128_union diff;
					std::string get_entry () const
					{
						return boost::str (boost::format ("representative %1% hardcoded %2% ledger %3% mismatch %4%")
						% rep.to_account () % hardcoded.format_balance (vxldollar::Mxrb_ratio, 0, true) % ledger.format_balance (vxldollar::Mxrb_ratio, 0, true) % diff.format_balance (vxldollar::Mxrb_ratio, 0, true));
					}
				};
				...
				std::transform (hardcoded.begin (), hardcoded.end (), std::back_inserter (mismatched), [&ledger] (auto const & rep) {
					...
					vxldollar::uint128_t ledger_weight = (ledger_rep == ledger.end () ? 0 : ledger_rep->second);
				...
				vxldollar::uint128_union const mismatch_total = std::accumulate (mismatched.begin (), mismatched.end (), vxldollar::uint128_t{ 0 }, [] (auto sum, mismatched_t const & sample) { return sum + sample.diff.number (); });
				vxldollar::uint128_union const mismatch_mean = mismatch_total.number () / mismatched.size ();

				vxldollar::uint512_union mismatch_variance = std::accumulate (mismatched.begin (), mismatched.end (), vxldollar::uint512_t (0), [M = mismatch_mean.number (), N = mismatched.size ()] (vxldollar::uint512_t sum, mismatched_t const & sample) {
					auto x = sample.diff.number ();
					vxldollar::uint512_t const mean_diff = x > M ? x - M : M - x;
					vxldollar::uint512_t const sqr = mean_diff * mean_diff;
					return sum + sqr;
				})
				/ mismatched.size ();

				vxldollar::uint128_union const mismatch_stddev = vxldollar::narrow_cast<vxldollar::uint128_t> (boost::multiprecision::sqrt (mismatch_variance.number ()));

				auto const outlier_threshold = std::max (vxldollar::Gxrb_ratio, mismatch_mean.number () + 1 * mismatch_stddev.number ());
				...
				auto const newcomer_threshold = std::max (vxldollar::Gxrb_ratio, mismatch_mean.number ());
				std::vector<std::pair<vxldollar::account, vxldollar::uint128_t>> newcomers;
				...
				auto newcomer_entry = [] (auto const & rep) {
					return boost::str (boost::format ("representative %1% hardcoded --- ledger %2%") % rep.first.to_account () % vxldollar::uint128_union (rep.second).format_balance (vxldollar::Mxrb_ratio, 0, true));
				};

				std::cout << boost::str (boost::format ("hardcoded weight %1% Mvxldollar at %2% blocks\nledger weight %3% Mvxldollar at %4% blocks\nmismatched\n\tsamples %5%\n\ttotal %6% Mvxldollar\n\tmean %7% Mvxldollar\n\tsigma %8% Mvxldollar\n")
				% total_hardcoded.format_balance (vxldollar::Mxrb_ratio, 0, true)
				% hardcoded_height
				% total_ledger.format_balance (vxldollar::Mxrb_ratio, 0, true)
				% ledger_height
				% mismatched.size ()
				% mismatch_total.format_balance (vxldollar::Mxrb_ratio, 0, true)
				% mismatch_mean.format_balance (vxldollar::Mxrb_ratio, 0, true)
				% mismatch_stddev.format_balance (vxldollar::Mxrb_ratio, 0, true));
				...
				// Log more data
				auto const log_threshold = vxldollar::Gxrb_ratio;
		...
		{
			auto inactive_node = vxldollar::default_inactive_node (data_path, vm);
			...
			for (; i != end; ++i)
			{
				vxldollar::block_hash hash = i->first;
				vxldollar::block_w_sideband sideband = i->second;
				std::shared_ptr<vxldollar::block> b = sideband.block;
		...
		else if (vm.count ("debug_bootstrap_generate"))
		{
			...
			if (key_it != vm.end ())
			{
				vxldollar::uint256_union key;
				if (!key.decode_hex (key_it->second.as<std::string> ()))
				{
					vxldollar::keypair genesis (key.to_string ());
					vxldollar::work_pool work{ network_params.network, std::numeric_limits<unsigned>::max () };
					std::cout << "Genesis: " << genesis.prv.to_string () << "\n"
							  << "Public: " << genesis.pub.to_string () << "\n"
							  << "Account: " << genesis.pub.to_account () << "\n";
					vxldollar::keypair landing;
					...
					vxldollar::uint128_t balance (std::numeric_limits<vxldollar::uint128_t>::max ());
					vxldollar::open_block genesis_block (reinterpret_cast<vxldollar::block_hash const &> (genesis.pub), genesis.pub, genesis.pub, genesis.prv, genesis.pub, *work.generate (vxldollar::work_version::work_1, genesis.pub, network_params.work.epoch_1));
					std::cout << genesis_block.to_json ();
					std::cout.flush ();
					vxldollar::block_hash previous (genesis_block.hash ());
					for (auto i (0); i != 8; ++i)
					{
						vxldollar::uint128_t yearly_distribution (vxldollar::uint128_t (1) << (127 - (i == 7 ? 6 : i)));
						...
						for (auto j (0); j != 52; ++j)
						{
							...
							vxldollar::send_block send (previous, landing.pub, balance, genesis.prv, genesis.pub, *work.generate (vxldollar::work_version::work_1, previous, network_params.work.epoch_1));
							previous = send.hash ();
		...
		else if (vm.count ("debug_dump_representatives"))
		{
			...
			auto node = inactive_node.node;
			auto transaction (node->store.tx_begin_read ());
			vxldollar::uint128_t total;
			auto rep_amounts = node->ledger.cache.rep_weights.get_rep_amounts ();
			std::map<vxldollar::account, vxldollar::uint128_t> ordered_reps (rep_amounts.begin (), rep_amounts.end ());
		...
		else if (vm.count ("debug_dump_frontier_unchecked_dependents"))
		{
			...
			auto transaction (node->store.tx_begin_read ());
			std::unordered_set<vxldollar::block_hash> frontier_hashes;
		...
		{
			auto inactive_node = vxldollar::default_inactive_node (data_path, vm);
			vxldollar::uint256_union result;
			vxldollar::uint256_union salt (0);
			std::string password ("");
			while (true)
			{
		...
		{
			uint64_t difficulty{ vxldollar::work_thresholds::publish_full.base };
			...
			if (multiplier_it != vm.end ())
			{
				try
				{
					...
					difficulty = vxldollar::difficulty::from_multiplier (multiplier, difficulty);
				}
			...
			else
			{
				...
				{
					if (vxldollar::from_string_hex (difficulty_it->second.as<std::string> (), difficulty))
					{
			...
			vxldollar::work_pool work{ network_params.network, std::numeric_limits<unsigned>::max (), pow_rate_limiter };
			vxldollar::change_block block (0, 0, vxldollar::keypair ().prv, 0, 0);
			if (!result)
			{
				std::cerr << boost::str (boost::format ("Starting generation profiling. Difficulty: %1$#x (%2%x from base difficulty %3$#x)\n") % difficulty % vxldollar::to_string (vxldollar::difficulty::to_multiplier (difficulty, vxldollar::work_thresholds::publish_full.base), 4) % vxldollar::work_thresholds::publish_full.base);
				while (!result)
				{
					block.hashables.previous.qwords[0] += 1;
					auto begin1 (std::chrono::high_resolution_clock::now ());
					block.block_work_set (*work.generate (vxldollar::work_version::work_1, block.root (), difficulty));
					auto end1 (std::chrono::high_resolution_clock::now ());
				...
				if (!result)
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							vxldollar::opencl_config config (platform, device, threads);
							auto opencl (vxldollar::opencl_work::create (true, config, logger, network_params.work));
							vxldollar::work_pool work_pool{ network_params.network, 0, std::chrono::nanoseconds (0), opencl ? [&opencl] (vxldollar::work_version const version_a, vxldollar::root const & root_a, uint64_t difficulty_a, std::atomic<int> &) {
							...
							vxldollar::change_block block (0, 0, vxldollar::keypair ().prv, 0, 0);
							std::cerr << boost::str (boost::format ("Starting OpenCL generation profiling. Platform: %1%. Device: %2%. Threads: %3%. Difficulty: %4$#x (%5%x from base difficulty %6$#x)\n") % platform % device % threads % difficulty % vxldollar::to_string (vxldollar::difficulty::to_multiplier (difficulty, vxldollar::work_thresholds::publish_full.base), 4) % vxldollar::work_thresholds::publish_full.base);
							for (uint64_t i (0); true; ++i)
							{
								block.hashables.previous.qwords[0] += 1;
								auto begin1 (std::chrono::high_resolution_clock::now ());
								block.block_work_set (*work_pool.generate (vxldollar::work_version::work_1, block.root (), difficulty));
								auto end1 (std::chrono::high_resolution_clock::now ());
								std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
							}
						}
		...
		{
			if (boost::filesystem::exists ("vxldollar_node_backtrace.dump"))
			{
				// There is a backtrace, so output the contents
				std::ifstream ifs ("vxldollar_node_backtrace.dump");
		...
		else if (vm.count ("debug_generate_crash_report"))
		{
			if (boost::filesystem::exists ("vxldollar_node_backtrace.dump"))
			{
				...
				std::string crash_report_filename = "vxldollar_node_crash_report.txt";
				...
				ofs << st;
#else
				// Read all the vxldollar node files
				boost::system::error_code err;
				auto running_executable_filepath = boost::dll::program_location (err);
				if (!err)
				{
					auto num = 0;
					auto format = boost::format ("vxldollar_node_crash_load_address_dump_%1%.txt");
					...
					{
						std::ofstream ofs (crash_report_filename);
						vxldollar::set_secure_perm_file (crash_report_filename);
					}
					...
					// Now run using relative addresses. This will give actual results for other dlls, the results from the vxldollar_node executable.
					run_addr2line (true);

					if (std::find (system_codes.begin (), system_codes.end (), 0) == system_codes.end ())
					{
						std::cerr << "Error: Check that addr2line is installed and that vxldollar_node_crash_load_address_dump_*.txt files exist." << std::endl;
						result = -1;
					}
				}
				else
			...
			{
				std::cerr << "Error: vxldollar_node_backtrace.dump could not be found";
				result = -1;
			}
		}
		else if (vm.count ("debug_verify_profile"))
		{
			...
			vxldollar::uint256_union message;
			auto signature = vxldollar::sign_message (key.prv, key.pub, message);
			auto begin (std::chrono::high_resolution_clock::now ());
			for (auto i (0u); i < 1000; ++i)
			{
				vxldollar::validate_message (key.pub, message, signature);
			}
		...
		else if (vm.count ("debug_verify_profile_batch"))
		{
			...
			vxldollar::uint256_union message;
			vxldollar::uint512_union signature (vxldollar::sign_message (key.prv, key.pub, message));
			...
			auto begin (std::chrono::high_resolution_clock::now ());
			vxldollar::validate_message_batch (messages.data (), lengths.data (), pub_keys.data (), signatures.data (), batch_count, verifications.data ());
			auto end (std::chrono::high_resolution_clock::now ());
		...
		else if (vm.count ("debug_profile_sign"))
		{
			...
			{
				vxldollar::keypair key;
				vxldollar::block_hash latest (0);
				auto begin1 (std::chrono::high_resolution_clock::now ());
				for (uint64_t balance (0); balance < 1000; ++balance)
				{
					vxldollar::send_block send (latest, key.pub, balance, key.prv, key.pub, 0);
					latest = send.hash ();
				}
				auto end1 (std::chrono::high_resolution_clock::now ());
				std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
		...
		else if (vm.count ("debug_profile_process"))
		{
			...
			vxldollar::update_flags (node_flags, vm);
			vxldollar::inactive_node inactive_node (vxldollar::unique_path (), data_path, node_flags);
			auto node = inactive_node.node;

			vxldollar::block_hash genesis_latest (node->latest (vxldollar::dev::genesis_key.pub));
			vxldollar::uint128_t genesis_balance (std::numeric_limits<vxldollar::uint128_t>::max ());
			// Generating keys
			std::vector<vxldollar::keypair> keys (num_accounts);
			std::vector<vxldollar::root> frontiers (num_accounts);
			std::vector<vxldollar::uint128_t> balances (num_accounts, 1000000000);
			// Generating blocks
			std::deque<std::shared_ptr<vxldollar::block>> blocks;
			for (auto i (0); i != num_accounts; ++i)
			{
				...
				auto send = builder.state ()
							...
							.work (*node->work.generate (vxldollar::work_version::work_1, genesis_latest, node->network_params.work.epoch_1))
							.build ();
				...
				auto open = builder.state ()
							...
							.work (*node->work.generate (vxldollar::work_version::work_1, keys[i].pub, node->network_params.work.epoch_1))
							.build ();
			...
			for (auto i (0); i != num_iterations; ++i)
			{
				for (auto j (0); j != num_accounts; ++j)
				{
					...
					auto send = builder.state ()
								...
								.work (*node->work.generate (vxldollar::work_version::work_1, frontiers[j], node->network_params.work.epoch_1))
								.build ();
					...
					auto receive = builder.state ()
								   ...
								   .work (*node->work.generate (vxldollar::work_version::work_1, frontiers[other], node->network_params.work.epoch_1))
								   .build ();
			...
			vxldollar::timer<std::chrono::seconds> timer_l (vxldollar::timer_state::started);
		...
		else if (vm.count ("debug_profile_votes"))
		{
			...
			vxldollar::node_flags node_flags;
			vxldollar::update_flags (node_flags, vm);
			vxldollar::node_wrapper node_wrapper (vxldollar::unique_path (), data_path, node_flags);
			auto node = node_wrapper.node;

			vxldollar::block_hash genesis_latest (node->latest (vxldollar::dev::genesis_key.pub));
			vxldollar::uint128_t genesis_balance (std::numeric_limits<vxldollar::uint128_t>::max ());
			// Generating keys
			std::vector<vxldollar::keypair> keys (num_representatives);
			vxldollar::uint128_t balance ((node->config.online_weight_minimum.number () / num_representatives) + 1);
			...
			// Generating votes
			std::deque<std::shared_ptr<vxldollar::vote>> votes;
			for (auto j (0); j != num_representatives; ++j)
			{
				uint64_t sequence (1);
				for (auto & i : blocks)
				{
					auto vote (std::make_shared<vxldollar::vote> (keys[j].pub, keys[j].prv, sequence, 0, std::vector<vxldollar::block_hash> (1, i->hash ())));
		...
		else if (vm.count ("debug_profile_frontiers_confirmation"))
		{
			...
			auto path2 (vxldollar::unique_path ());
			logging.init (path1);
			std::vector<std::string> config_overrides;
			auto config (vm.find ("config"));
			if (config != vm.end ())
			{
				config_overrides = vxldollar::config_overrides (config->second.as<std::vector<vxldollar::config_key_value_pair>> ());
			}
			vxldollar::daemon_config daemon_config{ data_path, network_params };
			auto error = vxldollar::read_node_config_toml (data_path, daemon_config, config_overrides);

			vxldollar::node_config config1 = daemon_config.node;
			config1.peering_port = 24000;

			vxldollar::node_flags flags;
			vxldollar::update_flags (flags, vm);
			flags.disable_lazy_bootstrap = true;
			flags.disable_legacy_bootstrap = true;
			flags.disable_wallet_bootstrap = true;
			flags.disable_bootstrap_listener = true;
			auto node1 (std::make_shared<vxldollar::node> (io_ctx1, path1, config1, work, flags, 0));
			vxldollar::block_hash genesis_latest (node1->latest (vxldollar::dev::genesis_key.pub));
			vxldollar::uint128_t genesis_balance (std::numeric_limits<vxldollar::uint128_t>::max ());
			// Generating blocks
			std::deque<std::shared_ptr<vxldollar::block>> blocks;
			for (auto i (0); i != count; ++i)
			{
				...
				auto send = builder.state ()
							...
							.link (key.pub)
							.sign (vxldollar::dev::genesis_key.prv, vxldollar::dev::genesis_key.pub)
							.work (*work.generate (vxldollar::work_version::work_1, genesis_latest, vxldollar::dev::network_params.work.epoch_1))
							.build ();
				...
				auto open = builder.state ()
							...
							.work (*work.generate (vxldollar::work_version::work_1, key.pub, vxldollar::dev::network_params.work.epoch_1))
							.build ();
			...
			node1->start ();
			vxldollar::thread_runner runner1 (io_ctx1, node1->config.io_threads);
			...
			// Start new node
			vxldollar::node_config config2 = daemon_config.node;
			...
			auto node2 (std::make_shared<vxldollar::node> (io_ctx2, path2, config2, work, flags, 1));
			node2->start ();
			vxldollar::thread_runner runner2 (io_ctx2, node2->config.io_threads);
		...
		else if (vm.count ("debug_random_feed"))
		{
			 ...
			 *
			 *   ./vxldollar_node --debug_random_feed | dieharder -a -g 200
			 */
		...
		else if (vm.count ("debug_rpc"))
		{
			...
			vxldollar::inactive_node inactive_node_l (data_path, node_flags);

			vxldollar::node_rpc_config config;
			vxldollar::ipc::ipc_server server (*inactive_node_l.node, config);
			auto handler_l (std::make_shared<vxldollar::json_handler> (*inactive_node_l.node, config, command_l.str (), response_handler_l));
			handler_l->process_request ();
		}
		else if (vm.count ("validate_blocks") || vm.count ("debug_validate_blocks"))
		{
			vxldollar::timer<std::chrono::seconds> timer;
			timer.start ();
			auto node_flags = vxldollar::inactive_node_flag_defaults ();
			vxldollar::update_flags (node_flags, vm);
			node_flags.generate_cache.block_count = true;
			...
			std::deque<std::pair<vxldollar::account, vxldollar::account_info>> accounts;
			...
			auto print_error_message = [&silent, &errors] (std::string const & error_message_a) {
				if (!silent)
				{
					static vxldollar::mutex cerr_mutex;
					vxldollar::lock_guard<vxldollar::mutex> lock (cerr_mutex);
			...
			auto check_account = [&print_error_message, &silent, &count, &block_count] (std::shared_ptr<vxldollar::node> const & node, vxldollar::read_transaction const & transaction, vxldollar::account const & account, vxldollar::account_info const & info) {
				++count;
				...
				vxldollar::block_hash calculated_hash (0);
				...
				vxldollar::account calculated_representative{};
				while (!hash.is_zero () && block != nullptr)
				{
					...
					if (block->type () == vxldollar::block_type::open || block->type () == vxldollar::block_type::state)
					{
					...
					if (height == 0 && block->type () != vxldollar::block_type::open && block->type () != vxldollar::block_type::state)
					{
					...
					if (validate_message (account, hash, block->block_signature ()))
					{
						...
						{
							auto & state_block (static_cast<vxldollar::state_block &> (*block.get ()));
					...
					if (block->type () == vxldollar::block_type::open || block->type () == vxldollar::block_type::change || block->type () == vxldollar::block_type::state)
					{
			...
			count = 0;
			finished = false;
			std::deque<std::pair<vxldollar::pending_key, vxldollar::pending_info>> pending;

			auto check_pending = [&print_error_message, &silent, &count] (std::shared_ptr<vxldollar::node> const & node, vxldollar::read_transaction const & transaction, vxldollar::pending_key const & key, vxldollar::pending_info const & info) {
				++count;
				...
				else
				{
					...
					if (auto state = dynamic_cast<vxldollar::state_block *> (block.get ()))
					{
					...
					else if (auto send = dynamic_cast<vxldollar::send_block *> (block.get ()))
					{
		...
		else if (vm.count ("debug_profile_bootstrap"))
		{
			...
			vxldollar::update_flags (node_flags, vm);
			vxldollar::inactive_node node (vxldollar::unique_path (), node_flags);
			auto begin (std::chrono::high_resolution_clock::now ());
			uint64_t block_count (0);
			size_t count (0);
			std::deque<vxldollar::unchecked_info> epoch_open_blocks;
			{
				auto node_flags = vxldollar::inactive_node_flag_defaults ();
				vxldollar::update_flags (node_flags, vm);
				node_flags.generate_cache.block_count = true;
				...
				for (auto i (source_node->store.account.begin (transaction)), n (source_node->store.account.end ()); i != n; ++i)
				{
					...
					while (!hash.is_zero ())
					{
						...
						if (block != nullptr)
						{
							...
							vxldollar::unchecked_info unchecked_info (block, account, vxldollar::signature_verification::unknown);
							node.node->block_processor.add (unchecked_info);
							if (block->type () == vxldollar::block_type::state && block->previous ().is_zero () && source_node->ledger.is_epoch_link (block->link ()))
							{
			...
			vxldollar::remove_temporary_directories ();
		...
		else if (vm.count ("debug_peers"))
		{
			...
			{
				std::cout << boost::str (boost::format ("%1%\n") % vxldollar::endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ()));
			}
		}
		else if (vm.count ("debug_cemented_block_count"))
		{
			...
			vxldollar::update_flags (node_flags, vm);
			vxldollar::inactive_node node (data_path, node_flags);
		...
		{
#ifdef BOOST_WINDOWS
			if (!vxldollar::event_log_reg_entry_exists () && !vxldollar::is_windows_elevated ())
			{
				std::cerr << "The event log requires the HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Vxldollar\\Vxldollar registry entry, run again as administator to create it.\n";
				return 1;
			}
#endif
			auto inactive_node = vxldollar::default_inactive_node (data_path, vm);
			inactive_node->node->logger.always_log (vxldollar::severity_level::error, "Testing system logger");
		}
		else if (vm.count ("debug_account_versions"))
		{
			auto inactive_node = vxldollar::default_inactive_node (data_path, vm);
			auto node = inactive_node->node;
			auto const epoch_count = vxldollar::normalized_epoch (vxldollar::epoch::max) + static_cast<std::underlying_type<vxldollar::epoch>::type> (1);
			// Cache the accounts in a collection to make searching quicker against unchecked keys. Group by epoch
			vxldollar::locked<std::vector<boost::unordered_set<vxldollar::account>>> opened_account_versions_shared (epoch_count);
			using opened_account_versions_t = decltype (opened_account_versions_shared)::value_type;
			node->store.account.for_each_par (
			[&opened_account_versions_shared, epoch_count] (vxldollar::read_transaction const & /*unused*/, vxldollar::store_iterator<vxldollar::account, vxldollar::account_info> i, vxldollar::store_iterator<vxldollar::account, vxldollar::account_info> n) {
				// First cache locally
				...
				for (; i != n; ++i)
				{
					...
					auto epoch_idx = vxldollar::normalized_epoch (account_info.epoch ());
			...
			boost::unordered_set<vxldollar::account> opened_accounts;
			{
				auto opened_account_versions = opened_account_versions_shared.lock ();
			...
			vxldollar::locked<boost::unordered_map<vxldollar::account, std::underlying_type_t<vxldollar::epoch>>> unopened_highest_pending_shared;
			using unopened_highest_pending_t = decltype (unopened_highest_pending_shared)::value_type;
			node->store.pending.for_each_par (
			[&unopened_highest_pending_shared, &opened_accounts] (vxldollar::read_transaction const & /*unused*/, vxldollar::store_iterator<vxldollar::pending_key, vxldollar::pending_info> i, vxldollar::store_iterator<vxldollar::pending_key, vxldollar::pending_info> n) {
				// First cache locally
				...
				for (; i != n; ++i)
				{
					...
					if (!exists)
					{
						...
						auto epoch = vxldollar::normalized_epoch (info.epoch);
		...
		{
			std::cout << "Version " << VXLDOLLAR_VERSION_STRING << "\n"
		...
		else
		{
			...
			vxldollar::sort_options_description (description, sorted_description);

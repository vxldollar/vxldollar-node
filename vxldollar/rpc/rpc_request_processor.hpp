...
{
	ipc_connection (vxldollar::ipc::ipc_client && client_a, bool is_available_a) :
		client (std::move (client_a)), is_available (is_available_a)
	{
	}

	vxldollar::ipc::ipc_client client;
...
{
public:
	rpc_request_processor (boost::asio::io_context & io_ctx, vxldollar::rpc_config & rpc_config);
	rpc_request_processor (boost::asio::io_context & io_ctx, vxldollar::rpc_config & rpc_config, std::uint16_t ipc_port_a);
...
private:
	void run ();
	void read_payload (std::shared_ptr<vxldollar::ipc_connection> const & connection, std::shared_ptr<std::vector<uint8_t>> const & res, std::shared_ptr<vxldollar::rpc_request> const & rpc_request);
	void try_reconnect_and_execute_request (std::shared_ptr<vxldollar::ipc_connection> const & connection, vxldollar::shared_const_buffer const & req, std::shared_ptr<std::vector<uint8_t>> const & res, std::shared_ptr<vxldollar::rpc_request> const & rpc_request);
	void make_available (vxldollar::ipc_connection & connection);

	std::vector<std::shared_ptr<vxldollar::ipc_connection>> connections;
	vxldollar::mutex request_mutex;
	vxldollar::mutex connections_mutex;
	bool stopped{ false };
	std::deque<std::shared_ptr<vxldollar::rpc_request>> requests;
...
class ipc_rpc_processor final : public vxldollar::rpc_handler_interface
{
public:
	ipc_rpc_processor (boost::asio::io_context & io_ctx, vxldollar::rpc_config & rpc_config) :
		rpc_request_processor (io_ctx, rpc_config)
	{
	}
	ipc_rpc_processor (boost::asio::io_context & io_ctx, vxldollar::rpc_config & rpc_config, std::uint16_t ipc_port_a) :
	...
	{
		rpc_request_processor.add (std::make_shared<vxldollar::rpc_request> (action_a, body_a, response_a));
	}
	...
	void process_request_v2 (rpc_handler_request_params const & params_a, std::string const & body_a, std::function<void (std::shared_ptr<std::string> const &)> response_a) override
	{
		...
		rpc_request_processor.add (std::make_shared<vxldollar::rpc_request> (2 /* rpc version */, body_l, [response_a] (std::string const & resp) {
	...
	void rpc_instance (vxldollar::rpc & rpc) override
	{

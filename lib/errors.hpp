...
REGISTER_ERROR_CODES (vxldollar, error_common);
REGISTER_ERROR_CODES (vxldollar, error_blocks);
REGISTER_ERROR_CODES (vxldollar, error_rpc);
REGISTER_ERROR_CODES (vxldollar, error_process);
REGISTER_ERROR_CODES (vxldollar, error_config);
...
public:
	error () = default;
	error (vxldollar::error const & error_a) = default;
	error (vxldollar::error && error_a) = default;
	...
	error & operator= (vxldollar::error const & err_a);
	error & operator= (vxldollar::error && err_a);
	...
	error & then (std::function<vxldollar::error &()> next);
	...
	error & set (std::string message_a, std::error_code code_a = vxldollar::error_common::generic);
...
/**
 * A type that manages a vxldollar::error.
 * The default return type is vxldollar::error&, though shared_ptr<vxldollar::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = vxldollar::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, vxldollar::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<vxldollar::error>>::value, "Must be vxldollar::error& or shared_ptr<vxldollar::error>");

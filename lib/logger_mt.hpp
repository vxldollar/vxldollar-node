...
inline boost::log::formatting_ostream & operator<< (boost::log::formatting_ostream & strm, boost::log::to_log_manip<vxldollar::severity_level, severity_tag> const & manip)
{
	...
	vxldollar::severity_level level = manip.get ();
...
private:
	...
	void output (vxldollar::severity_level severity_level, LogItems &&... log_items)
	{
...
public:
	 ...
	 */
	template <typename... LogItems>
	void always_log (vxldollar::severity_level severity_level, LogItems &&... log_items)
	{
	...
	{
		always_log (vxldollar::severity_level::normal, std::forward<LogItems> (log_items)...);
	}
	 ...
	 */
	template <typename... LogItems>
	bool try_log (vxldollar::severity_level severity_level, LogItems &&... log_items)
	{
		auto error (true);
		auto time_now = std::chrono::steady_clock::now ();
		vxldollar::unique_lock<vxldollar::mutex> lk (last_log_time_mutex);
	...
	{
		return try_log (vxldollar::severity_level::normal, std::forward<LogItems> (log_items)...);
	}

	std::chrono::milliseconds min_log_delta{ 0 };

private:
	vxldollar::mutex last_log_time_mutex;

#include <vxldollar/boost/asio/post.hpp>
...
namespace
{
thread_local vxldollar::thread_role::name current_thread_role = vxldollar::thread_role::name::unknown;
}

vxldollar::thread_role::name vxldollar::thread_role::get ()
{
	return current_thread_role;
}

std::string vxldollar::thread_role::get_string (vxldollar::thread_role::name role)
{
	std::string thread_role_name_string;

	switch (role)
	{
		case vxldollar::thread_role::name::unknown:
			thread_role_name_string = "<unknown>";
			break;
		case vxldollar::thread_role::name::io:
			thread_role_name_string = "I/O";
			break;
		case vxldollar::thread_role::name::work:
			thread_role_name_string = "Work pool";
			break;
		case vxldollar::thread_role::name::packet_processing:
			thread_role_name_string = "Pkt processing";
			break;
		case vxldollar::thread_role::name::vote_processing:
			thread_role_name_string = "Vote processing";
			break;
		case vxldollar::thread_role::name::block_processing:
			thread_role_name_string = "Blck processing";
			break;
		case vxldollar::thread_role::name::request_loop:
			thread_role_name_string = "Request loop";
			break;
		case vxldollar::thread_role::name::wallet_actions:
			thread_role_name_string = "Wallet actions";
			break;
		case vxldollar::thread_role::name::bootstrap_initiator:
			thread_role_name_string = "Bootstrap init";
			break;
		case vxldollar::thread_role::name::bootstrap_connections:
			thread_role_name_string = "Bootstrap conn";
			break;
		case vxldollar::thread_role::name::voting:
			thread_role_name_string = "Voting";
			break;
		case vxldollar::thread_role::name::signature_checking:
			thread_role_name_string = "Signature check";
			break;
		case vxldollar::thread_role::name::rpc_request_processor:
			thread_role_name_string = "RPC processor";
			break;
		case vxldollar::thread_role::name::rpc_process_container:
			thread_role_name_string = "RPC process";
			break;
		case vxldollar::thread_role::name::confirmation_height_processing:
			thread_role_name_string = "Conf height";
			break;
		case vxldollar::thread_role::name::worker:
			thread_role_name_string = "Worker";
			break;
		case vxldollar::thread_role::name::request_aggregator:
			thread_role_name_string = "Req aggregator";
			break;
		case vxldollar::thread_role::name::state_block_signature_verification:
			thread_role_name_string = "State block sig";
			break;
		case vxldollar::thread_role::name::epoch_upgrader:
			thread_role_name_string = "Epoch upgrader";
			break;
		case vxldollar::thread_role::name::db_parallel_traversal:
			thread_role_name_string = "DB par traversl";
			break;
		case vxldollar::thread_role::name::election_scheduler:
			thread_role_name_string = "Election Sched";
			break;
		case vxldollar::thread_role::name::unchecked:
			thread_role_name_string = "Unchecked";
			break;
		default:
			debug_assert (false && "vxldollar::thread_role::get_string unhandled thread role");
	}
...
std::string vxldollar::thread_role::get_string ()
{
	return get_string (current_thread_role);
}

void vxldollar::thread_role::set (vxldollar::thread_role::name role)
{
	auto thread_role_name_string (get_string (role));

	vxldollar::thread_role::set_os_name (thread_role_name_string);

	current_thread_role = role;
}

void vxldollar::thread_attributes::set (boost::thread::attributes & attrs)
{
	auto attrs_l (&attrs);
	attrs_l->set_stack_size (8000000); //8MB
}

vxldollar::thread_runner::thread_runner (boost::asio::io_context & io_ctx_a, unsigned service_threads_a) :
		...
		threads.emplace_back (attrs, [&io_ctx_a] () {
			...
			try
			{
				...
				while (true)
				{
					...
					if (count == 1 && timer.since_start ().count () >= VXLDOLLAR_ASIO_HANDLER_TRACKING)
					{
...
vxldollar::thread_runner::~thread_runner ()
{
	join ();
}

void vxldollar::thread_runner::join ()
{
	io_guard.reset ();
	for (auto & i : threads)
	{
...
void vxldollar::thread_runner::stop_event_processing ()
{
	io_guard.get_executor ().context ().stop ();
}

vxldollar::thread_pool::thread_pool (unsigned num_threads, vxldollar::thread_role::name thread_name) :
...
vxldollar::thread_pool::~thread_pool ()
{
	stop ();
}

void vxldollar::thread_pool::stop ()
{
...
void vxldollar::thread_pool::push_task (std::function<void ()> task)
{
	++num_tasks;
...
void vxldollar::thread_pool::add_timed_task (std::chrono::steady_clock::time_point const & expiry_time, std::function<void ()> task)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
...
unsigned vxldollar::thread_pool::get_num_threads () const
{
	return num_threads;
}

uint64_t vxldollar::thread_pool::num_queued_tasks () const
{
	return num_tasks;
}

// Set the names of all the threads in the thread pool for easier identification
void vxldollar::thread_pool::set_thread_names (unsigned num_threads, vxldollar::thread_role::name thread_name)
{
	...
	for (auto i = 0u; i < num_threads; ++i)
	{
		boost::asio::post (*thread_pool_m, [&promise = promises[i], thread_name] () {
			vxldollar::thread_role::set (thread_name);
			promise.set_value ();
		});
	}
...
std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (thread_pool & thread_pool, std::string const & name)
{

...
std::string vxldollar::to_string (vxldollar::work_version const version_a)
{
	...
	switch (version_a)
	{
		case vxldollar::work_version::work_1:
			result = "work_1";
			break;
		case vxldollar::work_version::unspecified:
			result = "unspecified";
			break;
	}
	return result;
...
vxldollar::work_pool::work_pool (vxldollar::network_constants & network_constants, unsigned max_threads_a, std::chrono::nanoseconds pow_rate_limiter_a, std::function<boost::optional<uint64_t> (vxldollar::work_version const, vxldollar::root const &, uint64_t, std::atomic<int> &)> opencl_a) :
	...
	opencl (opencl_a)
{
	...
	for (auto i (0u); i < count; ++i)
	{
		threads.emplace_back (attrs, [this, i] () {
			vxldollar::thread_role::set (vxldollar::thread_role::name::work);
			vxldollar::work_thread_reprioritize ();
			loop (i);
		});
	}
}

vxldollar::work_pool::~work_pool ()
{
	stop ();
	for (auto & i : threads)
	{
...
void vxldollar::work_pool::loop (uint64_t thread)
{
	// Quick RNG for work attempts.
	xorshift1024star rng;
	vxldollar::random_pool::generate_block (reinterpret_cast<uint8_t *> (rng.s.data ()), rng.s.size () * sizeof (decltype (rng.s)::value_type));
	uint64_t work;
	uint64_t output;
	blake2b_state hash;
...
void vxldollar::work_pool::cancel (vxldollar::root const & root_a)
{
...
void vxldollar::work_pool::generate (vxldollar::work_version const version_a, vxldollar::root const & root_a, uint64_t difficulty_a, std::function<void (boost::optional<uint64_t> const &)> callback_a)
{
...
boost::optional<uint64_t> vxldollar::work_pool::generate (vxldollar::root const & root_a)
{
	debug_assert (network_constants.is_dev_network ());
	return generate (vxldollar::work_version::work_1, root_a, network_constants.work.base);
}

boost::optional<uint64_t> vxldollar::work_pool::generate (vxldollar::root const & root_a, uint64_t difficulty_a)
{
	debug_assert (network_constants.is_dev_network ());
	return generate (vxldollar::work_version::work_1, root_a, difficulty_a);
}

boost::optional<uint64_t> vxldollar::work_pool::generate (vxldollar::work_version const version_a, vxldollar::root const & root_a, uint64_t difficulty_a)
{
...
size_t vxldollar::work_pool::size ()
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	return pending.size ();
}

std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (work_pool & work_pool, std::string const & name)
{
	size_t count;
	{
		vxldollar::lock_guard<vxldollar::mutex> guard (work_pool.mutex);

...
#include <vxldollar/boost/asio/executor_work_guard.hpp>
#include <vxldollar/boost/asio/io_context.hpp>
#include <vxldollar/boost/asio/steady_timer.hpp>
#include <vxldollar/boost/asio/thread_pool.hpp>
...
namespace thread_role
{
	 ...
	 */
	vxldollar::thread_role::name get ();
	void set (vxldollar::thread_role::name);

	/*
	 * Get the thread name as a string from enum
	 */
	std::string get_string (vxldollar::thread_role::name);
...
{
public:
	explicit thread_pool (unsigned, vxldollar::thread_role::name);
	~thread_pool ();
...
private:
	...
	void set_thread_names (unsigned num_threads, vxldollar::thread_role::name thread_name);
};

std::unique_ptr<vxldollar::container_info_component> collect_container_info (thread_pool & thread_pool, std::string const & name);
}

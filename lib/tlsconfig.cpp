...
namespace vxldollar
{
vxldollar::error vxldollar::tls_config::serialize_toml (vxldollar::tomlconfig & toml) const
{
...
vxldollar::error vxldollar::tls_config::deserialize_toml (vxldollar::tomlconfig & toml)
{
...
namespace
{
	bool on_verify_certificate (bool preverified, boost::asio::ssl::verify_context & ctx, vxldollar::tls_config & config_a, vxldollar::logger_mt & logger_a)
	{
	...
	void load_certs (vxldollar::tls_config & config_a, vxldollar::logger_mt & logger_a)
	{
		try
		{
...
vxldollar::error read_tls_config_toml (boost::filesystem::path const & data_path_a, vxldollar::tls_config & config_a, vxldollar::logger_mt & logger_a, std::vector<std::string> const & config_overrides)
{
	vxldollar::error error;
	auto toml_config_path = vxldollar::get_tls_toml_config_path (data_path_a);
...
#else
		auto msg ("https or wss is enabled in the TLS configuration, but the node is not built with VXLDOLLAR_SECURE_RPC");

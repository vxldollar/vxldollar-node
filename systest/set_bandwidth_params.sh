...
# if VXLDOLLAR_NODE_EXE is unset ot empty then "../../build/vxldollar_node" is used
VXLDOLLAR_NODE_EXE=${VXLDOLLAR_NODE_EXE:-../../build/vxldollar_node}

mkdir -p "$DATADIR/log"
clean_data_dir

# start vxldollar_node and store its pid so we can later send it
# the SIGHUP signal and so we can terminate it
msg start vxldollar_node
$VXLDOLLAR_NODE_EXE --daemon --data_path "$DATADIR" >/dev/null &
pid=$!
msg pid=$pid
...
# send vxldollar_node the SIGHUP signal
kill -HUP $pid
...
# terminate vxldollar node and wait for it to die
kill $pid
wait $pid

# Vxldollar PoW Server
[![Build Status](https://travis-ci.org/vxldollarcurrency/vxldollar-pow-server.svg)](https://travis-ci.org/vxldollarcurrency/vxldollar-pow-server)

This project is a standalone work server for [vxldollar-pow](https://github.com/vxldollarcurrency/vxldollar-pow), the proof-of-work algorithm used by Vxldollar.

## Download

* [Linux](s3://repo.vxldollar.org/pow-server/vxldollar_pow_server-latest-Linux.tar.gz)
* [Mac](s3://repo.vxldollar.org/pow-server/vxldollar_pow_server-latest-Darwin.tar.gz)
* [Windows](s3://repo.vxldollar.org/pow-server/vxldollar_pow_server-latest-win64.tar.gz)
...
```
git clone --recursive https://github.com/vxldollarcurrency/vxldollar-pow-server.git
cd vxldollar-pow-server; mkdir build; cd build
cmake -DVXLDOLLAR_POW_STANDALONE=ON ..
make
```
...
```
./vxldollar_pow_server --config server.log_to_stderr=true
```
...
To override defaults using a config file, create a file called `vxldollar-pow-server.toml` and add the required keys and values under their respective TOML table. The file name can optionally be specified with the `--config_path` option (if not specified, the working directory is searched)

**A documented configuration file can be created with the --generate_config command**

**A documented configuration file is created in the build root when the vxldollar_pow_server target is created**
...
Authentication and access control is beyond the scope of the Vxldollar PoW Server. To expose functionality, consider using a reverse proxy; this can be used to control access to the REST, WebSocket and admin interfaces.

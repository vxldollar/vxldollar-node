...
class configbase : public vxldollar::error_aware<>
{
public:
	configbase () = default;
	configbase (std::shared_ptr<vxldollar::error> const & error_a) :
		error (error_a)
	{
	}

	/** Returns the current error */
	vxldollar::error & get_error () override
	{
		return *error;
	}
...
protected:
	...
	/** We're a vxldollar::error_aware type. Child nodes share the error state. */
	std::shared_ptr<vxldollar::error> error;

...
vxldollar::error vxldollar::rpc_secure_config::serialize_toml (vxldollar::tomlconfig & toml) const
{
...
vxldollar::error vxldollar::rpc_secure_config::deserialize_toml (vxldollar::tomlconfig & toml)
{
...
vxldollar::rpc_config::rpc_config (vxldollar::network_constants & network_constants) :
	rpc_process{ network_constants },
	address{ boost::asio::ip::address_v6::loopback ().to_string () }
{
}

vxldollar::rpc_config::rpc_config (vxldollar::network_constants & network_constants, uint16_t port_a, bool enable_control_a) :
	rpc_process{ network_constants },
	address{ boost::asio::ip::address_v6::loopback ().to_string () },
	port{ port_a },
...
vxldollar::error vxldollar::rpc_config::serialize_toml (vxldollar::tomlconfig & toml) const
{
	...
	vxldollar::tomlconfig rpc_process_l;
	...
	vxldollar::tomlconfig rpc_logging_l;
	rpc_logging_l.put ("log_rpc", rpc_logging.log_rpc, "Whether to log RPC calls.\ntype:bool");
	toml.put_child ("logging", rpc_logging_l);
	return toml.get_error ();
}

vxldollar::error vxldollar::rpc_config::deserialize_toml (vxldollar::tomlconfig & toml)
{
	if (!toml.empty ())
	{
		...
		if (rpc_secure_l)
		{
			return vxldollar::error ("The RPC secure configuration has moved to config-tls.toml. Please update the configuration.");
		}
...
vxldollar::rpc_process_config::rpc_process_config (vxldollar::network_constants & network_constants) :
...
namespace vxldollar
{
vxldollar::error read_rpc_config_toml (boost::filesystem::path const & data_path_a, vxldollar::rpc_config & config_a, std::vector<std::string> const & config_overrides)
{
	vxldollar::error error;
	auto toml_config_path = vxldollar::get_rpc_toml_config_path (data_path_a);
...
std::string get_default_rpc_filepath ()
{
	...
	// Construct the vxldollar_rpc executable file path based on where the currently running executable is found.

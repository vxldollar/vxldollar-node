...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    DISTRO_CFG="-DVXLDOLLAR_DISTRO_NAME=${distro}"
    ...
    CPACK_TYPE="TBZ2"
    DISTRO_CFG="-DVXLDOLLAR_DISTRO_NAME='freebsd'"
else
    CPACK_TYPE="TBZ2"
fi

if [[ ${SIMD} -eq 1 ]]; then
    SIMD_CFG="-DVXLDOLLAR_SIMD_OPTIMIZATIONS=ON"
...
if [[ "${BETA}" -eq 1 ]]; then
    NETWORK_CFG="-DACTIVE_NETWORK=vxldollar_beta_network"
    CONFIGURATION="RelWithDebInfo"
else
    NETWORK_CFG="-DACTIVE_NETWORK=vxldollar_live_network"

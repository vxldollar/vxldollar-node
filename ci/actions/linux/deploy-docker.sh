...
if [[ "$GITHUB_WORKFLOW" != "Develop" ]]; then
    ...
    ghcr_image_name="ghcr.io/${GITHUB_REPOSITORY}/vxldollar${network_tag_suffix}"
...
if [ -n "$DOCKER_PASSWORD" ]; then
    ...
    if [[ "$GITHUB_WORKFLOW" = "Develop" ]]; then
        "$scripts"/custom-timeout.sh 30 docker push "vxldollarcurrency/vxldollar-env:base"
        "$scripts"/custom-timeout.sh 30 docker push "vxldollarcurrency/vxldollar-env:gcc"
        "$scripts"/custom-timeout.sh 30 docker push "vxldollarcurrency/vxldollar-env:clang-6"
        echo "Deployed vxldollar-env"
        exit 0
    else
        if [[ "$GITHUB_WORKFLOW" = "Live" ]]; then
            tags=$(docker images --format '{{.Repository}}:{{.Tag }}' | grep vxldollarcurrency | grep -vE "env|ghcr.io|none|latest")
        else
            tags=$(docker images --format '{{.Repository}}:{{.Tag }}' | grep vxldollarcurrency | grep -vE "env|ghcr.io|none")
        fi
        for a in $tags; do

...
bool vxldollar_qt::eventloop_processor::event (QEvent * event_a)
{
	debug_assert (dynamic_cast<vxldollar_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<vxldollar_qt::eventloop_event *> (event_a)->action ();
	return true;
}

vxldollar_qt::eventloop_event::eventloop_event (std::function<void ()> const & action_a) :
	QEvent (QEvent::Type::User),
	action (action_a)
{
}

vxldollar_qt::self_pane::self_pane (vxldollar_qt::wallet & wallet_a, vxldollar::account const & account_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	self_layout (new QHBoxLayout),
	self_window (new QWidget),
	your_account_label (new QLabel ("Your Vxldollar account:")),
	...
	wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("%1% %2% network") % VXLDOLLAR_VERSION_STRING % network).c_str ());
...
void vxldollar_qt::self_pane::set_balance_text (std::pair<vxldollar::uint128_t, vxldollar::uint128_t> balance_a)
{
...
vxldollar_qt::accounts::accounts (vxldollar_qt::wallet & wallet_a) :
...
void vxldollar_qt::accounts::refresh ()
{
	...
	{
		vxldollar::public_key key (i->first);
...
vxldollar_qt::import::import (vxldollar_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
vxldollar_qt::history::history (vxldollar::ledger & ledger_a, vxldollar::account const & account_a, vxldollar_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	model (new QStandardItemModel),
	view (new QTableView),
...
namespace
{
class short_text_visitor : public vxldollar::block_visitor
{
public:
	short_text_visitor (vxldollar::transaction const & transaction_a, vxldollar::ledger & ledger_a) :
	...
	void open_block (vxldollar::open_block const & block_a)
	{
		...
		else
		{
			...
			amount = vxldollar::dev::constants.genesis_amount;
		}
	}
	...
	std::string type;
	vxldollar::uint128_t amount;
	vxldollar::account account;
};
}

void vxldollar_qt::history::refresh ()
{
...
vxldollar_qt::block_viewer::block_viewer (vxldollar_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
void vxldollar_qt::block_viewer::rebroadcast_action (vxldollar::block_hash const & hash_a)
{
	auto done (true);
...
vxldollar_qt::account_viewer::account_viewer (vxldollar_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this] () {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (VXLDOLLAR): ") + wallet.format_balance (balance.first));
...
vxldollar_qt::stats_viewer::stats_viewer (vxldollar_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	refresh (new QPushButton ("Refresh")),
...
void vxldollar_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
vxldollar_qt::status::status (vxldollar_qt::wallet & wallet_a) :
	wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (vxldollar_qt::status_types::nominal);
	set_text ();
}

void vxldollar_qt::status::erase (vxldollar_qt::status_types status_a)
{
	debug_assert (status_a != vxldollar_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void vxldollar_qt::status::insert (vxldollar_qt::status_types status_a)
{
	debug_assert (status_a != vxldollar_qt::status_types::nominal);
...
std::string vxldollar_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case vxldollar_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case vxldollar_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case vxldollar_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case vxldollar_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case vxldollar_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case vxldollar_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case vxldollar_qt::status_types::nominal:
...
std::string vxldollar_qt::status::color ()
{
	debug_assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
vxldollar_qt::wallet::wallet (QApplication & application_a, vxldollar_qt::eventloop_processor & processor_a, vxldollar::node & node_a, std::shared_ptr<vxldollar::wallet> const & wallet_a, vxldollar::account & account_a) :
	rendering_ratio (vxldollar::Mxrb_ratio),
	node (node_a),
	wallet_m (wallet_a),
	account (account_a),
...
void vxldollar_qt::wallet::ongoing_refresh ()
{
	std::weak_ptr<vxldollar_qt::wallet> wallet_w (shared_from_this ());
...
void vxldollar_qt::wallet::start ()
{
	ongoing_refresh ();
	std::weak_ptr<vxldollar_qt::wallet> this_w (shared_from_this ());
					...
					if (actual <= balance)
					{
						...
						if (this_l->wallet_m->store.valid_password (transaction))
						{
							...
							this_l->node.background ([this_w, account_l, actual] () {
								if (auto this_l = this_w.lock ())
								{
									this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w] (std::shared_ptr<vxldollar::block> const & block_a) {
										if (auto this_l = this_w.lock ())
										{
	...
	node.observers.blocks.add ([this_w] (vxldollar::election_status const & status_a, std::vector<vxldollar::vote_with_weight_info> const & votes_a, vxldollar::account const & account_a, vxldollar::uint128_t const & amount_a, bool, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w] (vxldollar::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a] () {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (vxldollar_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (vxldollar_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w] (std::shared_ptr<vxldollar::transport::channel> const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w] () {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a] () {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (vxldollar_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (vxldollar_qt::status_types::synchronizing);
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working] () {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (vxldollar_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (vxldollar_qt::status_types::working);
					}
				}
			}));
		}
...
void vxldollar_qt::wallet::refresh ()
{
	{
...
void vxldollar_qt::wallet::update_connected ()
{
	if (node.network.empty ())
	{
		active_status.insert (vxldollar_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (vxldollar_qt::status_types::disconnected);
	}
}

void vxldollar_qt::wallet::empty_password ()
{
...
void vxldollar_qt::wallet::change_rendering_ratio (vxldollar::uint128_t const & rendering_ratio_a)
{
...
//
std::string vxldollar_qt::wallet::format_balance (vxldollar::uint128_t const & balance) const
{
	auto balance_str = vxldollar::amount (balance).format_balance (rendering_ratio, 3, false);
	auto unit = std::string ("vxld");
	if (rendering_ratio == vxldollar::raw_ratio)
	{
		unit = std::string ("raw");
	}
...
void vxldollar_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void vxldollar_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

vxldollar_qt::settings::settings (vxldollar_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	password (new QLineEdit),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (change_rep, &QPushButton::released, [this] () {
		vxldollar::account representative_l;
...
void vxldollar_qt::settings::refresh_representative ()
{
...
void vxldollar_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void vxldollar_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (vxldollar_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (vxldollar_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (vxldollar_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (vxldollar_qt::status_types::vulnerable);
	}
}

vxldollar_qt::advanced_actions::advanced_actions (vxldollar_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	...
	vxldollar_unit (new QRadioButton ("vxld")), //
	...
	wallet (wallet_a)
{
	ratio_group->addButton (vxldollar_unit);
	ratio_group->setId (vxldollar_unit, ratio_group->buttons ().size () - 1);
	ratio_group->addButton (raw_unit);
	ratio_group->setId (raw_unit, ratio_group->buttons ().size () - 1);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (vxldollar_unit);
	...
	QObject::connect (vxldollar_unit, &QRadioButton::toggled, [this] () {
		if (vxldollar_unit->isChecked ())
		{
			this->wallet.change_rendering_ratio (vxldollar::Mxrb_ratio);
			QSettings ().setValue (saved_ratio_key, ratio_group->id (vxldollar_unit));
		}
	});
	QObject::connect (raw_unit, &QRadioButton::toggled, [this] () {
		if (raw_unit->isChecked ())
		{
			this->wallet.change_rendering_ratio (vxldollar::raw_ratio);
	...
	{
		selected_ratio_button = vxldollar_unit;
	}
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this] () {
		...
		auto error (vxldollar::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void vxldollar_qt::advanced_actions::refresh_peers ()
{
...
void vxldollar_qt::advanced_actions::refresh_ledger ()
{
	...
	for (auto i (wallet.node.ledger.store.account.begin (transaction)), j (wallet.node.ledger.store.account.end ()); i != j; ++i)
	{
		...
		std::string balance;
		vxldollar::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void vxldollar_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

vxldollar_qt::block_entry::block_entry (vxldollar_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	block (new QPlainTextEdit),
	status (new QLabel),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this] () {
		...
		try
		{
			...
			auto block_l (vxldollar::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
...
vxldollar_qt::block_creation::block_creation (vxldollar_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
void vxldollar_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void vxldollar_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void vxldollar_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void vxldollar_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void vxldollar_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
...
void vxldollar_qt::block_creation::create_send ()
{
	vxldollar::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		vxldollar::amount amount_l;
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						(void)error;
						debug_assert (!error);
						vxldollar::state_block send (account_l, info.head, info.representative, balance - amount_l.number (), destination_l, key, account_l, 0);
...
void vxldollar_qt::block_creation::create_receive ()
{
	vxldollar::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				vxldollar::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending.get (block_transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							vxldollar::state_block receive (pending_key.account, info.head, info.representative, info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
							vxldollar::block_details details;
...
void vxldollar_qt::block_creation::create_change ()
{
	vxldollar::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					vxldollar::state_block change (account_l, info.head, representative_l, info.balance, 0, key, account_l, 0);
					vxldollar::block_details details;
					details.epoch = info.epoch ();
...
void vxldollar_qt::block_creation::create_open ()
{
	vxldollar::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending.get (block_transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								vxldollar::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, key, pending_key.account, 0);
								vxldollar::block_details details;
								details.is_receive = true;

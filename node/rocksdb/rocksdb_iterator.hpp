...
namespace
{
inline bool is_read (vxldollar::transaction const & transaction_a)
{
	return (dynamic_cast<vxldollar::read_transaction const *> (&transaction_a) != nullptr);
}

inline rocksdb::ReadOptions & snapshot_options (vxldollar::transaction const & transaction_a)
{
...
public:
	...
	rocksdb_iterator (rocksdb::DB * db, vxldollar::transaction const & transaction_a, rocksdb::ColumnFamilyHandle * handle_a, rocksdb_val const * val_a, bool const direction_asc)
	{
	...
	rocksdb_iterator (rocksdb::DB * db, vxldollar::transaction const & transaction_a, rocksdb::ColumnFamilyHandle * handle_a) :
		rocksdb_iterator (db, transaction_a, handle_a, nullptr)
	{
	}

	rocksdb_iterator (vxldollar::rocksdb_iterator<T, U> && other_a)
	{
	...
	rocksdb_iterator (vxldollar::rocksdb_iterator<T, U> const &) = delete;
	...
	std::pair<vxldollar::rocksdb_val, vxldollar::rocksdb_val> * operator-> ()
	{
		return &current;
	}
	...
	{
		auto const other_a (boost::polymorphic_downcast<vxldollar::rocksdb_iterator<T, U> const *> (&base_a));
	...
	void clear ()
	{
		current.first = vxldollar::rocksdb_val{};
		current.second = vxldollar::rocksdb_val{};
		debug_assert (is_end_sentinal ());
	}
	vxldollar::rocksdb_iterator<T, U> & operator= (vxldollar::rocksdb_iterator<T, U> && other_a)
	{
	...
	std::pair<vxldollar::rocksdb_val, vxldollar::rocksdb_val> current;

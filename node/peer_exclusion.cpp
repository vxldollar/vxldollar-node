...
constexpr std::chrono::hours vxldollar::peer_exclusion::exclude_time_hours;
constexpr std::chrono::hours vxldollar::peer_exclusion::exclude_remove_hours;
constexpr std::size_t vxldollar::peer_exclusion::size_max;
constexpr double vxldollar::peer_exclusion::peers_percentage_limit;

uint64_t vxldollar::peer_exclusion::add (vxldollar::tcp_endpoint const & endpoint_a, std::size_t const network_peers_count_a)
{
	uint64_t result (0);
...
bool vxldollar::peer_exclusion::check (vxldollar::tcp_endpoint const & endpoint_a)
{
	bool excluded (false);
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	auto & peers_by_endpoint (peers.get<tag_endpoint> ());
...
void vxldollar::peer_exclusion::remove (vxldollar::tcp_endpoint const & endpoint_a)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	peers.get<tag_endpoint> ().erase (endpoint_a.address ());
}

std::size_t vxldollar::peer_exclusion::limited_size (std::size_t const network_peers_count_a) const
{
	return std::min (size_max, static_cast<std::size_t> (network_peers_count_a * peers_percentage_limit));
}

std::size_t vxldollar::peer_exclusion::size () const
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
	return peers.size ();
}

std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (vxldollar::peer_exclusion const & excluded_peers, std::string const & name)
{
	auto composite = std::make_unique<container_info_composite> (name);

	std::size_t excluded_peers_count = excluded_peers.size ();
	auto sizeof_excluded_peers_element = sizeof (vxldollar::peer_exclusion::ordered_endpoints::value_type);

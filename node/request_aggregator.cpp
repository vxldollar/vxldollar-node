...
vxldollar::request_aggregator::request_aggregator (vxldollar::node_config const & config_a, vxldollar::stat & stats_a, vxldollar::vote_generator & generator_a, vxldollar::vote_generator & final_generator_a, vxldollar::local_vote_history & history_a, vxldollar::ledger & ledger_a, vxldollar::wallets & wallets_a, vxldollar::active_transactions & active_a) :
	config{ config_a },
	...
	thread ([this] () { run (); })
{
	generator.set_reply_action ([this] (std::shared_ptr<vxldollar::vote> const & vote_a, std::shared_ptr<vxldollar::transport::channel> const & channel_a) {
		this->reply_action (vote_a, channel_a);
	});
	final_generator.set_reply_action ([this] (std::shared_ptr<vxldollar::vote> const & vote_a, std::shared_ptr<vxldollar::transport::channel> const & channel_a) {
		this->reply_action (vote_a, channel_a);
	});
...
void vxldollar::request_aggregator::add (std::shared_ptr<vxldollar::transport::channel> const & channel_a, std::vector<std::pair<vxldollar::block_hash, vxldollar::root>> const & hashes_roots_a)
{
	debug_assert (wallets.reps ().voting > 0);
	bool error = true;
	auto const endpoint (vxldollar::transport::map_endpoint_to_v6 (channel_a->get_endpoint ()));
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	...
	stats.inc (vxldollar::stat::type::aggregator, !error ? vxldollar::stat::detail::aggregator_accepted : vxldollar::stat::detail::aggregator_dropped);
}

void vxldollar::request_aggregator::run ()
{
	vxldollar::thread_role::set (vxldollar::thread_role::name::request_aggregator);
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	started = true;
	lock.unlock ();
	condition.notify_all ();
		...
		if (!requests.empty ())
		{
			...
			if (front->deadline < std::chrono::steady_clock::now ())
			{
				...
				if (!remaining.first.empty ())
				{
					...
					stats.add (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_cannot_vote, stat::dir::in, remaining.first.size () - generated);
				}
				if (!remaining.second.empty ())
				{
					...
					stats.add (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_cannot_vote, stat::dir::in, remaining.second.size () - generated);
				}
				lock.lock ();
			}
			else
...
void vxldollar::request_aggregator::stop ()
{
	{
...
std::size_t vxldollar::request_aggregator::size ()
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	return requests.size ();
}

bool vxldollar::request_aggregator::empty ()
{
	return size () == 0;
}

void vxldollar::request_aggregator::reply_action (std::shared_ptr<vxldollar::vote> const & vote_a, std::shared_ptr<vxldollar::transport::channel> const & channel_a) const
{
	vxldollar::confirm_ack confirm{ config.network_params.network, vote_a };
	channel_a->send (confirm);
}

void vxldollar::request_aggregator::erase_duplicates (std::vector<std::pair<vxldollar::block_hash, vxldollar::root>> & requests_a) const
{
...
std::pair<std::vector<std::shared_ptr<vxldollar::block>>, std::vector<std::shared_ptr<vxldollar::block>>> vxldollar::request_aggregator::aggregate (std::vector<std::pair<vxldollar::block_hash, vxldollar::root>> const & requests_a, std::shared_ptr<vxldollar::transport::channel> & channel_a) const
{
	auto transaction (ledger.store.tx_begin_read ());
	std::size_t cached_hashes = 0;
	std::vector<std::shared_ptr<vxldollar::block>> to_generate;
	std::vector<std::shared_ptr<vxldollar::block>> to_generate_final;
	std::vector<std::shared_ptr<vxldollar::vote>> cached_votes;
	for (auto const & [hash, root] : requests_a)
	{
		...
		else
		{
			...
			if (block)
			{
				...
				if (block->hash () != hash)
				{
					vxldollar::publish publish (config.network_params.network, block);
					channel_a->send (publish);
				}
			}
			else
			{
				stats.inc (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_unknown, stat::dir::in);
			}
		}
	}
	// Unique votes
	...
	stats.add (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_cached_hashes, stat::dir::in, cached_hashes);
	stats.add (vxldollar::stat::type::requests, vxldollar::stat::detail::requests_cached_votes, stat::dir::in, cached_votes.size ());
	return std::make_pair (to_generate, to_generate_final);
}

std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (vxldollar::request_aggregator & aggregator, std::string const & name)
{

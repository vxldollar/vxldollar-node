...
vxldollar::socket::socket (vxldollar::node & node_a, endpoint_type_t endpoint_type_a) :
	strand{ node_a.io_ctx.get_executor () },
	tcp_socket{ node_a.io_ctx },
	node{ node_a },
	endpoint_type_m{ endpoint_type_a },
	timeout{ std::numeric_limits<uint64_t>::max () },
	last_completion_time_or_init{ vxldollar::seconds_since_epoch () },
	last_receive_time_or_init{ vxldollar::seconds_since_epoch () },
...
vxldollar::socket::~socket ()
{
	close_internal ();
}

void vxldollar::socket::async_connect (vxldollar::tcp_endpoint const & endpoint_a, std::function<void (boost::system::error_code const &)> callback_a)
{
	...
	[this_l, callback = std::move (callback_a), endpoint_a] (boost::system::error_code const & ec) {
		if (ec)
		{
			this_l->node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_connect_error, vxldollar::stat::dir::in);
		}
		else
		{
...
void vxldollar::socket::async_read (std::shared_ptr<std::vector<uint8_t>> const & buffer_a, std::size_t size_a, std::function<void (boost::system::error_code const &, std::size_t)> callback_a)
{
		...
		if (!closed)
		{
			...
			boost::asio::post (strand, boost::asio::bind_executor (strand, [buffer_a, callback = std::move (callback_a), size_a, this_l] () mutable {
				...
				[this_l, buffer_a, cbk = std::move (callback)] (boost::system::error_code const & ec, std::size_t size_a) {
					if (ec)
					{
						this_l->node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_read_error, vxldollar::stat::dir::in);
					}
					else
					{
						this_l->node.stats.add (vxldollar::stat::type::traffic_tcp, vxldollar::stat::dir::in, size_a);
						this_l->set_last_completion ();
	...
	{
		debug_assert (false && "vxldollar::socket::async_read called with incorrect buffer size");
...
void vxldollar::socket::async_write (vxldollar::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, std::size_t)> callback_a)
{
	if (closed)
	{
		if (callback_a)
	...
	boost::asio::post (strand, boost::asio::bind_executor (strand, [buffer_a, callback = std::move (callback_a), this_l = shared_from_this ()] () mutable {
		...
		vxldollar::async_write (this_l->tcp_socket, buffer_a,
		boost::asio::bind_executor (this_l->strand,
		[buffer_a, cbk = std::move (callback), this_l] (boost::system::error_code ec, std::size_t size_a) {
			...
			if (ec)
			{
				this_l->node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_write_error, vxldollar::stat::dir::in);
			}
			else
			{
				this_l->node.stats.add (vxldollar::stat::type::traffic_tcp, vxldollar::stat::dir::out, size_a);
				this_l->set_last_completion ();
			}
...
void vxldollar::socket::set_default_timeout ()
{
 ...
 */
void vxldollar::socket::set_timeout (std::chrono::seconds timeout_a)
{
	timeout = timeout_a.count ();
}

void vxldollar::socket::set_last_completion ()
{
	last_completion_time_or_init = vxldollar::seconds_since_epoch ();
}

void vxldollar::socket::set_last_receive_time ()
{
	last_receive_time_or_init = vxldollar::seconds_since_epoch ();
}

void vxldollar::socket::checkup ()
{
	std::weak_ptr<vxldollar::socket> this_w (shared_from_this ());
	node.workers.add_timed_task (std::chrono::steady_clock::now () + std::chrono::seconds (2), [this_w] () {
		if (auto this_l = this_w.lock ())
		{
			uint64_t now (vxldollar::seconds_since_epoch ());
			...
			{
				this_l->node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_silent_connection_drop, vxldollar::stat::dir::in);
			...
			{
				this_l->node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_io_timeout_drop,
				this_l->endpoint_type () == endpoint_type_t::server ? vxldollar::stat::dir::in : vxldollar::stat::dir::out);
				condition_to_disconnect = true;
			}
...
bool vxldollar::socket::has_timed_out () const
{
	return timed_out;
}

void vxldollar::socket::set_default_timeout_value (std::chrono::seconds timeout_a)
{
	default_timeout = timeout_a;
}

void vxldollar::socket::set_silent_connection_tolerance_time (std::chrono::seconds tolerance_time_a)
{
	auto this_l (shared_from_this ());
...
void vxldollar::socket::close_internal ()
{
	if (!closed.exchange (true))
	{
		...
		if (ec)
		{
			...
			node.stats.inc (vxldollar::stat::type::bootstrap, vxldollar::stat::detail::error_socket_close);
		}
	}
}

vxldollar::tcp_endpoint vxldollar::socket::remote_endpoint () const
{
	return remote;
}

vxldollar::tcp_endpoint vxldollar::socket::local_endpoint () const
{
	return tcp_socket.local_endpoint ();
}

vxldollar::server_socket::server_socket (vxldollar::node & node_a, boost::asio::ip::tcp::endpoint local_a, std::size_t max_connections_a) :
...
void vxldollar::server_socket::start (boost::system::error_code & ec_a)
{
...
void vxldollar::server_socket::close ()
{
	auto this_l (std::static_pointer_cast<vxldollar::server_socket> (shared_from_this ()));
...
boost::asio::ip::network_v6 vxldollar::socket_functions::get_ipv6_subnet_address (boost::asio::ip::address_v6 const & ip_address, size_t network_prefix)
{
	return boost::asio::ip::make_network_v6 (ip_address, network_prefix);
}

boost::asio::ip::address vxldollar::socket_functions::first_ipv6_subnet_address (boost::asio::ip::address_v6 const & ip_address, size_t network_prefix)
{
...
boost::asio::ip::address vxldollar::socket_functions::last_ipv6_subnet_address (boost::asio::ip::address_v6 const & ip_address, size_t network_prefix)
{
	auto range = get_ipv6_subnet_address (ip_address, network_prefix).hosts ();
	debug_assert (!range.empty ());
	return *(--range.end ());
...
size_t vxldollar::socket_functions::count_subnetwork_connections (
vxldollar::address_socket_mmap const & per_address_connections,
...
bool vxldollar::server_socket::limit_reached_for_incoming_subnetwork_connections (std::shared_ptr<vxldollar::socket> const & new_connection)
{
	debug_assert (strand.running_in_this_thread ());
	if (node.flags.disable_max_peers_per_subnetwork || vxldollar::transport::is_ipv4_or_v4_mapped_address (new_connection->remote.address ()))
	{
...
bool vxldollar::server_socket::limit_reached_for_incoming_ip_connections (std::shared_ptr<vxldollar::socket> const & new_connection)
{
	debug_assert (strand.running_in_this_thread ());
...
void vxldollar::server_socket::on_connection (std::function<bool (std::shared_ptr<vxldollar::socket> const &, boost::system::error_code const &)> callback_a)
{
	auto this_l (std::static_pointer_cast<vxldollar::server_socket> (shared_from_this ()));
	...
	boost::asio::post (strand, boost::asio::bind_executor (strand, [this_l, callback = std::move (callback_a)] () mutable {
		...
		auto new_connection = std::make_shared<vxldollar::socket> (this_l->node, endpoint_type_t::server);
		...
		[this_l, new_connection, cbk = std::move (callback)] (boost::system::error_code const & ec_a) mutable {
			...
			if (this_l->connections_per_address.size () >= this_l->max_inbound_connections)
			{
				...
				this_l->node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_accept_failure, vxldollar::stat::dir::in);
			...
			if (this_l->limit_reached_for_incoming_ip_connections (new_connection))
			{
				...
				this_l->node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_max_per_ip, vxldollar::stat::dir::in);
				this_l->on_connection_requeue_delayed (std::move (cbk));
				return;
			}
			...
			if (this_l->limit_reached_for_incoming_subnetwork_connections (new_connection))
			{
				...
				% remote_ip_address.to_string ());
				this_l->node.logger.try_log (log_message);
				this_l->node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_max_per_subnetwork, vxldollar::stat::dir::in);
				this_l->on_connection_requeue_delayed (std::move (cbk));
				return;
			}
			...
			if (!ec_a)
			{
				...
				this_l->node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_accept_success, vxldollar::stat::dir::in);
...
void vxldollar::server_socket::on_connection_requeue_delayed (std::function<bool (std::shared_ptr<vxldollar::socket> const &, boost::system::error_code const &)> callback_a)
{
	auto this_l (std::static_pointer_cast<vxldollar::server_socket> (shared_from_this ()));
...
void vxldollar::server_socket::evict_dead_connections ()
{
	debug_assert (strand.running_in_this_thread ());

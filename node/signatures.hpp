...
private:
	...
	struct Task final
	{
		Task (vxldollar::signature_check_set & check, std::size_t pending) :
		...
		vxldollar::signature_check_set & check;
		std::atomic<std::size_t> pending;
	};

	bool verify_batch (vxldollar::signature_check_set const & check_a, std::size_t index, std::size_t size);
	void verify_async (vxldollar::signature_check_set & check_a, std::size_t num_batches, std::promise<void> & promise);

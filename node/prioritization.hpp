...
class prioritization final
{
	...
	std::vector<vxldollar::uint128_t> minimums;
...
public:
	...
	void push (uint64_t time, std::shared_ptr<vxldollar::block> block);
	std::shared_ptr<vxldollar::block> top () const;
	void pop ();
	...
	std::unique_ptr<vxldollar::container_info_component> collect_container_info (std::string const &);
};
}

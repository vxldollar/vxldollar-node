...
vxldollar::mdb_store::mdb_store (vxldollar::logger_mt & logger_a, boost::filesystem::path const & path_a, vxldollar::ledger_constants & constants, vxldollar::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a, vxldollar::lmdb_config const & lmdb_config_a, bool backup_before_upgrade_a) :
	// clang-format off
	store_partial{
		constants,
		block_store_partial,
	...
	logger (logger_a),
	env (error, path_a, vxldollar::mdb_env::options::make ().set_config (lmdb_config_a).set_use_no_mem_init (true)),
...
bool vxldollar::mdb_store::vacuum_after_upgrade (boost::filesystem::path const & path_a, vxldollar::lmdb_config const & lmdb_config_a)
{
	...
	if (vacuum_success)
	{
		...
		auto options = vxldollar::mdb_env::options::make ()
...
void vxldollar::mdb_store::serialize_mdb_tracker (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	mdb_txn_tracker.serialize_json (json, min_read_time, min_write_time);
}

void vxldollar::mdb_store::serialize_memory_stats (boost::property_tree::ptree & json)
{
	MDB_stat stats;
...
vxldollar::write_transaction vxldollar::mdb_store::tx_begin_write (std::vector<vxldollar::tables> const &, std::vector<vxldollar::tables> const &)
{
	return env.tx_begin_write (create_txn_callbacks ());
}

vxldollar::read_transaction vxldollar::mdb_store::tx_begin_read () const
{
	return env.tx_begin_read (create_txn_callbacks ());
}

std::string vxldollar::mdb_store::vendor_get () const
{
	return boost::str (boost::format ("LMDB %1%.%2%.%3%") % MDB_VERSION_MAJOR % MDB_VERSION_MINOR % MDB_VERSION_PATCH);
}

vxldollar::mdb_txn_callbacks vxldollar::mdb_store::create_txn_callbacks () const
{
	vxldollar::mdb_txn_callbacks mdb_txn_callbacks;
	if (txn_tracking_enabled)
	{
		mdb_txn_callbacks.txn_start = ([&mdb_txn_tracker = mdb_txn_tracker] (vxldollar::transaction_impl const * transaction_impl) {
			mdb_txn_tracker.add (transaction_impl);
		});
		mdb_txn_callbacks.txn_end = ([&mdb_txn_tracker = mdb_txn_tracker] (vxldollar::transaction_impl const * transaction_impl) {
...
void vxldollar::mdb_store::open_databases (bool & error_a, vxldollar::transaction const & transaction_a, unsigned flags)
{
...
bool vxldollar::mdb_store::do_upgrades (vxldollar::write_transaction & transaction_a, bool & needs_vacuuming)
{
	auto error (false);
	auto version_l = version.get (transaction_a);
	switch (version_l)
...
void vxldollar::mdb_store::upgrade_v14_to_v15 (vxldollar::write_transaction & transaction_a)
{
	logger.always_log ("Preparing v14 to v15 database upgrade...");

	std::vector<std::pair<vxldollar::account, vxldollar::account_info>> account_infos;
	...
	vxldollar::mdb_merge_iterator<vxldollar::account, vxldollar::account_info_v14> i_account (transaction_a, accounts_v0_handle, accounts_v1_handle);
	vxldollar::mdb_merge_iterator<vxldollar::account, vxldollar::account_info_v14> n_account{};
	for (; i_account != n_account; ++i_account)
	{
		vxldollar::account account (i_account->first);
		vxldollar::account_info_v14 account_info_v14 (i_account->second);
		...
		account_infos.emplace_back (account, vxldollar::account_info{ account_info_v14.head, rep_block->representative (), account_info_v14.open_block, account_info_v14.balance, account_info_v14.modified, account_info_v14.block_count, i_account.from_first_database ? vxldollar::epoch::epoch_0 : vxldollar::epoch::epoch_1 });
		// Move confirmation height from account_info database to its own table
		mdb_put (env.tx (transaction_a), confirmation_height_handle, vxldollar::mdb_val (account), vxldollar::mdb_val (account_info_v14.confirmation_height), MDB_APPEND);
	...
	for (auto const & account_account_info_pair : account_infos)
	{
		...
		mdb_put (env.tx (transaction_a), accounts_handle, vxldollar::mdb_val (account_account_info_pair.first), vxldollar::mdb_val (account_info), MDB_APPEND);
	}
	...
	vxldollar::mdb_merge_iterator<vxldollar::block_hash, vxldollar::state_block_w_sideband_v14> i_state (transaction_a, state_blocks_v0_handle, state_blocks_v1_handle);
	vxldollar::mdb_merge_iterator<vxldollar::block_hash, vxldollar::state_block_w_sideband_v14> n_state{};
	auto num = 0u;
	for (; i_state != n_state; ++i_state, ++num)
	{
		vxldollar::block_hash hash (i_state->first);
		vxldollar::state_block_w_sideband_v14 state_block_w_sideband_v14 (i_state->second);
		auto & sideband_v14 = state_block_w_sideband_v14.sideband;

		vxldollar::block_sideband_v18 sideband (sideband_v14.account, sideband_v14.successor, sideband_v14.balance, sideband_v14.height, sideband_v14.timestamp, i_state.from_first_database ? vxldollar::epoch::epoch_0 : vxldollar::epoch::epoch_1, false, false, false);
		...
		vxldollar::mdb_val value{ data.size (), (void *)data.data () };
		auto s = mdb_put (env.tx (transaction_a), state_blocks_new, vxldollar::mdb_val (hash), value, MDB_APPEND);
	...
	std::vector<std::pair<vxldollar::pending_key, vxldollar::pending_info>> pending_infos;
	pending_infos.reserve (pending_counters.before_v0 + pending_counters.before_v1);

	vxldollar::mdb_merge_iterator<vxldollar::pending_key, vxldollar::pending_info_v14> i_pending (transaction_a, pending_v0_handle, pending_v1_handle);
	vxldollar::mdb_merge_iterator<vxldollar::pending_key, vxldollar::pending_info_v14> n_pending{};
	for (; i_pending != n_pending; ++i_pending)
	{
		vxldollar::pending_info_v14 info (i_pending->second);
		pending_infos.emplace_back (vxldollar::pending_key (i_pending->first), vxldollar::pending_info{ info.source, info.amount, i_pending.from_first_database ? vxldollar::epoch::epoch_0 : vxldollar::epoch::epoch_1 });
	...
	{
		mdb_put (env.tx (transaction_a), pending_handle, vxldollar::mdb_val (pending_key_pending_info_pair.first), vxldollar::mdb_val (pending_key_pending_info_pair.second), MDB_APPEND);
	}
...
void vxldollar::mdb_store::upgrade_v15_to_v16 (vxldollar::write_transaction const & transaction_a)
{
...
void vxldollar::mdb_store::upgrade_v16_to_v17 (vxldollar::write_transaction const & transaction_a)
{
	...
	std::vector<std::pair<vxldollar::account, vxldollar::confirmation_height_info>> confirmation_height_infos;
	auto num = 0u;
	for (vxldollar::mdb_iterator<vxldollar::account, uint64_t> i (transaction_a, confirmation_height_handle), n (vxldollar::mdb_iterator<vxldollar::account, uint64_t>{}); i != n; ++i, ++account_info_i, ++num)
	{
		vxldollar::account account (i->first);
		...
		{
			confirmation_height_infos.emplace_back (account, confirmation_height_info{ 0, vxldollar::block_hash (0) });
		}
		else
		{
	...
	{
		mdb_put (env.tx (transaction_a), confirmation_height_handle, vxldollar::mdb_val (confirmation_height_info_pair.first), vxldollar::mdb_val (confirmation_height_info_pair.second), MDB_APPEND);
	}
...
void vxldollar::mdb_store::upgrade_v17_to_v18 (vxldollar::write_transaction const & transaction_a)
{
	...
	auto num = 0u;
	for (vxldollar::mdb_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::state_block>> state_i (transaction_a, state_blocks_handle), state_n{}; state_i != state_n; ++state_i, ++num)
	{
		vxldollar::block_w_sideband_v18<vxldollar::state_block> block_w_sideband (state_i->second);
		...
		vxldollar::block_sideband_v18 new_sideband (sideband.account, sideband.successor, sideband.balance, sideband.height, sideband.timestamp, sideband.details.epoch, is_send, is_receive, is_epoch);
		// Write these out
		std::vector<uint8_t> data;
		{
			vxldollar::vectorstream stream (data);
...
void vxldollar::mdb_store::upgrade_v18_to_v19 (vxldollar::write_transaction const & transaction_a)
{
	...
	std::map<vxldollar::block_hash, vxldollar::block_w_sideband> legacy_open_receive_change_blocks;

	for (auto i (vxldollar::store_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::change_block>> (std::make_unique<vxldollar::mdb_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::change_block>>> (transaction_a, change_blocks_handle))), n (vxldollar::store_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::change_block>> (nullptr)); i != n; ++i)
	{
		vxldollar::block_sideband_v18 const & old_sideband (i->second.sideband);
		vxldollar::block_sideband new_sideband (old_sideband.account, old_sideband.successor, old_sideband.balance, old_sideband.height, old_sideband.timestamp, vxldollar::epoch::epoch_0, false, false, false, vxldollar::epoch::epoch_0);
		legacy_open_receive_change_blocks[i->first] = { vxldollar::block_w_sideband{ i->second.block, new_sideband } };
	}

	for (auto i (vxldollar::store_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::open_block>> (std::make_unique<vxldollar::mdb_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::open_block>>> (transaction_a, open_blocks_handle))), n (vxldollar::store_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::open_block>> (nullptr)); i != n; ++i)
	{
		vxldollar::block_sideband_v18 const & old_sideband (i->second.sideband);
		vxldollar::block_sideband new_sideband (old_sideband.account, old_sideband.successor, old_sideband.balance, old_sideband.height, old_sideband.timestamp, vxldollar::epoch::epoch_0, false, false, false, vxldollar::epoch::epoch_0);
		legacy_open_receive_change_blocks[i->first] = { vxldollar::block_w_sideband{ i->second.block, new_sideband } };
	...
	for (auto i (vxldollar::store_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::receive_block>> (std::make_unique<vxldollar::mdb_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::receive_block>>> (transaction_a, receive_blocks_handle))), n (vxldollar::store_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::receive_block>> (nullptr)); i != n; ++i)
	{
		vxldollar::block_sideband_v18 const & old_sideband (i->second.sideband);
		vxldollar::block_sideband new_sideband (old_sideband.account, old_sideband.successor, old_sideband.balance, old_sideband.height, old_sideband.timestamp, vxldollar::epoch::epoch_0, false, false, false, vxldollar::epoch::epoch_0);
		legacy_open_receive_change_blocks[i->first] = { vxldollar::block_w_sideband{ i->second.block, new_sideband } };
	...
	MDB_dbi temp_legacy_open_receive_change_blocks;
	{
		...
		for (auto const & legacy_block : legacy_open_receive_change_blocks)
		{
			...
			{
				vxldollar::vectorstream stream (data);
				vxldollar::serialize_block (stream, *legacy_block.second.block);
				legacy_block.second.sideband.serialize (stream, legacy_block.second.block->type ());
			}

			vxldollar::mdb_val value{ data.size (), (void *)data.data () };
			auto s = mdb_put (env.tx (transaction_a), temp_legacy_open_receive_change_blocks, vxldollar::mdb_val (legacy_block.first), value, MDB_APPEND);
			release_assert_success (*this, s);
		}
	}
	...
	MDB_dbi temp_legacy_send_blocks;
	{
		...
		for (auto i (vxldollar::store_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::send_block>> (std::make_unique<vxldollar::mdb_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::send_block>>> (transaction_a, send_blocks_handle))), n (vxldollar::store_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::send_block>> (nullptr)); i != n; ++i)
		{
			...
			{
				vxldollar::vectorstream stream (data);
				vxldollar::serialize_block (stream, *block_w_sideband_v18.block);
				block_w_sideband_v18.sideband.serialize (stream, vxldollar::block_type::send); // Equal to new version for legacy blocks
			}

			vxldollar::mdb_val value{ data.size (), (void *)data.data () };
			auto s = mdb_put (env.tx (transaction_a), temp_legacy_send_blocks, vxldollar::mdb_val (i->first), value, MDB_APPEND);
			release_assert_success (*this, s);
		}
	}
	...
	MDB_dbi temp_legacy_send_open_receive_change_blocks;
	{
		...
		vxldollar::mdb_merge_iterator<vxldollar::block_hash, vxldollar::block_w_sideband> i (transaction_a, temp_legacy_open_receive_change_blocks, temp_legacy_send_blocks);
		vxldollar::mdb_merge_iterator<vxldollar::block_hash, vxldollar::block_w_sideband> n{};
		for (; i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), temp_legacy_send_open_receive_change_blocks, vxldollar::mdb_val (i->first), vxldollar::mdb_val (i->second), MDB_APPEND);
			release_assert_success (*this, s);
		}
	...
	{
		auto type_state (vxldollar::block_type::state);
		mdb_dbi_open (env.tx (transaction_a), "temp_state_blocks", MDB_CREATE, &temp_state_blocks);

		for (auto i (vxldollar::store_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::state_block>> (std::make_unique<vxldollar::mdb_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::state_block>>> (transaction_a, state_blocks_handle))), n (vxldollar::store_iterator<vxldollar::block_hash, vxldollar::block_w_sideband_v18<vxldollar::state_block>> (nullptr)); i != n; ++i)
		{
			auto const & block_w_sideband_v18 (i->second);
			vxldollar::block_sideband_v18 const & old_sideband (block_w_sideband_v18.sideband);
			...
			if (old_sideband.details.is_receive)
			{
				...
				{
					vxldollar::bufferstream stream (reinterpret_cast<uint8_t const *> (db_val.get ().data ()), db_val.get ().size ());
					auto source_block (vxldollar::deserialize_block (stream, type_state));
					release_assert (source_block != nullptr);
					vxldollar::block_sideband_v18 source_sideband;
					auto error (source_sideband.deserialize (stream, type_state));
					release_assert (!error);
					source_epoch = source_sideband.details.epoch;
				}
			}
			vxldollar::block_sideband new_sideband (old_sideband.account, old_sideband.successor, old_sideband.balance, old_sideband.height, old_sideband.timestamp, old_sideband.details.epoch, old_sideband.details.is_send, old_sideband.details.is_receive, old_sideband.details.is_epoch, source_epoch);

			std::vector<uint8_t> data;
			{
				vxldollar::vectorstream stream (data);
				vxldollar::serialize_block (stream, *block_w_sideband_v18.block);
				new_sideband.serialize (stream, vxldollar::block_type::state);
			}

			vxldollar::mdb_val value{ data.size (), (void *)data.data () };
			auto s = mdb_put (env.tx (transaction_a), temp_state_blocks, vxldollar::mdb_val (i->first), value, MDB_APPEND);
			release_assert_success (*this, s);
		}
	}
	...
	vxldollar::mdb_merge_iterator<vxldollar::block_hash, vxldollar::block_w_sideband> i (transaction_a, temp_legacy_send_open_receive_change_blocks, temp_state_blocks);
	vxldollar::mdb_merge_iterator<vxldollar::block_hash, vxldollar::block_w_sideband> n{};
	mdb_dbi_open (env.tx (transaction_a), "blocks", MDB_CREATE, &blocks_handle);
	for (; i != n; ++i)
	{
		auto s = mdb_put (env.tx (transaction_a), blocks_handle, vxldollar::mdb_val (i->first), vxldollar::mdb_val (i->second), MDB_APPEND);
		release_assert_success (*this, s);
	}
...
void vxldollar::mdb_store::upgrade_v19_to_v20 (vxldollar::write_transaction const & transaction_a)
{
...
void vxldollar::mdb_store::upgrade_v20_to_v21 (vxldollar::write_transaction const & transaction_a)
{
...
void vxldollar::mdb_store::create_backup_file (vxldollar::mdb_env & env_a, boost::filesystem::path const & filepath_a, vxldollar::logger_mt & logger_a)
{
...
vxldollar::unchecked_mdb_store::unchecked_mdb_store (vxldollar::mdb_store & mdb_store_a) :
	unchecked_store_partial<MDB_val, mdb_store> (mdb_store_a){};

bool vxldollar::mdb_store::exists (vxldollar::transaction const & transaction_a, tables table_a, vxldollar::mdb_val const & key_a) const
{
	vxldollar::mdb_val junk;
...
int vxldollar::mdb_store::get (vxldollar::transaction const & transaction_a, tables table_a, vxldollar::mdb_val const & key_a, vxldollar::mdb_val & value_a) const
{
	return mdb_get (env.tx (transaction_a), table_to_dbi (table_a), key_a, value_a);
}

int vxldollar::mdb_store::put (vxldollar::write_transaction const & transaction_a, tables table_a, vxldollar::mdb_val const & key_a, vxldollar::mdb_val const & value_a) const
{
	return (mdb_put (env.tx (transaction_a), table_to_dbi (table_a), key_a, value_a, 0));
}

int vxldollar::mdb_store::del (vxldollar::write_transaction const & transaction_a, tables table_a, vxldollar::mdb_val const & key_a) const
{
	return (mdb_del (env.tx (transaction_a), table_to_dbi (table_a), key_a, nullptr));
}

int vxldollar::mdb_store::drop (vxldollar::write_transaction const & transaction_a, tables table_a)
{
	return clear (transaction_a, table_to_dbi (table_a));
}

int vxldollar::mdb_store::clear (vxldollar::write_transaction const & transaction_a, MDB_dbi handle_a)
{
	return mdb_drop (env.tx (transaction_a), handle_a, 0);
}

uint64_t vxldollar::mdb_store::count (vxldollar::transaction const & transaction_a, tables table_a) const
{
	return count (transaction_a, table_to_dbi (table_a));
}

uint64_t vxldollar::mdb_store::count (vxldollar::transaction const & transaction_a, MDB_dbi db_a) const
{
	MDB_stat stats;
...
MDB_dbi vxldollar::mdb_store::table_to_dbi (tables table_a) const
{
	switch (table_a)
	{
...
bool vxldollar::mdb_store::not_found (int status) const
{
	return (status_code_not_found () == status);
}

bool vxldollar::mdb_store::success (int status) const
{
	return (MDB_SUCCESS == status);
}

int vxldollar::mdb_store::status_code_not_found () const
{
	return MDB_NOTFOUND;
}

std::string vxldollar::mdb_store::error_string (int status) const
{
	return mdb_strerror (status);
}

bool vxldollar::mdb_store::copy_db (boost::filesystem::path const & destination_file)
{
	return !mdb_env_copy2 (env.environment, destination_file.string ().c_str (), MDB_CP_COMPACT);
}

void vxldollar::mdb_store::rebuild_db (vxldollar::write_transaction const & transaction_a)
{
	...
	for (auto const & table : tables)
	{
		...
		for (auto i (vxldollar::store_iterator<vxldollar::uint256_union, vxldollar::mdb_val> (std::make_unique<vxldollar::mdb_iterator<vxldollar::uint256_union, vxldollar::mdb_val>> (transaction_a, table))), n (vxldollar::store_iterator<vxldollar::uint256_union, vxldollar::mdb_val> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), temp, vxldollar::mdb_val (i->first), i->second, MDB_APPEND);
			release_assert_success (*this, s);
		}
		...
		for (auto i (vxldollar::store_iterator<vxldollar::uint256_union, vxldollar::mdb_val> (std::make_unique<vxldollar::mdb_iterator<vxldollar::uint256_union, vxldollar::mdb_val>> (transaction_a, temp))), n (vxldollar::store_iterator<vxldollar::uint256_union, vxldollar::mdb_val> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), table, vxldollar::mdb_val (i->first), i->second, MDB_APPEND);
			release_assert_success (*this, s);
		}
		release_assert (count (transaction_a, table) == count (transaction_a, temp));
	...
	// Pending table
	{
		...
		mdb_dbi_open (env.tx (transaction_a), "temp_table", MDB_CREATE, &temp);
		// Copy all values to temporary table
		for (auto i (vxldollar::store_iterator<vxldollar::pending_key, vxldollar::pending_info> (std::make_unique<vxldollar::mdb_iterator<vxldollar::pending_key, vxldollar::pending_info>> (transaction_a, pending_handle))), n (vxldollar::store_iterator<vxldollar::pending_key, vxldollar::pending_info> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), temp, vxldollar::mdb_val (i->first), vxldollar::mdb_val (i->second), MDB_APPEND);
			release_assert_success (*this, s);
		}
		release_assert (count (transaction_a, pending_handle) == count (transaction_a, temp));
		mdb_drop (env.tx (transaction_a), pending_handle, 0);
		// Put values from copy
		for (auto i (vxldollar::store_iterator<vxldollar::pending_key, vxldollar::pending_info> (std::make_unique<vxldollar::mdb_iterator<vxldollar::pending_key, vxldollar::pending_info>> (transaction_a, temp))), n (vxldollar::store_iterator<vxldollar::pending_key, vxldollar::pending_info> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), pending_handle, vxldollar::mdb_val (i->first), vxldollar::mdb_val (i->second), MDB_APPEND);
			release_assert_success (*this, s);
		}
		release_assert (count (transaction_a, pending_handle) == count (transaction_a, temp));
		mdb_drop (env.tx (transaction_a), temp, 1);
...
bool vxldollar::mdb_store::init_error () const
{
	return error;
}

std::shared_ptr<vxldollar::block> vxldollar::mdb_store::block_get_v18 (vxldollar::transaction const & transaction_a, vxldollar::block_hash const & hash_a) const
{
	...
	if (value.size () != 0)
	{
		...
		release_assert (!error);
		result->sideband_set (vxldollar::block_sideband (sideband.account, sideband.successor, sideband.balance, sideband.height, sideband.timestamp, sideband.details.epoch, sideband.details.is_send, sideband.details.is_receive, sideband.details.is_epoch, vxldollar::epoch::epoch_0));
	}
	return result;
}

vxldollar::mdb_val vxldollar::mdb_store::block_raw_get_v18 (vxldollar::transaction const & transaction_a, vxldollar::block_hash const & hash_a, vxldollar::block_type & type_a) const
{
	vxldollar::mdb_val result;
	// Table lookups are ordered by match probability
	vxldollar::block_type block_types[]{ vxldollar::block_type::state, vxldollar::block_type::send, vxldollar::block_type::receive, vxldollar::block_type::open, vxldollar::block_type::change };
...
boost::optional<vxldollar::mdb_val> vxldollar::mdb_store::block_raw_get_by_type_v18 (vxldollar::transaction const & transaction_a, vxldollar::block_hash const & hash_a, vxldollar::block_type & type_a) const
{
	vxldollar::mdb_val value;
	vxldollar::mdb_val hash (hash_a);
	...
	boost::optional<vxldollar::mdb_val> result;
	if (success (status))
	{
		result = value;
	}
...
vxldollar::uint128_t vxldollar::mdb_store::block_balance_v18 (vxldollar::transaction const & transaction_a, vxldollar::block_hash const & hash_a) const
{
	auto block (block_get_v18 (transaction_a, hash_a));
	release_assert (block);
	vxldollar::uint128_t result (this->block.balance_calculated (block));
	return result;
}

// All the v14 functions below are only needed during upgrades
std::size_t vxldollar::mdb_store::block_successor_offset_v14 (vxldollar::transaction const & transaction_a, std::size_t entry_size_a, vxldollar::block_type type_a) const
{
	return entry_size_a - vxldollar::block_sideband_v14::size (type_a);
}

vxldollar::block_hash vxldollar::mdb_store::block_successor_v14 (vxldollar::transaction const & transaction_a, vxldollar::block_hash const & hash_a) const
{
	vxldollar::block_type type;
	...
	if (value.size () != 0)
	{
		...
		vxldollar::bufferstream stream (reinterpret_cast<uint8_t const *> (value.data ()) + block_successor_offset_v14 (transaction_a, value.size (), type), result.bytes.size ());
...
vxldollar::mdb_val vxldollar::mdb_store::block_raw_get_v14 (vxldollar::transaction const & transaction_a, vxldollar::block_hash const & hash_a, vxldollar::block_type & type_a, bool * is_state_v1) const
{
	vxldollar::mdb_val result;
	// Table lookups are ordered by match probability
	vxldollar::block_type block_types[]{ vxldollar::block_type::state, vxldollar::block_type::send, vxldollar::block_type::receive, vxldollar::block_type::open, vxldollar::block_type::change };
...
boost::optional<vxldollar::mdb_val> vxldollar::mdb_store::block_raw_get_by_type_v14 (vxldollar::transaction const & transaction_a, vxldollar::block_hash const & hash_a, vxldollar::block_type & type_a, bool * is_state_v1) const
{
	vxldollar::mdb_val value;
	vxldollar::mdb_val hash (hash_a);
	int status = status_code_not_found ();
...
std::shared_ptr<vxldollar::block> vxldollar::mdb_store::block_get_v14 (vxldollar::transaction const & transaction_a, vxldollar::block_hash const & hash_a, vxldollar::block_sideband_v14 * sideband_a, bool * is_state_v1) const
{
	vxldollar::block_type type;
...
vxldollar::mdb_store::upgrade_counters::upgrade_counters (uint64_t count_before_v0, uint64_t count_before_v1) :
	before_v0 (count_before_v0),
	before_v1 (count_before_v1)
{
}

bool vxldollar::mdb_store::upgrade_counters::are_equal () const
{
	return (before_v0 == after_v0) && (before_v1 == after_v1);
}

unsigned vxldollar::mdb_store::max_block_write_batch_num () const
{
	return std::numeric_limits<unsigned>::max ();
}

// Explicitly instantiate
template class vxldollar::store_partial<MDB_val, vxldollar::mdb_store>;

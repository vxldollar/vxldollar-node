...
{
public:
	explicit matches_txn (vxldollar::transaction_impl const * transaction_impl_a) :
		transaction_impl (transaction_impl_a)
	{
	}

	bool operator() (vxldollar::mdb_txn_stats const & mdb_txn_stats)
	{
...
vxldollar::read_mdb_txn::read_mdb_txn (vxldollar::mdb_env const & environment_a, vxldollar::mdb_txn_callbacks txn_callbacks_a) :
...
vxldollar::read_mdb_txn::~read_mdb_txn ()
{
...
void vxldollar::read_mdb_txn::reset ()
{
	mdb_txn_reset (handle);
	txn_callbacks.txn_end (this);
}

void vxldollar::read_mdb_txn::renew ()
{
...
void * vxldollar::read_mdb_txn::get_handle () const
{
	return handle;
}

vxldollar::write_mdb_txn::write_mdb_txn (vxldollar::mdb_env const & environment_a, vxldollar::mdb_txn_callbacks txn_callbacks_a) :
	env (environment_a),
	txn_callbacks (txn_callbacks_a)
{
	renew ();
...
vxldollar::write_mdb_txn::~write_mdb_txn ()
{
	commit ();
}

void vxldollar::write_mdb_txn::commit ()
{
	if (active)
	{
		auto status (mdb_txn_commit (handle));
...
void vxldollar::write_mdb_txn::renew ()
{
...
void * vxldollar::write_mdb_txn::get_handle () const
{
	return handle;
}

bool vxldollar::write_mdb_txn::contains (vxldollar::tables table_a) const
{
	// LMDB locks on every write
	return true;
}

vxldollar::mdb_txn_tracker::mdb_txn_tracker (vxldollar::logger_mt & logger_a, vxldollar::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a) :
	logger (logger_a),
...
void vxldollar::mdb_txn_tracker::serialize_json (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	...
	for (std::size_t i = 0; i < times_since_start.size (); ++i)
	{
		...
		{
			vxldollar::jsonconfig mdb_lock_config;
			...
			{
				vxldollar::jsonconfig frame_json;
			...
			vxldollar::jsonconfig stack (stacktrace_config);
...
void vxldollar::mdb_txn_tracker::log_if_held_long_enough (vxldollar::mdb_txn_stats const & mdb_txn_stats) const
{
	...
	bool is_blk_processing_thread = mdb_txn_stats.thread_name == vxldollar::thread_role::get_string (vxldollar::thread_role::name::block_processing);
...
void vxldollar::mdb_txn_tracker::add (vxldollar::transaction_impl const * transaction_impl)
{
	vxldollar::lock_guard<vxldollar::mutex> guard (mutex);
...
void vxldollar::mdb_txn_tracker::erase (vxldollar::transaction_impl const * transaction_impl)
{
...
vxldollar::mdb_txn_stats::mdb_txn_stats (vxldollar::transaction_impl const * transaction_impl) :
	transaction_impl (transaction_impl),
	thread_name (vxldollar::thread_role::get_string ()),
	stacktrace (std::make_shared<boost::stacktrace::stacktrace> ())
{
	timer.start ();
}

bool vxldollar::mdb_txn_stats::is_write () const
{
	return (dynamic_cast<vxldollar::write_transaction_impl const *> (transaction_impl) != nullptr);
}

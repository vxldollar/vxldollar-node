...
vxldollar::transport::channel_udp::channel_udp (vxldollar::transport::udp_channels & channels_a, vxldollar::endpoint const & endpoint_a, uint8_t protocol_version_a) :
...
std::size_t vxldollar::transport::channel_udp::hash_code () const
{
	std::hash<::vxldollar::endpoint> hash;
	return hash (endpoint);
}

bool vxldollar::transport::channel_udp::operator== (vxldollar::transport::channel const & other_a) const
{
	bool result (false);
	auto other_l (dynamic_cast<vxldollar::transport::channel_udp const *> (&other_a));
	if (other_l != nullptr)
	{
		return *this == *other_l;
	}
...
void vxldollar::transport::channel_udp::send_buffer (vxldollar::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, std::size_t)> const & callback_a, vxldollar::buffer_drop_policy drop_policy_a)
{
	set_last_packet_sent (std::chrono::steady_clock::now ());
	channels.send (buffer_a, endpoint, [node = std::weak_ptr<vxldollar::node> (channels.node.shared ()), callback_a] (boost::system::error_code const & ec, std::size_t size_a) {
		if (auto node_l = node.lock ())
		{
			if (ec == boost::system::errc::host_unreachable)
			{
				node_l->stats.inc (vxldollar::stat::type::error, vxldollar::stat::detail::unreachable_host, vxldollar::stat::dir::out);
			}
			if (size_a > 0)
			{
				node_l->stats.add (vxldollar::stat::type::traffic_udp, vxldollar::stat::dir::out, size_a);
			}
...
std::string vxldollar::transport::channel_udp::to_string () const
{
	return boost::str (boost::format ("%1%") % endpoint);
}

vxldollar::transport::udp_channels::udp_channels (vxldollar::node & node_a, uint16_t port_a, std::function<void (vxldollar::message const &, std::shared_ptr<vxldollar::transport::channel> const &)> sink) :
	node{ node_a },
	...
	sink{ std::move (sink) }
{
	...
	{
		socket = std::make_unique<boost::asio::ip::udp::socket> (node_a.io_ctx, vxldollar::endpoint (boost::asio::ip::address_v6::any (), port_a));
		...
		local_endpoint = vxldollar::endpoint (boost::asio::ip::address_v6::loopback (), port);
	}
	else
	{
		local_endpoint = vxldollar::endpoint (boost::asio::ip::address_v6::loopback (), 0);
		stopped = true;
	}
}

void vxldollar::transport::udp_channels::send (vxldollar::shared_const_buffer const & buffer_a, vxldollar::endpoint endpoint_a, std::function<void (boost::system::error_code const &, std::size_t)> const & callback_a)
{
...
std::shared_ptr<vxldollar::transport::channel_udp> vxldollar::transport::udp_channels::insert (vxldollar::endpoint const & endpoint_a, unsigned network_version_a)
{
	debug_assert (endpoint_a.address ().is_v6 ());
	std::shared_ptr<vxldollar::transport::channel_udp> result;
	if (!node.network.not_a_peer (endpoint_a, node.config.allow_local_peers) && (node.network_params.network.is_dev_network () || !max_ip_or_subnetwork_connections (endpoint_a)))
	{
		...
		{
			result = std::make_shared<vxldollar::transport::channel_udp> (*this, endpoint_a, network_version_a);
...
void vxldollar::transport::udp_channels::erase (vxldollar::endpoint const & endpoint_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	channels.get<endpoint_tag> ().erase (endpoint_a);
}

std::size_t vxldollar::transport::udp_channels::size () const
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	return channels.size ();
}

std::shared_ptr<vxldollar::transport::channel_udp> vxldollar::transport::udp_channels::channel (vxldollar::endpoint const & endpoint_a) const
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	std::shared_ptr<vxldollar::transport::channel_udp> result;
	auto existing (channels.get<endpoint_tag> ().find (endpoint_a));
...
std::unordered_set<std::shared_ptr<vxldollar::transport::channel>> vxldollar::transport::udp_channels::random_set (std::size_t count_a, uint8_t min_version) const
{
	std::unordered_set<std::shared_ptr<vxldollar::transport::channel>> result;
	...
	if (!channels.empty ())
	{
		...
		{
			auto index (vxldollar::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (peers_size - 1)));
...
void vxldollar::transport::udp_channels::random_fill (std::array<vxldollar::endpoint, 8> & target_a) const
{
...
std::shared_ptr<vxldollar::transport::channel_udp> vxldollar::transport::udp_channels::find_node_id (vxldollar::account const & node_id_a)
{
	std::shared_ptr<vxldollar::transport::channel_udp> result;
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
...
void vxldollar::transport::udp_channels::clean_node_id (vxldollar::account const & node_id_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	channels.get<node_id_tag> ().erase (node_id_a);
}

void vxldollar::transport::udp_channels::clean_node_id (vxldollar::endpoint const & endpoint_a, vxldollar::account const & node_id_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
...
vxldollar::tcp_endpoint vxldollar::transport::udp_channels::bootstrap_peer (uint8_t connection_protocol_version_min)
{
	...
	for (auto i (channels.get<last_bootstrap_attempt_tag> ().begin ()), n (channels.get<last_bootstrap_attempt_tag> ().end ()); i != n;)
	{
		...
		{
			result = vxldollar::transport::map_endpoint_to_tcp (i->endpoint ());
...
void vxldollar::transport::udp_channels::receive ()
{
	if (!stopped)
	{
		...
		socket->async_receive_from (boost::asio::buffer (data->buffer, vxldollar::network::buffer_size), data->endpoint,
...
void vxldollar::transport::udp_channels::start ()
{
...
void vxldollar::transport::udp_channels::stop ()
{
...
void vxldollar::transport::udp_channels::close_socket ()
{
...
vxldollar::endpoint vxldollar::transport::udp_channels::get_local_endpoint () const
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
...
namespace
{
class udp_message_visitor : public vxldollar::message_visitor
{
public:
	udp_message_visitor (vxldollar::node & node_a, vxldollar::endpoint endpoint_a, std::function<void (vxldollar::message const &, std::shared_ptr<vxldollar::transport::channel> const &)> sink) :
		node{ node_a },
	...
	void keepalive (vxldollar::keepalive const & message_a) override
	{
		if (!node.network.udp_channels.max_ip_or_subnetwork_connections (endpoint))
		{
			...
			if (cookie)
			{
				...
				else if (!node.network.tcp_channels.find_channel (vxldollar::transport::map_endpoint_to_tcp (endpoint)))
				{
					// Don't start connection if TCP channel to same IP:port exists
					find_channel = std::make_shared<vxldollar::transport::channel_udp> (node.network.udp_channels, endpoint, node.network_params.network.protocol_version);
					node.network.send_node_id_handshake (find_channel, *cookie, boost::none);
				}
			}
			...
			{
				vxldollar::endpoint new_endpoint (endpoint.address (), peer0.port ());
	...
	void telemetry_req (vxldollar::telemetry_req const & message_a) override
	{
		...
		if (find_channel)
		{
			...
			auto cache_exceeded = std::chrono::steady_clock::now () >= find_channel->get_last_telemetry_req () + vxldollar::telemetry_cache_cutoffs::network_to_time (node.network_params.network);
			if (is_very_first_message || cache_exceeded)
			{
				node.network.udp_channels.modify (find_channel, [] (std::shared_ptr<vxldollar::transport::channel_udp> const & channel_a) {
	...
	void node_id_handshake (vxldollar::node_id_handshake const & message_a) override
	{
		...
		boost::optional<vxldollar::uint256_union> out_query;
		boost::optional<vxldollar::uint256_union> out_respond_to;
		if (message_a.query)
		{
		...
		if (message_a.response)
		{
			if (!node.network.syn_cookies.validate (endpoint, message_a.response->first, message_a.response->second))
			{
				...
				if (message_a.response->first != node.node_id.pub && !node.network.tcp_channels.find_node_id (message_a.response->first))
				{
					...
					if (new_channel)
					{
						node.network.udp_channels.modify (new_channel, [&message_a] (std::shared_ptr<vxldollar::transport::channel_udp> const & channel_a) {
	...
	void message (vxldollar::message const & message_a)
	{
		auto find_channel (node.network.udp_channels.channel (endpoint));
		if (find_channel)
		{
...
void vxldollar::transport::udp_channels::receive_action (vxldollar::message_buffer * data_a)
{
	...
	else if (vxldollar::transport::reserved_address (data_a->endpoint, node.config.allow_local_peers))
	{
		allowed_sender = false;
	}
	if (allowed_sender)
	{
		...
		vxldollar::message_parser parser (node.network.publish_filter, node.block_uniquer, node.vote_uniquer, visitor, node.work, node.network_params.network);
		parser.deserialize_buffer (data_a->buffer, data_a->size);
		if (parser.status == vxldollar::message_parser::parse_status::success)
		{
			node.stats.add (vxldollar::stat::type::traffic_udp, vxldollar::stat::dir::in, data_a->size);
		}
		else if (parser.status == vxldollar::message_parser::parse_status::duplicate_publish_message)
		{
			node.stats.inc (vxldollar::stat::type::filter, vxldollar::stat::detail::duplicate_publish);
		}
		else
		{
			node.stats.inc (vxldollar::stat::type::error);
			...
			switch (parser.status)
			{
				...
				case vxldollar::message_parser::parse_status::invalid_header:
					node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::invalid_header);
					break;
				case vxldollar::message_parser::parse_status::invalid_message_type:
					node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::invalid_message_type);
					break;
				case vxldollar::message_parser::parse_status::invalid_keepalive_message:
					node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::invalid_keepalive_message);
					break;
				case vxldollar::message_parser::parse_status::invalid_publish_message:
					node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::invalid_publish_message);
					break;
				case vxldollar::message_parser::parse_status::invalid_confirm_req_message:
					node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::invalid_confirm_req_message);
					break;
				case vxldollar::message_parser::parse_status::invalid_confirm_ack_message:
					node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::invalid_confirm_ack_message);
					break;
				case vxldollar::message_parser::parse_status::invalid_node_id_handshake_message:
					node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::invalid_node_id_handshake_message);
					break;
				case vxldollar::message_parser::parse_status::invalid_telemetry_req_message:
					node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::invalid_telemetry_req_message);
					break;
				case vxldollar::message_parser::parse_status::invalid_telemetry_ack_message:
					node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::invalid_telemetry_ack_message);
					break;
				case vxldollar::message_parser::parse_status::outdated_version:
					node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::outdated_version);
					break;
	...
	else
	{
		...
		node.stats.inc_detail_only (vxldollar::stat::type::error, vxldollar::stat::detail::bad_sender);
	}
}

void vxldollar::transport::udp_channels::process_packets ()
{
	while (!stopped)
	{
...
std::shared_ptr<vxldollar::transport::channel> vxldollar::transport::udp_channels::create (vxldollar::endpoint const & endpoint_a)
{
	return std::make_shared<vxldollar::transport::channel_udp> (*this, endpoint_a, node.network_params.network.protocol_version);
}

bool vxldollar::transport::udp_channels::max_ip_connections (vxldollar::endpoint const & endpoint_a)
{
	if (node.flags.disable_max_peers_per_ip)
	{
		return false;
	}
	auto const address (vxldollar::transport::ipv4_address_or_ipv6_subnet (endpoint_a.address ()));
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	auto const result = channels.get<ip_address_tag> ().count (address) >= node.network_params.network.max_peers_per_ip;
	if (!result)
	{
		node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::udp_max_per_ip, vxldollar::stat::dir::out);
	}
	return result;
}

bool vxldollar::transport::udp_channels::max_subnetwork_connections (vxldollar::endpoint const & endpoint_a)
{
	if (node.flags.disable_max_peers_per_subnetwork)
	{
		return false;
	}
	auto const subnet (vxldollar::transport::map_address_to_subnetwork (endpoint_a.address ()));
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	auto const result = channels.get<subnetwork_tag> ().count (subnet) >= node.network_params.network.max_peers_per_subnetwork;
	if (!result)
	{
		node.stats.inc (vxldollar::stat::type::udp, vxldollar::stat::detail::udp_max_per_subnetwork, vxldollar::stat::dir::out);
	}
	return result;
}

bool vxldollar::transport::udp_channels::max_ip_or_subnetwork_connections (vxldollar::endpoint const & endpoint_a)
{
	return max_ip_connections (endpoint_a) || max_subnetwork_connections (endpoint_a);
}

bool vxldollar::transport::udp_channels::reachout (vxldollar::endpoint const & endpoint_a)
{
	// Don't overload single IP
	bool error = max_ip_or_subnetwork_connections (endpoint_a);
	if (!error && !node.flags.disable_udp)
	{
		auto endpoint_l (vxldollar::transport::map_endpoint_to_v6 (endpoint_a));
...
std::unique_ptr<vxldollar::container_info_component> vxldollar::transport::udp_channels::collect_container_info (std::string const & name)
{
...
void vxldollar::transport::udp_channels::purge (std::chrono::steady_clock::time_point const & cutoff_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
...
void vxldollar::transport::udp_channels::ongoing_keepalive ()
{
	vxldollar::keepalive message{ node.network_params.network };
	node.network.random_fill (message.peers);
	std::vector<std::shared_ptr<vxldollar::transport::channel_udp>> send_list;
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
...
void vxldollar::transport::udp_channels::list (std::deque<std::shared_ptr<vxldollar::transport::channel>> & deque_a, uint8_t minimum_version_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	// clang-format off
	vxldollar::transform_if (channels.get<random_access_tag> ().begin (), channels.get<random_access_tag> ().end (), std::back_inserter (deque_a),
...
void vxldollar::transport::udp_channels::modify (std::shared_ptr<vxldollar::transport::channel_udp> const & channel_a, std::function<void (std::shared_ptr<vxldollar::transport::channel_udp> const &)> modify_callback_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);

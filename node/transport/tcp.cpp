...
vxldollar::transport::channel_tcp::channel_tcp (vxldollar::node & node_a, std::weak_ptr<vxldollar::socket> socket_a) :
	channel (node_a),
	socket (std::move (socket_a))
{
}

vxldollar::transport::channel_tcp::~channel_tcp ()
{
...
std::size_t vxldollar::transport::channel_tcp::hash_code () const
{
	std::hash<::vxldollar::tcp_endpoint> hash;
	return hash (get_tcp_endpoint ());
}

bool vxldollar::transport::channel_tcp::operator== (vxldollar::transport::channel const & other_a) const
{
	bool result (false);
	auto other_l (dynamic_cast<vxldollar::transport::channel_tcp const *> (&other_a));
	if (other_l != nullptr)
	{
		return *this == *other_l;
	}
...
void vxldollar::transport::channel_tcp::send_buffer (vxldollar::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, std::size_t)> const & callback_a, vxldollar::buffer_drop_policy policy_a)
{
	if (auto socket_l = socket.lock ())
	{
		if (!socket_l->max () || (policy_a == vxldollar::buffer_drop_policy::no_socket_drop && !socket_l->full ()))
		{
			socket_l->async_write (
			buffer_a, [endpoint_a = socket_l->remote_endpoint (), node = std::weak_ptr<vxldollar::node> (node.shared ()), callback_a] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		{
			if (policy_a == vxldollar::buffer_drop_policy::no_socket_drop)
			{
				node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_write_no_socket_drop, vxldollar::stat::dir::out);
			}
			else
			{
				node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_write_drop, vxldollar::stat::dir::out);
			}
			if (callback_a)
			{
...
std::string vxldollar::transport::channel_tcp::to_string () const
{
	return boost::str (boost::format ("%1%") % get_tcp_endpoint ());
}

void vxldollar::transport::channel_tcp::set_endpoint ()
{
	vxldollar::lock_guard<vxldollar::mutex> lk (channel_mutex);
	debug_assert (endpoint == vxldollar::tcp_endpoint (boost::asio::ip::address_v6::any (), 0)); // Not initialized endpoint value
...
vxldollar::transport::tcp_channels::tcp_channels (vxldollar::node & node, std::function<void (vxldollar::message const &, std::shared_ptr<vxldollar::transport::channel> const &)> sink) :
	node{ node },
	sink{ std::move (sink) }
{
}

bool vxldollar::transport::tcp_channels::insert (std::shared_ptr<vxldollar::transport::channel_tcp> const & channel_a, std::shared_ptr<vxldollar::socket> const & socket_a, std::shared_ptr<vxldollar::bootstrap_server> const & bootstrap_server_a)
{
	auto endpoint (channel_a->get_tcp_endpoint ());
	debug_assert (endpoint.address ().is_v6 ());
	auto udp_endpoint (vxldollar::transport::map_tcp_to_endpoint (endpoint));
	bool error (true);
...
void vxldollar::transport::tcp_channels::erase (vxldollar::tcp_endpoint const & endpoint_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	channels.get<endpoint_tag> ().erase (endpoint_a);
}

std::size_t vxldollar::transport::tcp_channels::size () const
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	return channels.size ();
}

std::shared_ptr<vxldollar::transport::channel_tcp> vxldollar::transport::tcp_channels::find_channel (vxldollar::tcp_endpoint const & endpoint_a) const
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
...
std::unordered_set<std::shared_ptr<vxldollar::transport::channel>> vxldollar::transport::tcp_channels::random_set (std::size_t count_a, uint8_t min_version, bool include_temporary_channels_a) const
{
...
void vxldollar::transport::tcp_channels::random_fill (std::array<vxldollar::endpoint, 8> & target_a) const
{
...
bool vxldollar::transport::tcp_channels::store_all (bool clear_peers)
{
	...
	std::vector<vxldollar::endpoint> endpoints;
	{
		...
		std::back_inserter (endpoints), [] (auto const & channel) { return vxldollar::transport::map_tcp_to_endpoint (channel.endpoint ()); });
	}
	bool result (false);
	if (!endpoints.empty ())
	{
		...
		{
			node.store.peer.put (transaction, vxldollar::endpoint_key{ endpoint.address ().to_v6 ().to_bytes (), endpoint.port () });
		}
		result = true;
	}
	return result;
...
std::shared_ptr<vxldollar::transport::channel_tcp> vxldollar::transport::tcp_channels::find_node_id (vxldollar::account const & node_id_a)
{
	std::shared_ptr<vxldollar::transport::channel_tcp> result;
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
...
vxldollar::tcp_endpoint vxldollar::transport::tcp_channels::bootstrap_peer (uint8_t connection_protocol_version_min)
{
...
void vxldollar::transport::tcp_channels::process_messages ()
{
	while (!stopped)
	{
...
void vxldollar::transport::tcp_channels::process_message (vxldollar::message const & message_a, vxldollar::tcp_endpoint const & endpoint_a, vxldollar::account const & node_id_a, std::shared_ptr<vxldollar::socket> const & socket_a)
{
	auto type_a = socket_a->type ();
	if (!stopped && message_a.header.version_using >= node.network_params.network.protocol_version_min)
	{
		auto channel (node.network.find_channel (vxldollar::transport::map_tcp_to_endpoint (endpoint_a)));
		if (channel)
		{
		...
		else
		{
			...
			else if (!node.network.excluded_peers.check (endpoint_a))
			{
				if (!node_id_a.is_zero ())
				{
					...
					auto temporary_channel (std::make_shared<vxldollar::transport::channel_tcp> (node, socket_a));
					...
					debug_assert (type_a == vxldollar::socket::type_t::realtime || type_a == vxldollar::socket::type_t::realtime_response_server);
					// Don't insert temporary channels for response_server
					if (type_a == vxldollar::socket::type_t::realtime)
					{
				...
				else
				{
					...
					debug_assert (message_a.header.type == vxldollar::message_type::node_id_handshake);
...
void vxldollar::transport::tcp_channels::start ()
{
	ongoing_keepalive ();
}

void vxldollar::transport::tcp_channels::stop ()
{
	stopped = true;
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
...
bool vxldollar::transport::tcp_channels::max_ip_connections (vxldollar::tcp_endpoint const & endpoint_a)
{
	...
	if (result)
	{
		node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_max_per_ip, vxldollar::stat::dir::out);
	}
	return result;
}

bool vxldollar::transport::tcp_channels::max_subnetwork_connections (vxldollar::tcp_endpoint const & endpoint_a)
{
	...
	if (result)
	{
		node.stats.inc (vxldollar::stat::type::tcp, vxldollar::stat::detail::tcp_max_per_subnetwork, vxldollar::stat::dir::out);
	}
	return result;
}

bool vxldollar::transport::tcp_channels::max_ip_or_subnetwork_connections (vxldollar::tcp_endpoint const & endpoint_a)
{
	return max_ip_connections (endpoint_a) || max_subnetwork_connections (endpoint_a);
}

bool vxldollar::transport::tcp_channels::reachout (vxldollar::endpoint const & endpoint_a)
{
	auto tcp_endpoint (vxldollar::transport::map_endpoint_to_tcp (endpoint_a));
...
std::unique_ptr<vxldollar::container_info_component> vxldollar::transport::tcp_channels::collect_container_info (std::string const & name)
{
...
void vxldollar::transport::tcp_channels::purge (std::chrono::steady_clock::time_point const & cutoff_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
...
void vxldollar::transport::tcp_channels::ongoing_keepalive ()
{
	vxldollar::keepalive message{ node.network_params.network };
	node.network.random_fill (message.peers);
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	// Wake up channels
	std::vector<std::shared_ptr<vxldollar::transport::channel_tcp>> send_list;
	...
	vxldollar::tcp_endpoint invalid_endpoint (boost::asio::ip::address_v6::any (), 0);
	if (!node.network_params.network.is_dev_network () && !node.flags.disable_udp)
	{
		...
		for (auto i (0); i <= random_count; ++i)
		{
			...
			{
				start_tcp (vxldollar::transport::map_tcp_to_endpoint (tcp_endpoint));
			}
		}
	}
...
void vxldollar::transport::tcp_channels::list (std::deque<std::shared_ptr<vxldollar::transport::channel>> & deque_a, uint8_t minimum_version_a, bool include_temporary_channels_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	// clang-format off
...
void vxldollar::transport::tcp_channels::modify (std::shared_ptr<vxldollar::transport::channel_tcp> const & channel_a, std::function<void (std::shared_ptr<vxldollar::transport::channel_tcp> const &)> modify_callback_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
...
void vxldollar::transport::tcp_channels::update (vxldollar::tcp_endpoint const & endpoint_a)
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	auto existing (channels.get<endpoint_tag> ().find (endpoint_a));
	if (existing != channels.get<endpoint_tag> ().end ())
...
void vxldollar::transport::tcp_channels::start_tcp (vxldollar::endpoint const & endpoint_a)
{
	...
	auto socket = std::make_shared<vxldollar::client_socket> (node);
	std::weak_ptr<vxldollar::socket> socket_w (socket);
	auto channel (std::make_shared<vxldollar::transport::channel_tcp> (node, socket_w));
	std::weak_ptr<vxldollar::node> node_w (node.shared ());
	socket->async_connect (vxldollar::transport::map_endpoint_to_tcp (endpoint_a),
	[node_w, channel, socket, endpoint_a] (boost::system::error_code const & ec) {
		if (auto node_l = node_w.lock ())
		{
			if (!ec && channel)
			{
				...
				vxldollar::node_id_handshake message (node_l->network_params.network, cookie, boost::none);
...
void vxldollar::transport::tcp_channels::start_tcp_receive_node_id (std::shared_ptr<vxldollar::transport::channel_tcp> const & channel_a, vxldollar::endpoint const & endpoint_a, std::shared_ptr<std::vector<uint8_t>> const & receive_buffer_a)
{
	std::weak_ptr<vxldollar::node> node_w (node.shared ());
	if (auto socket_l = channel_a->socket.lock ())
	{
		auto cleanup_node_id_handshake_socket = [socket_w = channel_a->socket, node_w] (vxldollar::endpoint const & endpoint_a) {
			if (auto node_l = node_w.lock ())
			{
		...
		auto cleanup_and_udp_fallback = [socket_w = channel_a->socket, node_w, cleanup_node_id_handshake_socket] (vxldollar::endpoint const & endpoint_a) {
			if (auto node_l = node_w.lock ())
			{
				node_l->network.tcp_channels.udp_fallback (endpoint_a);
		...
		socket_l->async_read (receive_buffer_a, 8 + sizeof (vxldollar::account) + sizeof (vxldollar::account) + sizeof (vxldollar::signature), [node_w, channel_a, endpoint_a, receive_buffer_a, cleanup_and_udp_fallback, cleanup_node_id_handshake_socket] (boost::system::error_code const & ec, std::size_t size_a) {
			if (auto node_l = node_w.lock ())
			{
				if (!ec && channel_a)
				{
					node_l->stats.inc (vxldollar::stat::type::message, vxldollar::stat::detail::node_id_handshake, vxldollar::stat::dir::in);
					auto error (false);
					vxldollar::bufferstream stream (receive_buffer_a->data (), size_a);
					vxldollar::message_header header (error, stream);
					if (!error && header.type == vxldollar::message_type::node_id_handshake)
					{
						if (header.version_using >= node_l->network_params.network.protocol_version_min)
						{
							vxldollar::node_id_handshake message (error, stream, header);
							if (!error && message.response && message.query)
							{
								...
								if (process)
								{
									...
									boost::optional<std::pair<vxldollar::account, vxldollar::signature>> response (std::make_pair (node_l->node_id.pub, vxldollar::sign_message (node_l->node_id.prv, node_l->node_id.pub, *message.query)));
									vxldollar::node_id_handshake response_message (node_l->network_params.network, boost::none, response);
									if (node_l->config.logging.network_node_id_handshake_logging ())
									{
										...
										if (auto node_l = node_w.lock ())
										{
											if (!ec && channel_a)
											{
												...
												if (auto socket_l = channel_a->socket.lock ())
												{
													...
													auto response_server = std::make_shared<vxldollar::bootstrap_server> (socket_l, node_l);
													node_l->network.tcp_channels.insert (channel_a, socket_l, response_server);
													// Listen for possible responses
													response_server->socket->type_set (vxldollar::socket::type_t::realtime_response_server);
													...
													{
														node_l->telemetry->get_metrics_single_peer_async (channel_a, [] (vxldollar::telemetry_data_response const &) {
						...
						else
						{
							...
							cleanup_node_id_handshake_socket (endpoint_a);
							{
								vxldollar::lock_guard<vxldollar::mutex> lock (node_l->network.tcp_channels.mutex);
								node_l->network.tcp_channels.attempts.get<endpoint_tag> ().erase (vxldollar::transport::map_endpoint_to_tcp (endpoint_a));
							}
						}
					}
					else
...
void vxldollar::transport::tcp_channels::udp_fallback (vxldollar::endpoint const & endpoint_a)
{
	{
		vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
		attempts.get<endpoint_tag> ().erase (vxldollar::transport::map_endpoint_to_tcp (endpoint_a));
	}

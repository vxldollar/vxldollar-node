...
namespace
{
void reset_confirmation_heights (vxldollar::write_transaction const & transaction, vxldollar::ledger_constants & constants, vxldollar::store & store);
...
std::string vxldollar::error_cli_messages::message (int ev) const
{
	switch (static_cast<vxldollar::error_cli> (ev))
	{
		case vxldollar::error_cli::generic:
			return "Unknown error";
		case vxldollar::error_cli::parse_error:
			return "Coud not parse command line";
		case vxldollar::error_cli::invalid_arguments:
			return "Invalid arguments";
		case vxldollar::error_cli::unknown_command:
			return "Unknown command";
		case vxldollar::error_cli::database_write_error:
			return "Database write error";
		case vxldollar::error_cli::reading_config:
			return "Config file read error";
		case vxldollar::error_cli::disable_all_network:
			return "Flags --disable_tcp_realtime and --disable_udp cannot be used together";
		case vxldollar::error_cli::ambiguous_udp_options:
			return "Flags --disable_udp and --enable_udp cannot be used together";
		case vxldollar::error_cli::ambiguous_pruning_voting_options:
...
void vxldollar::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
void vxldollar::add_node_flag_options (boost::program_options::options_description & description_a)
{
	// clang-format off
	description_a.add_options()
...
std::error_code vxldollar::update_flags (vxldollar::node_flags & flags_a, boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	...
	if (config != vm.end ())
	{
		flags_a.config_overrides = vxldollar::config_overrides (config->second.as<std::vector<vxldollar::config_key_value_pair>> ());
	}
	auto rpcconfig (vm.find ("rpcconfig"));
	if (rpcconfig != vm.end ())
	{
		flags_a.rpc_config_overrides = vxldollar::config_overrides (rpcconfig->second.as<std::vector<vxldollar::config_key_value_pair>> ());
	}
	return ec;
}

std::error_code vxldollar::flags_config_conflicts (vxldollar::node_flags const & flags_a, vxldollar::node_config const & config_a)
{
	std::error_code ec;
	if (flags_a.enable_pruning && config_a.enable_voting)
	{
		ec = vxldollar::error_cli::ambiguous_pruning_voting_options;
	}
	return ec;
}
...
void database_write_lock_error (std::error_code & ec)
{
	...
	ec = vxldollar::error_cli::database_write_error;
}
...
std::error_code vxldollar::handle_node_options (boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	boost::filesystem::path data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : vxldollar::working_path ();
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != nullptr)
				{
					...
					else
					{
						...
						ec = vxldollar::error_cli::invalid_arguments;
					}
				}
				else
				{
		...
		try
		{
			...
			if (!ec)
			{
				...
				if (success)
				{
					...
					if (using_rocksdb)
					{
						vxldollar::remove_all_files_in_dir (backup_path);
						vxldollar::move_all_files_to_dir (source_path, backup_path);
						vxldollar::move_all_files_to_dir (vacuum_path, source_path);
	...
	{
		auto data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : vxldollar::working_path ();
		auto node_flags = vxldollar::inactive_node_flag_defaults ();
			...
			if (account_it != vm.cend ())
			{
				...
				if (!account.decode_account (account_str))
				{
					...
					else
					{
						...
						ec = vxldollar::error_cli::generic;
					}
				}
				else
				{
	...
	else if (vm.count ("generate_config"))
	{
		...
		{
			valid_type = true;
			vxldollar::rpc_config config{ vxldollar::dev::network_params.network };
			config.serialize_toml (toml);
		}
		else if (type == "tls")
		{
			valid_type = true;
			vxldollar::tls_config config;
			config.serialize_toml (toml);
		}
		else
		{
		...
		if (valid_type)
		{
			std::cout << "# This is an example configuration file for Vxldollar. Visit https://docs.vxldollar.org/running-a-node/configuration/ for more information.\n#\n"
					  ...
					  << "# Additional information for notable configuration options is available in https://docs.vxldollar.org/running-a-node/configuration/#notable-configuration-options\n";
	...
	else if (vm.count ("diagnostics"))
	{
		...
		key.clear ();
		vxldollar::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		vxldollar::raw_key junk1;
		junk1.clear ();
		vxldollar::uint256_union junk2 (0);
		vxldollar::kdf kdf{ inactive_node->node->config.network_params.kdf_work };
		kdf.phs (junk1, "", junk2);
		std::cout << "Testing time retrieval latency... " << std::flush;
		vxldollar::timer<std::chrono::nanoseconds> timer (vxldollar::timer_state::started);
	...
	else if (vm.count ("wallet_create"))
	{
		...
		{
			auto inactive_node = vxldollar::default_inactive_node (data_path, vm);
			auto wallet_key = vxldollar::random_wallet_id ();
				...
				if (existing != inactive_node->node->wallets.items.end ())
				{
					...
					if (!existing->second->enter_password (transaction, password))
					{
						...
						for (auto i (existing->second->store.begin (transaction)), m (existing->second->store.end ()); i != m; ++i)
						{
							...
							if (vxldollar::pub_key (key) != account)
							{
						...
						else
						{
							...
							else
							{
								bool error (true);
								{
									...
									auto transaction (node->wallets.tx_begin_write ());
									vxldollar::wallet wallet (error, transaction, node->wallets, wallet_id.to_string (), contents.str ());
								}
								if (error)
								{
									std::cerr << "Unable to import wallet\n";
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node->wallets.items.begin ()), n (node->wallets.items.end ()); i != n; ++i)
		{
			...
			{
				std::cout << vxldollar::account (j->first).to_account () << '\n';
			}
		}
	}
	...
	{
		ec = vxldollar::error_cli::unknown_command;
	}

	return ec;
}

std::unique_ptr<vxldollar::inactive_node> vxldollar::default_inactive_node (boost::filesystem::path const & path_a, boost::program_options::variables_map const & vm_a)
{
	auto node_flags = vxldollar::inactive_node_flag_defaults ();
	vxldollar::update_flags (node_flags, vm_a);
	return std::make_unique<vxldollar::inactive_node> (path_a, node_flags);
}
...
bool is_using_rocksdb (boost::filesystem::path const & data_path, boost::program_options::variables_map const & vm, std::error_code & ec)
{
	...
	{
		config_overrides = vxldollar::config_overrides (config_arg->second.as<std::vector<vxldollar::config_key_value_pair>> ());
	}

	// config override...
	auto error = vxldollar::read_node_config_toml (data_path, config, config_overrides);
	if (!error)
	{
	...
	{
		ec = vxldollar::error_cli::reading_config;
	}

...
vxldollar::confirmation_height_bounded::confirmation_height_bounded (vxldollar::ledger & ledger_a, vxldollar::write_database_queue & write_database_queue_a, std::chrono::milliseconds batch_separate_pending_min_time_a, vxldollar::logging const & logging_a, vxldollar::logger_mt & logger_a, std::atomic<bool> & stopped_a, uint64_t & batch_write_size_a, std::function<void (std::vector<std::shared_ptr<vxldollar::block>> const &)> const & notify_observers_callback_a, std::function<void (vxldollar::block_hash const &)> const & notify_block_already_cemented_observers_callback_a, std::function<uint64_t ()> const & awaiting_processing_size_callback_a) :
	ledger (ledger_a),
...
vxldollar::confirmation_height_bounded::top_and_next_hash vxldollar::confirmation_height_bounded::get_next_block (boost::optional<top_and_next_hash> const & next_in_receive_chain_a, boost::circular_buffer_space_optimized<vxldollar::block_hash> const & checkpoints_a, boost::circular_buffer_space_optimized<receive_source_pair> const & receive_source_pairs, boost::optional<receive_chain_details> & receive_details_a, vxldollar::block const & original_block)
{
...
void vxldollar::confirmation_height_bounded::process (std::shared_ptr<vxldollar::block> original_block)
{
	if (pending_empty ())
	{
		clear_process_vars ();
	...
	boost::circular_buffer_space_optimized<vxldollar::block_hash> checkpoints{ max_items };
	...
	do
	{
		...
		vxldollar::account account (block->account ());
		if (account.is_zero ())
		{
		...
		if (!hit_receive || (receive_source_pairs.size () == 1 && top_most_non_receive_block_hash != current))
		{
			...
			if ((max_batch_write_size_reached || should_output || force_write) && !pending_writes.empty ())
			{
				...
				if (write_database_queue.process (vxldollar::writer::confirmation_height))
				{
...
vxldollar::block_hash vxldollar::confirmation_height_bounded::get_least_unconfirmed_hash_from_top_level (vxldollar::transaction const & transaction_a, vxldollar::block_hash const & hash_a, vxldollar::account const & account_a, vxldollar::confirmation_height_info const & confirmation_height_info_a, uint64_t & block_height_a)
{
	vxldollar::block_hash least_unconfirmed_hash = hash_a;
...
bool vxldollar::confirmation_height_bounded::iterate (vxldollar::read_transaction const & transaction_a, uint64_t bottom_height_a, vxldollar::block_hash const & bottom_hash_a, boost::circular_buffer_space_optimized<vxldollar::block_hash> & checkpoints_a, vxldollar::block_hash & top_most_non_receive_block_hash_a, vxldollar::block_hash const & top_level_hash_a, boost::circular_buffer_space_optimized<receive_source_pair> & receive_source_pairs_a, vxldollar::account const & account_a)
{
	...
	while (!hash.is_zero () && !reached_target && !stopped)
	{
		...
		if (!source.is_zero () && !ledger.is_epoch_link (source) && ledger.store.block.exists (transaction_a, source))
		{
			...
			auto next = !sideband.successor.is_zero () && sideband.successor != top_level_hash_a ? boost::optional<vxldollar::block_hash> (sideband.successor) : boost::none;
...
void vxldollar::confirmation_height_bounded::prepare_iterated_blocks_for_cementing (preparation_data & preparation_data_a)
{
...
void vxldollar::confirmation_height_bounded::cement_blocks (vxldollar::write_guard & scoped_write_guard_a)
{
	...
	std::vector<std::shared_ptr<vxldollar::block>> cemented_blocks;
	...
	vxldollar::timer<> cemented_batch_timer;
	auto error = false;
	{
		// This only writes to the confirmation_height table and is the only place to do so in a single process
		auto transaction (ledger.store.tx_begin_write ({}, { vxldollar::tables::confirmation_height }));
		...
		while (!error && !pending_writes.empty ())
		{
			...
			auto write_confirmation_height = [&account, &ledger = ledger, &transaction] (uint64_t num_blocks_cemented, uint64_t confirmation_height, vxldollar::block_hash const & confirmed_frontier) {
#ifndef NDEBUG
				// Extra debug checks
				vxldollar::confirmation_height_info confirmation_height_info;
				ledger.store.confirmation_height.get (transaction, account, confirmation_height_info);
...
#endif
				ledger.store.confirmation_height.put (transaction, account, vxldollar::confirmation_height_info{ confirmation_height, confirmed_frontier });
				ledger.cache.cemented_count += num_blocks_cemented;
				ledger.stats.add (vxldollar::stat::type::confirmation_height, vxldollar::stat::detail::blocks_confirmed, vxldollar::stat::dir::in, num_blocks_cemented);
				ledger.stats.add (vxldollar::stat::type::confirmation_height, vxldollar::stat::detail::blocks_confirmed_bounded, vxldollar::stat::dir::in, num_blocks_cemented);
			};
			...
			if (pending.top_height > confirmation_height_info.height)
			{
				...
				vxldollar::block_hash new_cemented_frontier;
				...
				for (auto num_blocks_iterated = 0; num_blocks_confirmed - num_blocks_iterated != 0; ++num_blocks_iterated)
				{
					...
					if (cemented_blocks.size () > batch_write_size + (batch_write_size / 10))
					{
						...
						{
							scoped_write_guard_a = write_database_queue.wait (vxldollar::writer::confirmation_height);
							transaction.renew ();
						}
...
bool vxldollar::confirmation_height_bounded::pending_empty () const
{
	return pending_writes.empty ();
}

void vxldollar::confirmation_height_bounded::clear_process_vars ()
{
	accounts_confirmed_info.clear ();
	accounts_confirmed_info_size = 0;
}

vxldollar::confirmation_height_bounded::receive_chain_details::receive_chain_details (vxldollar::account const & account_a, uint64_t height_a, vxldollar::block_hash const & hash_a, vxldollar::block_hash const & top_level_a, boost::optional<vxldollar::block_hash> next_a, uint64_t bottom_height_a, vxldollar::block_hash const & bottom_most_a) :
	account (account_a),
	height (height_a),
	hash (hash_a),
...
vxldollar::confirmation_height_bounded::write_details::write_details (vxldollar::account const & account_a, uint64_t bottom_height_a, vxldollar::block_hash const & bottom_hash_a, uint64_t top_height_a, vxldollar::block_hash const & top_hash_a) :
	account (account_a),
	bottom_height (bottom_height_a),
...
vxldollar::confirmation_height_bounded::receive_source_pair::receive_source_pair (confirmation_height_bounded::receive_chain_details const & receive_details_a, const block_hash & source_a) :
	receive_details (receive_details_a),
	source_hash (source_a)
{
}

vxldollar::confirmation_height_bounded::confirmed_info::confirmed_info (uint64_t confirmed_height_a, vxldollar::block_hash const & iterated_frontier_a) :
	confirmed_height (confirmed_height_a),
	iterated_frontier (iterated_frontier_a)
{
}

std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (confirmation_height_bounded & confirmation_height_bounded, std::string const & name_a)
{

...
bool vxldollar::prioritization::value_type::operator< (value_type const & other_a) const
{
...
void vxldollar::prioritization::next ()
{
	++current;
...
void vxldollar::prioritization::seek ()
{
	next ();
...
void vxldollar::prioritization::populate_schedule ()
{
 ...
 */
vxldollar::prioritization::prioritization (uint64_t maximum) :
	maximum{ maximum }
{
	static std::size_t constexpr bucket_count = 129;
	buckets.resize (bucket_count);
	vxldollar::uint128_t minimum{ 1 };
 ...
 */
void vxldollar::prioritization::push (uint64_t time, std::shared_ptr<vxldollar::block> block)
{
	auto was_empty = empty ();
	auto block_has_balance = block->type () == vxldollar::block_type::state || block->type () == vxldollar::block_type::send;
...
std::shared_ptr<vxldollar::block> vxldollar::prioritization::top () const
{
	debug_assert (!empty ());
...
void vxldollar::prioritization::pop ()
{
	debug_assert (!empty ());
	debug_assert (!buckets[*current].empty ());
...
std::size_t vxldollar::prioritization::size () const
{
	std::size_t result{ 0 };
...
std::size_t vxldollar::prioritization::bucket_count () const
{
	return buckets.size ();
}

/** Returns number of items in bucket with index 'index' */
std::size_t vxldollar::prioritization::bucket_size (std::size_t index) const
{
	return buckets[index].size ();
}

/** Returns true if all buckets are empty */
bool vxldollar::prioritization::empty () const
{
...
void vxldollar::prioritization::dump () const
{
...
std::unique_ptr<vxldollar::container_info_component> vxldollar::prioritization::collect_container_info (std::string const & name)
{

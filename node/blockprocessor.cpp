...
std::chrono::milliseconds constexpr vxldollar::block_processor::confirmation_request_delay;

vxldollar::block_post_events::block_post_events (std::function<vxldollar::read_transaction ()> && get_transaction_a) :
	get_transaction (std::move (get_transaction_a))
{
}

vxldollar::block_post_events::~block_post_events ()
{
...
vxldollar::block_processor::block_processor (vxldollar::node & node_a, vxldollar::write_database_queue & write_database_queue_a) :
...
{
	state_block_signature_verification.blocks_verified_callback = [this] (std::deque<vxldollar::state_block_signature_verification::value_type> & items, std::vector<int> const & verifications, std::vector<vxldollar::block_hash> const & hashes, std::vector<vxldollar::signature> const & blocks_signatures) {
		...
		{
			{
				...
				vxldollar::lock_guard<vxldollar::mutex> guard (this->mutex);
			}
			this->condition.notify_all ();
		}
	};
	processing_thread = std::thread ([this] () {
		vxldollar::thread_role::set (vxldollar::thread_role::name::block_processing);
		this->process_blocks ();
	});
}
...
void vxldollar::block_processor::stop ()
{
	{
...
void vxldollar::block_processor::flush ()
{
	node.checker.flush ();
	flushing = true;
...
std::size_t vxldollar::block_processor::size ()
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	return (blocks.size () + state_block_signature_verification.size () + forced.size ());
}

bool vxldollar::block_processor::full ()
{
	return size () >= node.flags.block_processor_full_size;
}

bool vxldollar::block_processor::half_full ()
{
	return size () >= node.flags.block_processor_full_size / 2;
}

void vxldollar::block_processor::add (std::shared_ptr<vxldollar::block> const & block_a)
{
	vxldollar::unchecked_info info (block_a, 0, vxldollar::signature_verification::unknown);
	add (info);
}

void vxldollar::block_processor::add (vxldollar::unchecked_info const & info_a)
{
	...
	if (verified == vxldollar::signature_verification::unknown && (block->type () == vxldollar::block_type::state || block->type () == vxldollar::block_type::open || !account.is_zero ()))
	{
...
void vxldollar::block_processor::add_local (vxldollar::unchecked_info const & info_a)
{
	release_assert (info_a.verified == vxldollar::signature_verification::unknown && (info_a.block->type () == vxldollar::block_type::state || !info_a.account.is_zero ()));
...
void vxldollar::block_processor::force (std::shared_ptr<vxldollar::block> const & block_a)
{
	{
		vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
...
void vxldollar::block_processor::wait_write ()
{
	vxldollar::lock_guard<vxldollar::mutex> lock (mutex);
	awaiting_write = true;
}

void vxldollar::block_processor::process_blocks ()
{
	vxldollar::unique_lock<vxldollar::mutex> lock (mutex);
	while (!stopped)
	{
...
bool vxldollar::block_processor::should_log ()
{
	auto result (false);
...
bool vxldollar::block_processor::have_blocks_ready ()
{
	debug_assert (!mutex.try_lock ());
	return !blocks.empty () || !forced.empty ();
}

bool vxldollar::block_processor::have_blocks ()
{
	debug_assert (!mutex.try_lock ());
	return have_blocks_ready () || state_block_signature_verification.size () != 0;
}

void vxldollar::block_processor::process_verified_state_blocks (std::deque<vxldollar::state_block_signature_verification::value_type> & items, std::vector<int> const & verifications, std::vector<vxldollar::block_hash> const & hashes, std::vector<vxldollar::signature> const & blocks_signatures)
{
	{
		...
		for (auto i (0); i < verifications.size (); ++i)
		{
			...
			if (!block->link ().is_zero () && node.ledger.is_epoch_link (block->link ()))
			{
				...
				{
					verified = vxldollar::signature_verification::valid_epoch;
					blocks.emplace_back (block, account, verified);
				}
				else
				{
					// Possible regular state blocks with epoch link (send subtype)
					verified = vxldollar::signature_verification::unknown;
					blocks.emplace_back (block, account, verified);
				}
			}
			...
			{
				// Non epoch blocks
				verified = vxldollar::signature_verification::valid;
				blocks.emplace_back (block, account, verified);
			}
			else
			{
...
void vxldollar::block_processor::process_batch (vxldollar::unique_lock<vxldollar::mutex> & lock_a)
{
	auto scoped_write_guard = write_database_queue.wait (vxldollar::writer::process_batch);
	...
	vxldollar::timer<std::chrono::milliseconds> timer_l;
	lock_a.lock ();
	timer_l.start ();
	// Processing blocks
	...
	while (have_blocks_ready () && (!deadline_reached () || !processor_batch_reached ()) && !awaiting_write && !store_batch_reached ())
	{
		...
		{
			info = vxldollar::unchecked_info (forced.front (), 0, vxldollar::signature_verification::unknown);
			forced.pop_front ();
			hash = info.block->hash ();
			force = true;
		...
		if (force)
		{
			...
			if (successor != nullptr && successor->hash () != hash)
			{
				...
				{
					node.stats.inc (vxldollar::stat::type::ledger, vxldollar::stat::detail::rollback_failed);
					node.logger.always_log (vxldollar::severity_level::error, boost::str (boost::format ("Failed to roll back %1% because it or a successor was confirmed") % successor->hash ().to_string ()));
				}
...
void vxldollar::block_processor::process_live (vxldollar::transaction const & transaction_a, vxldollar::block_hash const & hash_a, std::shared_ptr<vxldollar::block> const & block_a, vxldollar::process_return const & process_return_a, vxldollar::block_origin const origin_a)
{
	...
	if (origin_a == vxldollar::block_origin::local)
	{
		node.network.flood_block_initial (block_a);
	}
	else if (!node.flags.disable_block_processor_republishing)
	{
		node.network.flood_block (block_a, vxldollar::buffer_drop_policy::limiter);
	}

	if (node.websocket_server && node.websocket_server->any_subscriber (vxldollar::websocket::topic::new_unconfirmed_block))
	{
		node.websocket_server->broadcast (vxldollar::websocket::message_builder ().new_block_arrived (*block_a));
	}
}

vxldollar::process_return vxldollar::block_processor::process_one (vxldollar::write_transaction const & transaction_a, block_post_events & events_a, vxldollar::unchecked_info info_a, bool const forced_a, vxldollar::block_origin const origin_a)
{
	...
	switch (result.code)
	{
		case vxldollar::process_result::progress:
		{
			...
			{
				events_a.events.emplace_back ([this, hash, block = info_a.block, result, origin_a] (vxldollar::transaction const & post_event_transaction_a) { process_live (post_event_transaction_a, hash, block, result, origin_a); });
			}
			queue_unchecked (transaction_a, hash);
			/* For send blocks check epoch open unchecked (gap pending).
			For state blocks check only send subtype and only if block epoch is not last epoch.
			If epoch is last, then pending entry shouldn't trigger same epoch open block for destination account. */
			if (block->type () == vxldollar::block_type::send || (block->type () == vxldollar::block_type::state && block->sideband ().details.is_send && std::underlying_type_t<vxldollar::epoch> (block->sideband ().details.epoch) < std::underlying_type_t<vxldollar::epoch> (vxldollar::epoch::max)))
			{
		...
		case vxldollar::process_result::gap_previous:
		{
			...
			events_a.events.emplace_back ([this, hash] (vxldollar::transaction const & /* unused */) { this->node.gap_cache.add (hash); });
			node.stats.inc (vxldollar::stat::type::ledger, vxldollar::stat::detail::gap_previous);
			break;
		}
		case vxldollar::process_result::gap_source:
		{
			...
			events_a.events.emplace_back ([this, hash] (vxldollar::transaction const & /* unused */) { this->node.gap_cache.add (hash); });
			node.stats.inc (vxldollar::stat::type::ledger, vxldollar::stat::detail::gap_source);
			break;
		}
		...
		case vxldollar::process_result::old:
		{
			...
			node.stats.inc (vxldollar::stat::type::ledger, vxldollar::stat::detail::old);
			break;
		}
		case vxldollar::process_result::bad_signature:
		{
			...
			events_a.events.emplace_back ([this, hash, info_a] (vxldollar::transaction const & /* unused */) { requeue_invalid (hash, info_a); });
			break;
		}
		...
		{
			node.stats.inc (vxldollar::stat::type::ledger, vxldollar::stat::detail::fork);
			events_a.events.emplace_back ([this, block] (vxldollar::transaction const &) { this->node.active.publish (block); });
			if (node.config.logging.ledger_logging ())
			{
		...
		case vxldollar::process_result::opened_burn_account:
		{
		...
		case vxldollar::process_result::representative_mismatch:
		{
			if (node.config.logging.ledger_logging ())
			{
		...
		case vxldollar::process_result::insufficient_work:
		{
			if (node.config.logging.ledger_logging ())
			{
				node.logger.try_log (boost::str (boost::format ("Insufficient work for %1% : %2% (difficulty %3%)") % hash.to_string () % vxldollar::to_string_hex (block->block_work ()) % vxldollar::to_string_hex (node.network_params.work.difficulty (*block))));
			}
			break;
		}
	}
...
vxldollar::process_return vxldollar::block_processor::process_one (vxldollar::write_transaction const & transaction_a, block_post_events & events_a, std::shared_ptr<vxldollar::block> const & block_a)
{
...
void vxldollar::block_processor::queue_unchecked (vxldollar::write_transaction const & transaction_a, vxldollar::hash_or_account const & hash_or_account_a)
{
	node.unchecked.trigger (hash_or_account_a);
	node.gap_cache.erase (hash_or_account_a.hash);
}

void vxldollar::block_processor::requeue_invalid (vxldollar::block_hash const & hash_a, vxldollar::unchecked_info const & info_a)
{
	debug_assert (hash_a == info_a.block->hash ());
	node.bootstrap_initiator.lazy_requeue (hash_a, info_a.block->previous ());
}

std::unique_ptr<vxldollar::container_info_component> vxldollar::collect_container_info (block_processor & block_processor, std::string const & name)
{
	std::size_t blocks_count;
	std::size_t forced_count;

	{
		vxldollar::lock_guard<vxldollar::mutex> guard (block_processor.mutex);

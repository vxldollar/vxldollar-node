...
class peer_exclusion final
{
	...
	public:
		...
		decltype (std::declval<vxldollar::tcp_endpoint> ().address ()) address;
		uint64_t score;
	};
...
public:
	...
	uint64_t add (vxldollar::tcp_endpoint const &, std::size_t const);
	bool check (vxldollar::tcp_endpoint const &);
	void remove (vxldollar::tcp_endpoint const &);

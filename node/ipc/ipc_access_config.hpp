...
namespace ipc
{
	...
	{
	public:
		std::unordered_set<vxldollar::ipc::access_permission, enum_hash> permissions;
	...
	public:
		...
		std::string id;
		std::vector<vxldollar::ipc::access_role> roles;
		void clear () override;
	};
	...
	{
	public:
		bool has_access (std::string const & credentials_a, vxldollar::ipc::access_permission permission_a) const;
		bool has_access_to_all (std::string const & credentials_a, std::initializer_list<vxldollar::ipc::access_permission> permissions_a) const;
		bool has_access_to_oneof (std::string const & credentials_a, std::initializer_list<vxldollar::ipc::access_permission> permissions_a) const;
		vxldollar::error deserialize_toml (vxldollar::tomlconfig &);

	private:
		/** Process allow and deny entries for the given subject */
		void set_effective_permissions (vxldollar::ipc::access_subject & subject_a, std::shared_ptr<cpptoml::table> const & config_subject_a);

		/** Clear current users, roles and default permissions */
		void clear ();

		std::unordered_map<std::string, vxldollar::ipc::access_user> users;
		std::unordered_map<std::string, vxldollar::ipc::access_role> roles;

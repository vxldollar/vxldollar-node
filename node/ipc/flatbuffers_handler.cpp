...
namespace
{
auto handler_map = vxldollar::ipc::action_handler::handler_map ();
...
vxldollar::ipc::flatbuffers_handler::flatbuffers_handler (vxldollar::node & node_a, vxldollar::ipc::ipc_server & ipc_server_a, std::shared_ptr<vxldollar::ipc::subscriber> const & subscriber_a, vxldollar::ipc::ipc_config const & ipc_config_a) :
	node (node_a),
...
std::shared_ptr<flatbuffers::Parser> vxldollar::ipc::flatbuffers_handler::make_flatbuffers_parser (vxldollar::ipc::ipc_config const & ipc_config_a)
{
	...
	if (!api_path)
	{
		throw vxldollar::error ("Internal IPC error: unable to find api path");
	}

	char const * include_directories[] = { api_path->string ().c_str (), nullptr };
	std::string schemafile;
	if (!flatbuffers::LoadFile ((*api_path / "vxldollarapi.fbs").string ().c_str (), false, &schemafile))
	{
		throw vxldollar::error ("Internal IPC error: unable to load schema file");
	}
	...
	if (!parse_success)
	{
		...
		throw vxldollar::error (parser_error);
	}
	return parser;
}

void vxldollar::ipc::flatbuffers_handler::process_json (uint8_t const * message_buffer_a, std::size_t buffer_size_a,
...
void vxldollar::ipc::flatbuffers_handler::process (uint8_t const * message_buffer_a, std::size_t buffer_size_a,
std::function<void (std::shared_ptr<flatbuffers::FlatBufferBuilder> const &)> const & response_handler)
{
	...
	try
	{
		...
		if (ipc_config.flatbuffers.verify_buffers)
		{
			...
			{
				throw vxldollar::error ("Envelope buffer did not pass verifier");
			}
		}

		auto incoming = vxldollarapi::GetEnvelope (message_buffer_a);
		if (incoming == nullptr)
		{
			vxldollar::error err ("Invalid message");
		...
		{
			vxldollar::error err ("Unknown message type");
			actionhandler->make_error (err.error_code_as_int (), err.get_message ());
		}
	}
	catch (vxldollar::error const & err)

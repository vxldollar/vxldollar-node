...
vxldollar::daemon_config::daemon_config (boost::filesystem::path const & data_path_a, vxldollar::network_params & network_params) :
	node{ network_params },
	data_path{ data_path_a }
{
}

vxldollar::error vxldollar::daemon_config::serialize_toml (vxldollar::tomlconfig & toml)
{
	vxldollar::tomlconfig rpc_l;
	rpc.serialize_toml (rpc_l);
	rpc_l.doc ("enable", "Enable or disable RPC\ntype:bool");
	rpc_l.put ("enable", rpc_enable);
	toml.put_child ("rpc", rpc_l);

	vxldollar::tomlconfig node_l;
	node.serialize_toml (node_l);
	vxldollar::tomlconfig node (node_l);
	toml.put_child ("node", node);

	vxldollar::tomlconfig opencl_l;
	...
	vxldollar::tomlconfig pow_server_l;
	pow_server.serialize_toml (pow_server_l);
	vxldollar::tomlconfig pow_server (pow_server_l);
	toml.put_child ("vxldollar_pow_server", pow_server);

	return toml.get_error ();
}

vxldollar::error vxldollar::daemon_config::deserialize_toml (vxldollar::tomlconfig & toml)
{
	...
	auto pow_l (toml.get_optional_child ("vxldollar_pow_server"));
...
vxldollar::error vxldollar::read_node_config_toml (boost::filesystem::path const & data_path_a, vxldollar::daemon_config & config_a, std::vector<std::string> const & config_overrides)
{
	vxldollar::error error;
	auto toml_config_path = vxldollar::get_node_toml_config_path (data_path_a);
	auto toml_qt_config_path = vxldollar::get_qtwallet_toml_config_path (data_path_a);
